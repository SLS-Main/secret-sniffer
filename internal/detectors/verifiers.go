...
func verifyCalendly(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://api.calendly.com/users/me", nil)
	req.Header.Set("Authorization", "Bearer "+secret)
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			var response struct {
			...
			if json.Unmarshal(body, &response) == nil && response.Resource != nil && strings.HasPrefix(response.Resource.URI, "https://api.calendly.com/users/") && response.Resource.Name != "" && response.Resource.Slug != "" && response.Resource.Email != "" && strings.HasPrefix(response.Resource.SchedulingURL, "https://calendly.com/") && response.Resource.Timezone != "" && response.Resource.AvatarURL != nil && jsonNullOrString(response.Resource.AvatarURL) && response.Resource.CreatedAt != "" && response.Resource.UpdatedAt != "" && strings.HasPrefix(response.Resource.CurrentOrganization, "https://api.calendly.com/organizations/") && response.Resource.ResourceType == "User" && slices.Contains([]string{"en", "fr", "es", "de", "pt", "ps"}, response.Resource.Locale) {
				return VerificationResult{Status: VerificationVerified}, true
			}
			return unknownVerificationResult("provider_response", "provider returned an unexpected verification response"), true
		}
		...
		if statusCode == http.StatusForbidden {
			...
			if json.Unmarshal(body, &response) == nil && response.Title == "Insufficient scope" && slices.Contains(response.RequiredScopes, "users:read") {
...
func verifyRender(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://api.render.com/v1/users", nil)
	req.Header.Set("Authorization", "Bearer "+secret)
	req.Header.Set("Accept", "application/json")
	return verifyTopLevelStringIdentityRequest(ctx, req, "id", "email", "name")
}
...
func verifyResend(ctx context.Context, secret string) VerificationResult {
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			...
			if json.Unmarshal(body, &response) == nil && response.Object == "list" && response.HasMore != nil && response.Data != nil {
				valid := true
				for _, raw := range response.Data {
					var item map[string]json.RawMessage
					var id, name, created string
					if json.Unmarshal(raw, &item) != nil || json.Unmarshal(item["id"], &id) != nil || id == "" || json.Unmarshal(item["name"], &name) != nil || name == "" || json.Unmarshal(item["created_at"], &created) != nil || created == "" || item["last_used_at"] == nil || !jsonNullOrString(item["last_used_at"]) {
						valid = false
						break
					}
				}
		...
		switch {
		case statusCode == http.StatusUnauthorized && response.Name == "restricted_api_key":
			return VerificationResult{Status: VerificationVerified, Message: "provider authenticated a restricted key"}, true
		case statusCode == http.StatusBadRequest && response.StatusCode == http.StatusBadRequest && response.Name == "validation_error" && response.Message == "API key is invalid":
			return invalidCredentialResult(), true
		case statusCode == http.StatusForbidden && (response.Name == "restricted_api_key" || response.Name == "suspended_api_key"):
			return invalidCredentialResult(), true
		case statusCode == http.StatusForbidden && response.Name == "invalid_permission":
...
func verifySegment(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://platform.segmentapis.com/v1beta/workspaces", nil)
	req.Header.Set("Authorization", "Bearer "+secret)
	return verifyJSONReadRequestWithInvalidUnauthorized(ctx, req, func(body []byte) bool {
		return jsonHasTopLevelArray(body, "workspaces")
	})
}
...
func verifyWorkOS(ctx context.Context, secret string) VerificationResult {
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		switch {
		case statusCode >= 200 && statusCode < 300 && jsonTopLevelStringEquals(body, "object", "list") && jsonHasTopLevelArray(body, "data") && jsonHasTopLevelObject(body, "list_metadata"):
			return VerificationResult{Status: VerificationVerified}, true
		case statusCode >= 200 && statusCode < 300:
			return unknownVerificationResult("provider_response", "provider returned an unexpected verification response"), true
		case statusCode == http.StatusUnauthorized:
...
func verifyDeepgram(ctx context.Context, secret string) VerificationResult {
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			...
			if json.Unmarshal(body, &response) == nil && response.Projects != nil {
				valid := true
				for _, project := range response.Projects {
					for _, field := range []string{"project_id", "name"} {
						if value, ok := project[field]; ok {
		...
		_ = json.Unmarshal(body, &response)
		switch {
		case (statusCode == http.StatusUnauthorized || statusCode == http.StatusForbidden) && response.ErrorCode == "INSUFFICIENT_PERMISSIONS":
			return VerificationResult{Status: VerificationVerified, Message: "provider authenticated a token with insufficient permission"}, true
		case statusCode == http.StatusUnauthorized && response.ErrorCode == "INVALID_AUTH" && containsAnyFold(response.ErrorMsg, "invalid credentials"):
			return invalidCredentialResult(), true
		case statusCode == http.StatusUnauthorized || statusCode == http.StatusForbidden:
			return unknownVerificationResult("authorization", "provider authentication response was ambiguous"), true
		default:
...
func verifyCloseCRM(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://api.close.com/api/v1/me/?_fields=id", nil)
	req.SetBasicAuth(secret, "")
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		switch {
		case statusCode == http.StatusOK && jsonHasTopLevelStrings(body, "id"):
			var response struct {
				ID string `json:"id"`
			}
			if json.Unmarshal(body, &response) == nil && strings.HasPrefix(response.ID, "user_") {
				return VerificationResult{Status: VerificationVerified}, true
			}
			return unknownVerificationResult("provider_response", "provider returned an unexpected verification response"), true
		case statusCode >= 200 && statusCode < 300:
		...
		case statusCode == http.StatusPaymentRequired || statusCode == http.StatusForbidden:
			return unknownVerificationResult("authorization", "provider restricted account access"), true
		case statusCode == http.StatusTooManyRequests:
			return unknownVerificationResult("rate_limited", "provider rate limited verification"), true
		case statusCode >= 500:
			return unknownVerificationResult("provider", "provider unavailable"), true
...
func verifyBrevo(ctx context.Context, secret string) VerificationResult {
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			...
			if json.Unmarshal(body, &response) == nil && response.OrganizationID != "" && response.UserID > 0 && response.Enterprise != nil && response.Plan != nil && jsonObject(response.Relay) {
				return VerificationResult{Status: VerificationVerified}, true
			}
			return unknownVerificationResult("provider_response", "provider returned an unexpected verification response"), true
		}
		if statusCode == http.StatusBadRequest || statusCode == http.StatusUnauthorized {
			...
			if json.Unmarshal(body, &response) == nil && (containsAnyFold(response.Code, "unauthorized") || containsAnyFold(response.Message, "api-key not found", "key not found", "authentication failed")) {
				return invalidCredentialResult(), true
			}
			return unknownVerificationResult("authorization", "provider authentication response was ambiguous"), true
		}
...
func verifyDeepSeek(ctx context.Context, secret string) VerificationResult {
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		...
		case statusCode >= 200 && statusCode < 300:
			...
			if json.Unmarshal(body, &response) == nil && response.Object == "list" && response.Data != nil {
				valid := true
...
func verifyYNAB(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://api.ynab.com/v1/user", nil)
	req.Header.Set("Authorization", "Bearer "+secret)
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			var response struct {
			...
			if json.Unmarshal(body, &response) == nil && response.Data.User.ID != "" {
				return VerificationResult{Status: VerificationVerified}, true
			}
		}
		if statusCode == http.StatusOK {
		...
		_ = json.Unmarshal(body, &response)
		switch {
		case statusCode == http.StatusUnauthorized && response.Error.ID == "401":
			return invalidCredentialResult(), true
		case statusCode == http.StatusForbidden && slices.Contains([]string{"403.1", "403.2", "403.3", "403.4", "403.5"}, response.Error.ID):
			return VerificationResult{Status: VerificationVerified, Message: "provider authenticated a restricted account or token"}, true
		case statusCode == http.StatusUnauthorized || statusCode == http.StatusForbidden:
			return unknownVerificationResult("authorization", "provider authentication response was ambiguous"), true
		default:
			return verifyJSONReadClassification(statusCode, body)
...
func verifyTogglTrack(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://api.track.toggl.com/api/v9/me?with_related_data=false", nil)
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			...
			if json.Unmarshal(body, &response) == nil && response.ID > 0 && strings.TrimSpace(response.Email) != "" {
				return VerificationResult{Status: VerificationVerified}, true
			}
			return unknownVerificationResult("provider_response", "provider returned an unexpected verification response"), true
		}
		switch {
		case statusCode == http.StatusForbidden && strings.EqualFold(strings.TrimSpace(string(body)), "Incorrect username and/or password"):
			return invalidCredentialResult(), true
		...
		case statusCode == http.StatusPaymentRequired || statusCode == http.StatusTooManyRequests:
			return unknownVerificationResult("rate_limited", "provider limited verification"), true
		case statusCode >= 500:
			return unknownVerificationResult("provider", "provider unavailable"), true
		default:
			return unknownVerificationResult("provider_response", "provider returned an ambiguous response"), true
...
func verifyLunchMoney(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://api.lunchmoney.dev/v2/me", nil)
	req.Header.Set("Authorization", "Bearer "+secret)
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			var response struct {
			...
			if json.Unmarshal(body, &response) == nil && response.Name != "" && response.Email != "" && response.ID > 0 && response.AccountID > 0 && response.BudgetName != "" && response.PrimaryCurrency != "" && response.PrimaryCurrency == strings.ToLower(response.PrimaryCurrency) && response.APIKeyLabel != nil && jsonNullOrString(response.APIKeyLabel) {
				return VerificationResult{Status: VerificationVerified}, true
			}
			return unknownVerificationResult("provider_response", "provider returned an unexpected verification response"), true
		}
		if statusCode == http.StatusUnauthorized {
			...
			if json.Unmarshal(body, &response) == nil && response.Message == "Unauthorized" && len(response.Errors) > 0 && containsAnyFold(response.Errors[0].Message, "access token does not exist") {
				return invalidCredentialResult(), true
			}
			return unknownVerificationResult("authorization", "provider authentication response was ambiguous"), true
		}
...
func verifyLiveblocks(ctx context.Context, secret string) VerificationResult {
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			...
			if json.Unmarshal(body, &response) == nil && response.NextCursor != nil && jsonNullOrString(response.NextCursor) && response.Data != nil {
				valid := true
				for _, raw := range response.Data {
					var room map[string]json.RawMessage
					var roomType, id, createdAt, organizationID string
					if json.Unmarshal(raw, &room) != nil || json.Unmarshal(room["type"], &roomType) != nil || roomType != "room" || json.Unmarshal(room["id"], &id) != nil || id == "" || json.Unmarshal(room["createdAt"], &createdAt) != nil || createdAt == "" || json.Unmarshal(room["organizationId"], &organizationID) != nil || organizationID == "" || !jsonObjectOrEmpty(room["metadata"]) || !jsonArray(room["defaultAccesses"]) || !jsonObjectOrEmpty(room["usersAccesses"]) || !jsonObjectOrEmpty(room["groupsAccesses"]) {
						valid = false
						break
					}
				}
		...
		_ = json.Unmarshal(body, &response)
		switch response.Error {
		case "MISSING_SECRET_KEY", "WRONG_KEY_USED", "INVALID_SECRET_KEY", "INVALID_PUBLIC_KEY":
			return invalidCredentialResult(), true
		case "INVALID_MANAGEMENT_API_ACCESS_TOKEN_SCOPE", "FEATURE_NOT_ENABLED", "PUBLIC_KEY_NOT_ENABLED":
			return unknownVerificationResult("authorization", "provider could not authorize verification"), true
		}
		return verifyJSONReadClassification(statusCode, body)
	})
...
func verifyTwitch(ctx context.Context, secret string) VerificationResult {
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode >= 200 && statusCode < 300 {
			...
			if json.Unmarshal(body, &response) == nil && response.ClientID != "" && response.Scopes != nil && response.ExpiresIn > 0 {
				return VerificationResult{Status: VerificationVerified}, true
			}
			return unknownVerificationResult("provider_response", "provider returned an unexpected verification response"), true
		}
		if statusCode == http.StatusUnauthorized {
			...
			if json.Unmarshal(body, &response) == nil && response.Status == http.StatusUnauthorized && strings.EqualFold(response.Message, "invalid access token") {
				return invalidCredentialResult(), true
			}
			return unknownVerificationResult("authorization", "provider authentication response was ambiguous"), true
		}
...
func verifyPushbullet(ctx context.Context, secret string) VerificationResult {
	...
	return verifyJSONReadRequestWithInvalidUnauthorized(ctx, req, func(body []byte) bool {
		return jsonHasTopLevelStrings(body, "iden")
	})
}
...
func verifyEverhour(ctx context.Context, secret string) VerificationResult {
	...
	req.Header.Set("X-Api-Key", secret)
	req.Header.Set("Accept", "application/json")
	req.Header.Set("X-Accept-Version", "1.2")
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			var response struct {
				ID     int64  `json:"id"`
			...
			if json.Unmarshal(body, &response) == nil && response.ID > 0 && response.Name != "" && slices.Contains([]string{"admin", "supervisor", "member"}, response.Role) && slices.Contains([]string{"active", "invited", "pending", "removed"}, response.Status) {
				return VerificationResult{Status: VerificationVerified}, true
			}
			return unknownVerificationResult("provider_response", "provider returned an unexpected verification response"), true
		}
...
func verifyLoyverse(ctx context.Context, secret string) VerificationResult {
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			...
			if json.Unmarshal(body, &response) == nil && validNonzeroUUID(response.ID) {
				return VerificationResult{Status: VerificationVerified}, true
			}
			return unknownVerificationResult("provider_response", "provider returned an unexpected verification response"), true
		}
		...
		switch {
		case statusCode == http.StatusPaymentRequired && code == "PAYMENT_REQUIRED":
		...
		case statusCode == http.StatusUnauthorized && code == "UNAUTHORIZED":
			return invalidCredentialResult(), true
		case statusCode == http.StatusForbidden && code == "FORBIDDEN":
			return VerificationResult{Status: VerificationVerified, Message: "provider authenticated a token with insufficient permission"}, true
		case statusCode == http.StatusUnauthorized || statusCode == http.StatusForbidden:
			return unknownVerificationResult("authorization", "provider authentication response was ambiguous"), true
		default:
...
func verifyMeisterTask(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://www.meistertask.com/api/persons/me", nil)
	req.Header.Set("Authorization", "Bearer "+secret)
	return verifyJSONReadRequest(ctx, req, func(body []byte) bool {
		var response struct {
			ID    int64  `json:"id"`
		...
		return json.Unmarshal(body, &response) == nil && response.ID > 0 && response.Email != ""
	})
}
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			var response struct {
				...
				HourlyQuota *float64        `json:"hourly_quota"`
				Backlog     *float64        `json:"backlog"`
				Stats       json.RawMessage `json:"stats"`
			}
			if json.Unmarshal(body, &response) == nil && response.Username != "" && response.PublicID != "" && response.Reputation != nil && response.HourlyQuota != nil && response.Backlog != nil && mandrillStatsValid(response.Stats) {
				return VerificationResult{Status: VerificationVerified}, true
			}
			return unknownVerificationResult("provider_response", "provider returned an unexpected verification response"), true
		}
		...
		_ = json.Unmarshal(body, &response)
		switch {
		case statusCode == http.StatusUnauthorized && response.Status == "error" && response.Name == "Invalid_Key":
			return invalidCredentialResult(), true
		case statusCode == http.StatusUnauthorized:
			return unknownVerificationResult("authorization", "provider authentication response was ambiguous"), true
		default:
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			var response []struct {
				...
				PermissionToken []int           `json:"permission_token"`
			}
			if json.Unmarshal(body, &response) == nil && len(response) > 0 {
				for _, token := range response {
					if token.Header != "" || jsonObject(token.Caveats) || jsonObject(token.Nonce) || token.PermissionToken != nil {
						return VerificationResult{Status: VerificationVerified}, true
					}
				}
			}
		...
		if statusCode == http.StatusBadRequest {
			...
			if json.Unmarshal(body, &response) == nil && containsAnyFold(response.Error, "invalid token", "no tokens found", "malformed", "bad token") {
				return invalidCredentialResult(), true
			}
			return unknownVerificationResult("provider_response", "provider returned an ambiguous response"), true
		}
...
func verifyLemonSqueezy(ctx context.Context, secret string) VerificationResult {
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			...
			if json.Unmarshal(body, &response) == nil && response.Data != nil && response.Data.Type == "users" && response.Data.ID != "" && jsonObject(response.Data.Attributes) {
				return VerificationResult{Status: VerificationVerified}, true
			}
			return unknownVerificationResult("provider_response", "provider returned an unexpected verification response"), true
		}
		if statusCode == http.StatusUnauthorized {
			...
			if json.Unmarshal(body, &response) == nil {
				for _, providerErr := range response.Errors {
					if providerErr.Status == "401" && strings.EqualFold(providerErr.Detail, "Unauthenticated.") {
						return invalidCredentialResult(), true
					}
				}
			}
...
func verifyRecharge(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://api.rechargeapps.com/token_information", nil)
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			var response struct {
				TokenInformation *struct {
					Name   string   `json:"name"`
					Scopes []string `json:"scopes"`
				} `json:"token_information"`
			}
			if json.Unmarshal(body, &response) == nil && response.TokenInformation != nil && strings.TrimSpace(response.TokenInformation.Name) != "" && response.TokenInformation.Scopes != nil {
				valid := true
				for _, scope := range response.TokenInformation.Scopes {
		...
		case http.StatusForbidden:
			return VerificationResult{Status: VerificationVerified, Message: "provider authenticated a token with insufficient permission"}, true
		case http.StatusNotAcceptable, http.StatusUpgradeRequired:
			return unknownVerificationResult("provider_response", "provider rejected the verification contract"), true
		default:
			if statusCode >= 400 && statusCode < 500 && statusCode != http.StatusTooManyRequests {
				return unknownVerificationResult("provider_response", "provider returned an ambiguous response"), true
			}
...
func verifyOnfleet(ctx context.Context, secret string) VerificationResult {
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		...
		if statusCode == http.StatusUnauthorized {
			...
			if json.Unmarshal(body, &response) == nil && response.Message.Error == 1102 {
				return invalidCredentialResult(), true
			}
			return unknownVerificationResult("authorization", "provider authentication response was ambiguous"), true
		}
...
func verifyPodio(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://api.podio.com/user/status", nil)
	req.Header.Set("Authorization", "OAuth2 "+secret)
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			var response struct {
				User *struct {
			...
			if json.Unmarshal(body, &response) == nil && response.User != nil && response.User.UserID > 0 && slices.Contains([]string{"inactive", "active", "blacklisted"}, response.User.Status) {
				return VerificationResult{Status: VerificationVerified}, true
			}
			return unknownVerificationResult("provider_response", "provider returned an unexpected verification response"), true
		}
		if statusCode == http.StatusUnauthorized {
			...
			if json.Unmarshal(body, &response) == nil && response.Error == "unauthorized" && response.ErrorDescription == "expired_token" {
				return invalidCredentialResult(), true
			}
			return unknownVerificationResult("credential_type", "token may use Podio app authentication or different user context"), true
		}
		if statusCode == 420 {
			return unknownVerificationResult("rate_limited", "provider rate limited verification"), true
		}
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			var response struct {
				Carriers []struct {
					Name          string            `json:"name"`
					ServiceLevels []json.RawMessage `json:"service_levels"`
				} `json:"carriers"`
			}
			if json.Unmarshal(body, &response) == nil {
				for _, carrier := range response.Carriers {
					if carrier.Name == "usps" && carrier.ServiceLevels != nil {
						return VerificationResult{Status: VerificationVerified}, true
					}
				}
			}
		...
		_ = json.Unmarshal(body, &response)
		switch {
		case statusCode == http.StatusUnauthorized && response.Error.Code == "UNAUTHORIZED":
			return invalidCredentialResult(), true
		case statusCode == http.StatusForbidden && response.Error.Code == "APIKEY.INACTIVE":
			return invalidCredentialResult(), true
		case statusCode == http.StatusForbidden:
			return unknownVerificationResult("authorization", "provider could not authorize verification"), true
		default:
...
func verifyDatoCMS(ctx context.Context, secret string) VerificationResult {
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			...
			if json.Unmarshal(body, &response) == nil && response.Data != nil && response.Data.ID != "" && response.Data.Type == "site" && jsonObjectOrEmpty(response.Data.Attributes) && jsonObjectOrEmpty(response.Data.Relationships) && jsonObjectOrEmpty(response.Data.Meta) {
				return VerificationResult{Status: VerificationVerified}, true
			}
			return unknownVerificationResult("provider_response", "provider returned an unexpected verification response"), true
		}
		code := datoCMSErrorCode(body)
		switch {
		case (statusCode == http.StatusUnauthorized || statusCode == http.StatusForbidden) && code == "INSUFFICIENT_PERMISSIONS":
			return VerificationResult{Status: VerificationVerified, Message: "provider authenticated a token with insufficient permission"}, true
		case statusCode == http.StatusUnauthorized && code == "INVALID_AUTHORIZATION_HEADER":
			return invalidCredentialResult(), true
		case statusCode == http.StatusUnauthorized || statusCode == http.StatusForbidden:
			return unknownVerificationResult("authorization", "provider authentication response was ambiguous"), true
		default:
...
func verifyBraintree(ctx context.Context, secret string) VerificationResult {
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		...
		if response.Data.Ping == "pong" && len(response.Errors) == 0 && response.Extensions.RequestID != "" {
			return VerificationResult{Status: VerificationVerified}, true
		}
		for _, providerErr := range response.Errors {
			switch providerErr.Extensions.ErrorClass {
			case "AUTHENTICATION":
				return invalidCredentialResult(), true
			case "AUTHORIZATION":
				return VerificationResult{Status: VerificationVerified, Message: "provider authenticated a token with insufficient permission"}, true
			case "RESOURCE_LIMIT":
				return unknownVerificationResult("rate_limited", "provider limited verification"), true
			case "INTERNAL", "SERVICE_AVAILABILITY":
				return unknownVerificationResult("provider", "provider unavailable"), true
			}
		}
...
func verifyHarvest(ctx context.Context, secret string) VerificationResult {
	...
	req.Header.Set("Authorization", "Bearer "+secret)
	req.Header.Set("User-Agent", "secret-sniffer (security@example.invalid)")
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			var response struct {
				User *struct {
			...
			if json.Unmarshal(body, &response) == nil && response.User != nil && response.User.ID > 0 && response.Accounts != nil {
				valid := true
				for _, account := range response.Accounts {
					if _, ok := account["id"].(float64); !ok {
						valid = false
					}
					for _, field := range []string{"name", "product"} {
						if _, ok := account[field].(string); !ok {
							valid = false
						}
					}
				}
		...
		if statusCode == http.StatusUnauthorized {
			...
			if json.Unmarshal(body, &response) == nil && response.Error == "non-existent-token" {
				return invalidCredentialResult(), true
			}
			return unknownVerificationResult("authorization", "provider authentication response was ambiguous"), true
		}
...
func verifyStreak(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://api.streak.com/api/v1/users/me", nil)
	req.SetBasicAuth(secret, "")
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			var response struct {
			...
			if json.Unmarshal(body, &response) == nil && (response.Key != "" || response.UserKey != "") {
				return VerificationResult{Status: VerificationVerified}, true
			}
			return unknownVerificationResult("provider_response", "provider returned an unexpected verification response"), true
		}
		if statusCode == http.StatusUnauthorized {
			...
			if json.Unmarshal(body, &response) == nil && (strings.EqualFold(response.Error, "invalid api key") || strings.EqualFold(response.Error, "Authentication required")) {
				return invalidCredentialResult(), true
			}
			return unknownVerificationResult("authorization", "provider authentication response was ambiguous"), true
		}
...
func verifyQovery(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://api.qovery.com/account", nil)
	req.Header.Set("Authorization", "Token "+secret)
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			var response struct {
		...
		case http.StatusUnauthorized:
			return invalidCredentialResult(), true
		case http.StatusForbidden:
			return unknownVerificationResult("authorization", "provider could not authorize account access"), true
		default:
			return verifyJSONReadClassification(statusCode, body)
		}
	})
...
func verifyAfterShip(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://api.aftership.com/tracking/2026-07/couriers?slug=usps", nil)
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			...
			if json.Unmarshal(body, &response) == nil && response.Meta.Code == http.StatusOK && response.Data.Total != nil {
				for _, courier := range response.Data.Couriers {
					if courier.Slug == "usps" && courier.Name != "" {
						return VerificationResult{Status: VerificationVerified}, true
					}
				}
			}
		...
		if statusCode == http.StatusUnauthorized {
			...
			if json.Unmarshal(body, &response) == nil && response.Meta.Code == http.StatusUnauthorized && response.Meta.Type == "Unauthorized" {
				return invalidCredentialResult(), true
			}
			return unknownVerificationResult("authorization", "provider authentication response was ambiguous"), true
		}
...
func verifyTMetric(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://app.tmetric.com/api/v3/user", nil)
	req.Header.Set("Authorization", "Bearer "+secret)
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			var response struct {
			...
			if json.Unmarshal(body, &response) == nil && response.ID > 0 {
				return VerificationResult{Status: VerificationVerified}, true
			}
			return unknownVerificationResult("provider_response", "provider returned an unexpected verification response"), true
		}
...
func verifyFlat(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://api.flat.io/v2/me", nil)
	req.Header.Set("Authorization", "Bearer "+secret)
	req.Header.Set("Accept", "application/json")
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			...
			if json.Unmarshal(body, &response) == nil && response.ID != "" && slices.Contains([]string{"user", "guest"}, response.Type) && slices.Contains([]string{"flat", "flatEdu", "msnippet", "embed"}, response.Product) && response.Username != "" && response.Picture != nil && jsonNullOrString(response.Picture) {
				return VerificationResult{Status: VerificationVerified}, true
			}
			return unknownVerificationResult("provider_response", "provider returned an unexpected verification response"), true
		}
		...
		_ = json.Unmarshal(body, &response)
		switch {
		case statusCode == http.StatusUnauthorized && response.Code == "invalid_token":
			return invalidCredentialResult(), true
		case statusCode == http.StatusForbidden && response.Code == "API_RATE_LIMIT_EXCEEDED":
			return unknownVerificationResult("rate_limited", "provider rate limited verification"), true
		case statusCode == http.StatusUnauthorized || statusCode == http.StatusForbidden:
			return unknownVerificationResult("authorization", "provider authentication response was ambiguous"), true
		default:
...
func verifySalesloft(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://api.salesloft.com/v2/me", nil)
	req.Header.Set("Authorization", "Bearer "+secret)
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			var response struct {
			...
			if json.Unmarshal(body, &response) == nil && response.Data.ID > 0 {
				return VerificationResult{Status: VerificationVerified}, true
			}
		}
		if statusCode == http.StatusOK {
		...
		if statusCode == http.StatusUnauthorized {
			...
			if json.Unmarshal(body, &response) == nil && response.Error == "Invalid Bearer token" {
				return invalidCredentialResult(), true
			}
			return unknownVerificationResult("authorization", "provider authentication response was ambiguous"), true
		}
...
func verifyWorkable(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://workable.com/spi/v3/accounts", nil)
	req.Header.Set("Authorization", "Bearer "+secret)
	req.Header.Set("Accept", "application/json")
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			...
			if json.Unmarshal(body, &response) == nil && response.Accounts != nil {
				valid := true
				for _, account := range response.Accounts {
					for _, field := range []string{"id", "name", "subdomain", "description", "summary", "website_url"} {
						if _, ok := account[field].(string); !ok {
							valid = false
						}
					}
		...
		if statusCode == http.StatusUnauthorized {
			if containsAnyFold(string(body), "invalid_token", "access token is invalid") {
				return invalidCredentialResult(), true
			}
			return unknownVerificationResult("authorization", "provider authentication response was ambiguous"), true
		}
...
func verifyVeriphone(ctx context.Context, secret string) VerificationResult {
	...
	req.Header.Set("Authorization", "Bearer "+secret)
	result := verifyHTTPRequestWithClassifier(ctx, req, classifyReadOnlyAPI([]string{"email", "counter", "active"}, "invalid api key"))
	result.Response = ""
	return result
}
...
func verifyCodequiry(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://codequiry.com/api/v1/auth/validate", nil)
	req.Header.Set("apikey", secret)
	req.Header.Set("Accept", "application/json")
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			...
			if json.Unmarshal(body, &response) == nil && response.Valid && response.UserID > 0 {
				return VerificationResult{Status: VerificationVerified}, true
			}
			return unknownVerificationResult("provider_response", "provider returned an unexpected verification response"), true
		}
		...
		case http.StatusPaymentRequired:
			return VerificationResult{Status: VerificationVerified, Message: "provider authenticated a billing-restricted account"}, true
		default:
			if statusCode >= 400 && statusCode < 500 && statusCode != http.StatusTooManyRequests {
				return unknownVerificationResult("provider_response", "provider returned an ambiguous response"), true
			}
...
func verifySportsMonk(ctx context.Context, secret string) VerificationResult {
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusForbidden {
			return VerificationResult{Status: VerificationVerified, Message: "provider authenticated a token without feed access"}, true
		}
		return classifyReadOnlyAPI([]string{"data", "rate_limit", "requested_entity"}, "invalid token provided")(statusCode, body)
	})
	result.Response = ""
	return result
}
...
func verifyUpLead(ctx context.Context, secret string) VerificationResult {
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusForbidden {
			return VerificationResult{Status: VerificationVerified, Message: "provider authenticated a paused or unsubscribed account"}, true
		}
		return classifyReadOnlyAPI([]string{"credits"}, "api key is invalid")(statusCode, body)
	})
	result.Response = ""
	return result
}
...
func verifyReplyIO(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://api.reply.io/v3/whoami", nil)
	req.Header.Set("Authorization", "Bearer "+secret)
	req.Header.Set("Accept", "application/json")
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			...
			if json.Unmarshal(body, &response) == nil && response.UserID > 0 && strings.TrimSpace(response.Username) != "" && response.TeamID > 0 {
				return VerificationResult{Status: VerificationVerified}, true
			}
			return unknownVerificationResult("provider_response", "provider returned an unexpected verification response"), true
		}
		if statusCode == http.StatusUnauthorized {
			...
			if json.Unmarshal(body, &response) == nil {
				if response.Code == "USER_NOT_FOUND" {
					return VerificationResult{Status: VerificationVerified, Message: "provider recognized an organization key without usable user context"}, true
				}
				if response.Status == http.StatusUnauthorized && strings.EqualFold(response.Title, "Unauthorized") && containsAnyFold(response.Detail, "credentials are missing or invalid") {
					return invalidCredentialResult(), true
				}
			}
			return unknownVerificationResult("authorization", "provider authentication response was ambiguous"), true
		...
		_ = json.Unmarshal(body, &response)
		if (statusCode == http.StatusForbidden && (response.Code == "USER_REQUIRED" || response.Code == "TEAM_NOT_ACCESSIBLE")) || (statusCode == http.StatusNotFound && response.Code == "whoami.notFound") {
			return VerificationResult{Status: VerificationVerified, Message: "provider recognized a restricted or unavailable key context"}, true
		}
		return verifyJSONReadClassification(statusCode, body)
	})
	result.Response = ""
...
func verifyParseur(ctx context.Context, secret string) VerificationResult {
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		...
		if statusCode == http.StatusForbidden {
			var response struct {
				Error string `json:"non_field_errors"`
			}
			if json.Unmarshal(body, &response) == nil && (strings.EqualFold(response.Error, "Not authenticated") || strings.EqualFold(response.Error, "Authentication failed")) {
				return invalidCredentialResult(), true
			}
			return unknownVerificationResult("authorization", "provider authentication response was ambiguous"), true
		}
		if statusCode == http.StatusOK {
			...
			if json.Unmarshal(body, &response) == nil && parseurDiscoveryURL(response.Document, "/document") && parseurDiscoveryURL(response.Parser, "/parser") {
				return VerificationResult{Status: VerificationVerified}, true
			}
		}
		return unknownVerificationResult("provider_response", "provider returned an ambiguous response"), true
...
func verifyHumanity(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://www.humanity.com/api/v2/me", nil)
	req.Header.Set("Authorization", "Bearer "+secret)
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusTooManyRequests || statusCode >= 500 {
			return VerificationResult{}, false
		...
		if response.Status == 1 {
			...
			if json.Unmarshal(response.Data, &data) == nil && data.ID != "" {
				return VerificationResult{Status: VerificationVerified}, true
			}
		}
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		...
		if statusCode == http.StatusOK {
			...
			if json.Unmarshal(body, &response) == nil && response.Accounts != nil {
				valid := true
				for _, account := range response.Accounts {
					if account.Username == "" || account.Ducklings == nil {
						valid = false
					}
					for _, duckling := range account.Ducklings {
						if duckling.ID == "" || (duckling.Type != "read_write" && duckling.Type != "read_scaling") || (duckling.Status != "active" && duckling.Status != "cooldown") {
							valid = false
						}
					}
				}
		...
		var response struct {
			Code               string `json:"code"`
			FriendlyStatusCode string `json:"mdFriendlyStatusCode"`
		}
		_ = json.Unmarshal(body, &response)
		switch {
		case response.Code == "UNAUTHORIZED" || response.FriendlyStatusCode == "AUTHENTICATION_ERROR":
			return invalidCredentialResult(), true
		case statusCode == http.StatusForbidden && response.Code == "FORBIDDEN":
...
func verifySalesflare(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://api.salesflare.com/me", nil)
	req.Header.Set("Authorization", "Bearer "+secret)
	req.Header.Set("Accept", "application/json")
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
		...
		if statusCode == http.StatusUnauthorized {
			...
			if json.Unmarshal(body, &response) == nil && response.StatusCode == http.StatusUnauthorized && strings.EqualFold(response.Error, "Unauthorized") && strings.EqualFold(response.Message, "Missing authentication") {
				return invalidCredentialResult(), true
			}
			return unknownVerificationResult("authorization", "provider authentication response was ambiguous"), true
		}
...
func verifyMindMeister(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://www.mindmeister.com/api/v2/users/me", nil)
	req.Header.Set("Authorization", "Bearer "+secret)
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			var response struct {
		...
		_ = json.Unmarshal(body, &response)
		switch {
		case statusCode == http.StatusUnauthorized && response.Error.Type == "invalidCredentials":
			return invalidCredentialResult(), true
		case statusCode == http.StatusForbidden && response.Error.Type == "unauthorizedScopes":
			return VerificationResult{Status: VerificationVerified, Message: "provider authenticated a token with insufficient scope"}, true
		case statusCode == http.StatusForbidden || statusCode == http.StatusUnauthorized:
			return unknownVerificationResult("authorization", "provider authentication response was ambiguous"), true
		default:
...
func verifyReachMail(ctx context.Context, secret string) VerificationResult {
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			...
			if json.Unmarshal(body, &response) == nil && validNonzeroUUID(response.AccountID) && response.AccountKey != nil && response.Username != nil && response.CompanyName != nil && response.Email != nil && response.Name != nil {
				return VerificationResult{Status: VerificationVerified}, true
			}
			return unknownVerificationResult("provider_response", "provider returned an unexpected verification response"), true
		}
...
func verifyCourier(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://api.courier.com/preferences/sections", nil)
	req.Header.Set("Authorization", "Bearer "+secret)
	req.Header.Set("Accept", "application/json")
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			...
			if json.Unmarshal(body, &response) == nil && response.Results != nil {
				valid := true
				for _, section := range response.Results {
					if _, ok := section["id"].(string); !ok {
						valid = false
					}
					if _, ok := section["name"].(string); !ok {
						valid = false
					}
					if _, ok := section["topics"].([]any); !ok {
						valid = false
					}
				}
				if valid {
		...
		_ = json.Unmarshal(body, &response)
		switch {
		case statusCode == http.StatusUnauthorized && response.Type == "authentication_error":
			return invalidCredentialResult(), true
		case statusCode == http.StatusForbidden && response.Status == http.StatusUnauthorized && response.Error == "Authentication failed":
			return invalidCredentialResult(), true
		case statusCode == http.StatusForbidden && response.Type == "authorization_error":
			return VerificationResult{Status: VerificationVerified, Message: "provider authenticated a key with insufficient permission"}, true
		default:
			return verifyJSONReadClassification(statusCode, body)
		}
...
func verifyDocparser(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://api.docparser.com/v1/ping", nil)
	req.SetBasicAuth(secret, "")
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK && jsonTopLevelStringEquals(body, "msg", "pong") {
			return VerificationResult{Status: VerificationVerified}, true
		}
		if statusCode == http.StatusOK {
			return unknownVerificationResult("provider_response", "provider returned an unexpected verification response"), true
		...
		if statusCode == http.StatusForbidden {
			...
			if json.Unmarshal(body, &response) == nil && strings.EqualFold(response.Error, "api key not valid") {
				return invalidCredentialResult(), true
			}
			return unknownVerificationResult("authorization", "provider authentication response was ambiguous"), true
		}
...
func verifySelectPDF(ctx context.Context, secret string) VerificationResult {
	...
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		...
		if json.Unmarshal(body, &response) == nil && statusCode >= 200 && statusCode < 300 && response.Status == "License key active." && response.Limit != nil && response.Used != nil && response.Available != nil {
			return VerificationResult{Status: VerificationVerified}, true
		}
		return unknownVerificationResult("provider_response", "provider returned an ambiguous response"), true
	})
...
func verifyMailchimp(ctx context.Context, secret string) VerificationResult {
	...
	if len(dataCenter) < 3 || len(dataCenter) > 4 || !strings.HasPrefix(dataCenter, "us") {
		return VerificationResult{Status: VerificationUnsupported, Message: "Mailchimp key has an invalid data center"}
	}
	dataCenterNumber, err := strconv.Atoi(dataCenter[2:])
	if err != nil || dataCenterNumber < 1 || dataCenterNumber > 99 {
		return VerificationResult{Status: VerificationUnsupported, Message: "Mailchimp key has an invalid data center"}
	}
	...
	return verifyJSONReadRequestWithInvalidUnauthorized(ctx, req, func(body []byte) bool {
		return jsonTopLevelStringEquals(body, "health_status", "Everything's Chimpy!")
	})
}
...
func verifyHubSpot(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://api.hubapi.com/account-info/v3/api-usage/daily/private-apps", nil)
	req.Header.Set("Authorization", "Bearer "+secret)
	req.Header.Set("Accept", "application/json")
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			...
			if json.Unmarshal(body, &response) == nil && response.Results != nil {
				for _, usage := range response.Results {
					if _, err := time.Parse(time.RFC3339Nano, usage.CollectedAt); usage.Name == "private-apps-api-calls-daily" && usage.UsageLimit != nil && usage.CurrentUsage != nil && err == nil && (usage.FetchStatus == "SUCCESS" || usage.FetchStatus == "CACHED") {
						return VerificationResult{Status: VerificationVerified}, true
					}
				}
			}
		...
		case statusCode == http.StatusUnauthorized:
			return invalidCredentialResult(), true
		case statusCode == http.StatusForbidden:
			return unknownVerificationResult("authorization", "provider could not authorize usage access"), true
		case statusCode == http.StatusTooManyRequests:
			return unknownVerificationResult("rate_limited", "provider rate limited verification"), true
		case statusCode == 477 || statusCode >= 500:
			return unknownVerificationResult("provider", "provider unavailable"), true
		default:
			return unknownVerificationResult("provider_response", "provider returned an ambiguous response"), true
		}
...
func verifyXAI(ctx context.Context, secret string) VerificationResult {
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, "https://api.x.ai/v1/api-key", nil)
	req.Header.Set("Authorization", "Bearer "+secret)
	result := verifyHTTPRequestWithClassifier(ctx, req, func(statusCode int, body []byte) (VerificationResult, bool) {
		if statusCode == http.StatusOK {
			var response struct {
			...
			if json.Unmarshal(body, &response) == nil && response.ACLs != nil && response.APIKeyBlocked != nil && response.APIKeyDisabled != nil && response.TeamBlocked != nil && response.APIKeyID != "" && response.CreateTime != "" && response.ModifiedBy != "" && response.ModifyTime != "" && response.Name != "" && response.RedactedAPIKey != "" && response.TeamID != "" && response.UserID != "" {
				if *response.APIKeyBlocked || *response.APIKeyDisabled || *response.TeamBlocked {
					return VerificationResult{Status: VerificationVerified, Message: "provider recognized a blocked or disabled API key"}, true
				}
				return VerificationResult{Status: VerificationVerified}, true
			}
			return unknownVerificationResult("provider_response", "provider returned an unexpected verification response"), true
		...
		if statusCode == http.StatusBadRequest {
			...
			if json.Unmarshal(body, &response) == nil && response.Code == "invalid-argument" && containsAnyFold(response.Error, "incorrect api key") {
				return invalidCredentialResult(), true
			}
			return unknownVerificationResult("provider_response", "provider returned an ambiguous response"), true
		}
...
func parseurDiscoveryURL(value, expectedPath string) bool {
	parsed, err := url.Parse(value)
	return err == nil && (parsed.Scheme == "http" || parsed.Scheme == "https") && parsed.Host == "api.parseur.com" && parsed.Path == expectedPath && parsed.RawQuery == ""
}

func validNonzeroUUID(value string) bool {
...
func jsonNullOrString(raw json.RawMessage) bool {
	if string(raw) == "null" {
		return true
	}
	var value string
...
func mandrillStatsValid(raw json.RawMessage) bool {
	var stats map[string]json.RawMessage
	if json.Unmarshal(raw, &stats) != nil {
		return false
	}
	for _, window := range []string{"today", "last_7_days", "last_30_days", "last_60_days", "last_90_days", "all_time"} {
		if !jsonObjectOrEmpty(stats[window]) {
			return false
		}
	}
	return true
...
func datoCMSErrorCode(body []byte) string {
	var response struct {
		Data []struct {
			Type       string `json:"type"`
			Attributes struct {
	...
	for _, providerErr := range response.Data {
