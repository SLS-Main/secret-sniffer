...
func DefaultRegistry() []Detector {
	return []Detector{
		...
		NewRegex("novita-api-key", "Novita AI API Key", "critical", []string{"novita", "api.novita.ai"}, `(?i)\b(?:novita|api\.novita\.ai)\b[\s\S]{0,160}\b(?:api[_-]?key|authorization|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyNovita),
