...
func DefaultRegistry() []Detector {
	return []Detector{
		...
		NewRegex("xata-api-key", "Xata API Key", "critical", []string{"xata", "api.xata.io"}, `(?i)\b(?:xata|api\.xata\.io)\b[\s\S]{0,160}\b(?:api[_-]?key|api[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, nil),
