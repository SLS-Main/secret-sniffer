...
func DefaultRegistry() []Detector {
	return []Detector{
		...
		NewRegex("salesloft-api-key", "Salesloft API Key", "critical", []string{"salesloft", "api.salesloft.com"}, `(?i)\b(?:salesloft|api\.salesloft\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|access[_-]?token|oauth[_-]?token|token|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9._~/-]{32,256})\b`, 1, verifySalesloft),
		...
		NewRegex("podio-api-token", "Podio API Token", "critical", []string{"podio"}, `(?i)\bpodio\b.{0,80}\b(?:api[_-]?token|access[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyPodio),
		...
		NewRegex("aftership-api-key", "AfterShip API Key", "critical", []string{"aftership", "api.aftership.com"}, `(?i)\b(?:aftership|api\.aftership\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|aftership[_-]?api[_-]?key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyAfterShip),
		...
		NewRegex("ynab-api-token", "YNAB API Token", "critical", []string{"youneedabudget", "ynab"}, `(?i)\b(?:youneedabudget|ynab)\b.{0,80}\b(?:api[_-]?token|personal[_-]?access[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyYNAB),
		...
		NewRegex("tmetric-api-token", "TMetric API Token", "high", []string{"tmetric"}, `(?i)\btmetric\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyTMetric),
		...
		NewRegex("streak-api-key", "Streak API Key", "high", []string{"streak"}, `(?i)\bstreak\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyStreak),
		...
		NewRegex("meistertask-api-token", "MeisterTask API Token", "high", []string{"meistertask", "meister task"}, `(?i)\b(?:meistertask|meister[ _-]?task)\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyMeisterTask),
