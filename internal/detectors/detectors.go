...
func DefaultRegistry() []Detector {
	return []Detector{
		...
		NewRegex("shodan-api-key", "Shodan API Key", "high", []string{"shodan"}, `(?i)\bshodan.{0,40}['\"\s:=]+([A-Za-z0-9]{32})\b`, 1, verifyShodan),
		...
		NewRegex("telnyx-api-key", "Telnyx API Key", "critical", []string{"telnyx"}, `(?i)\btelnyx\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?(KEY[A-Za-z0-9_-]{32,80})\b`, 1, verifyTelnyx),
		...
		NewRegex("lokalise-token", "Lokalise Token", "critical", []string{"lokalise"}, `(?i)\blokalise\b.{0,80}\b(?:api[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{40})\b`, 1, verifyLokalise),
		...
		NewRegex("parsehub-api-key", "ParseHub API Key", "high", []string{"parsehub"}, `(?i)\bparsehub\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyParseHub),
		...
		NewRegex("rocketreach-api-key", "RocketReach API Key", "critical", []string{"rocketreach"}, `(?i)\brocketreach\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyRocketReach),
		...
		NewRegex("apiflash-access-key", "APIFLASH Access Key", "high", []string{"apiflash"}, `(?i)\bapiflash\b.{0,80}\b(?:access[_-]?key|api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyAPIFlash),
		...
		NewRegex("browshot-api-key", "Browshot API Key", "high", []string{"browshot"}, `(?i)\bbrowshot\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9-]{28})\b`, 1, verifyBrowshot),
		...
		NewRegex("statuscake-api-key", "StatusCake API Key", "high", []string{"statuscake"}, `(?i)\bstatuscake\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyStatusCake),
		...
		NewRegex("vbout-api-key", "VBOUT API Key", "high", []string{"vbout"}, `(?i)\bvbout\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([0-9]{25})\b`, 1, verifyVBOUT),
		...
		NewRegex("captaindata-api-key", "Captain Data API Key", "high", []string{"captaindata", "captain data"}, `(?i)\b(?:captaindata|captain[ _-]?data)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyCaptainData),
		...
		NewRegex("dyspatch-api-key", "Dyspatch API Key", "critical", []string{"dyspatch"}, `(?i)\bdyspatch\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{52})\b`, 1, verifyDyspatch),
		...
		NewRegex("hybiscus-api-key", "Hybiscus API Key", "high", []string{"hybiscus"}, `(?i)\bhybiscus\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{43})\b`, 1, verifyHybiscus),
		...
		NewRegex("mailsac-api-key", "Mailsac API Key", "high", []string{"mailsac"}, `(?i)\bmailsac\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyMailsac),
