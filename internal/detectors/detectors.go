...
func DefaultRegistry() []Detector {
	return []Detector{
		...
		NewRegex("turso-api-token", "Turso API Token", "critical", []string{"turso", "turso.tech"}, `(?i)\b(?:turso|turso\.tech|api\.turso\.tech)\b[\s\S]{0,160}\b(?:api[_-]?token|auth[_-]?token|bearer|token|jwt)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,512})\b`, 1, nil),
