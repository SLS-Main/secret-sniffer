...
func DefaultRegistry() []Detector {
	return []Detector{
		...
		NewRegex("workos-api-key", "WorkOS API Key", "critical", []string{"WORKOS_API_KEY", "api.workos.com", "workos"}, `(?i)\b(?:workos[_-]?api[_-]?key|(?:api\.workos\.com|workos)\b.{0,80}\b(?:api[_-]?key|secret[_-]?key))\b\s*[:=]\s*['\"]?(sk_(?:test|live)_[A-Za-z0-9]{20,128})\b`, 1, nil),
