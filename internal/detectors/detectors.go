...
func DefaultRegistry() []Detector {
	return []Detector{
		...
		NewRegex("motherduck-token", "MotherDuck Token", "critical", []string{"motherduck", "motherduck.com"}, `(?i)\b(?:motherduck|motherduck\.com)\b[\s\S]{0,160}\b(?:token|access[_-]?token|api[_-]?key|md[_-]?token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, nil),
