...
func DefaultRegistry() []Detector {
	return []Detector{
		...
		NewRegex("discord-webhook", "Discord Webhook URL", "critical", []string{"discord.com/api/webhooks"}, `\b(https://discord\.com/api/webhooks/[0-9]{18,19}/[0-9A-Za-z-]{68})\b`, 1, verifyDiscordWebhook),
		...
		NewRegex("webex-bot-token", "Webex Bot Token", "critical", []string{"webex", "spark"}, `\b([A-Za-z0-9]{64}_[A-Za-z0-9]{4}_[A-Za-z0-9]{8}-[A-Za-z0-9]{4}-[A-Za-z0-9]{4}-[A-Za-z0-9]{4}-[A-Za-z0-9]{12})\b`, 1, verifyWebex),
		...
		NewRegex("groq-api-key", "Groq API Key", "critical", []string{"gsk_", "groq"}, `\b(gsk_[A-Za-z0-9]{52})\b`, 1, verifyGroq),
		...
		NewRegex("nightfall-api-key", "Nightfall API Key", "critical", []string{"NF-", "nightfall"}, `\b(NF-[A-Za-z0-9]{32})\b`, 1, verifyNightfall),
		...
		NewRegex("azure-app-config-connection-string", "Azure App Configuration Connection String", "critical", []string{".azconfig.io", "Endpoint=", "Secret="}, `(Endpoint=https://[A-Za-z0-9-]+\.azconfig\.io;Id=[A-Za-z0-9+/=]+;Secret=[A-Za-z0-9+/=]+)`, 1, verifyAzureAppConfiguration),
		NewRegex("azure-storage-connection-string", "Azure Storage Connection String", "critical", []string{"AccountName", "AccountKey", "core.windows.net"}, `(DefaultEndpointsProtocol=https?;AccountName=[a-z0-9]{3,24};AccountKey=[A-Za-z0-9+/-]{86,88}={0,2};EndpointSuffix=core\.windows\.net)`, 1, verifyAzureStorageConnectionString),
		...
		NewRegex("spectralops-token", "SpectralOps Token", "critical", []string{"spu-"}, `\b(spu-[a-z0-9]{32})\b`, 1, verifySpectralOps),
		...
		NewRegex("ringover-api-key", "Ringover API Key", "critical", []string{"ringover", "api.ringover.com"}, `(?i)\b(?:ringover|api\.ringover\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|api[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyRingover),
		...
		NewRegex("shortcut-api-token", "Shortcut API Token", "high", []string{"shortcut"}, `(?i)\bshortcut.{0,40}['\"\s:=]+([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})\b`, 1, verifyShortcut),
		...
		NewRegex("unit-api-token", "Unit API Token", "critical", []string{"api.unit.co", "api.s.unit.sh", "UNIT_API_TOKEN"}, `(?i)\b(?:api\.unit\.co|api\.s\.unit\.sh|unit[_-]?api[_-]?token|unit)\b[\s\S]{0,160}\b(?:api[_-]?token|org[_-]?api[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,512})\b`, 1, verifyUnit),
		...
		NewRegex("lithic-api-key", "Lithic API Key", "critical", []string{"lithic", "api.lithic.com"}, `(?i)\b(?:lithic|api\.lithic\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyLithic),
		...
		NewRegex("opticodds-api-key", "OpticOdds API Key", "critical", []string{"opticodds", "api.opticodds.com"}, `(?i)\b(?:opticodds|optic[ _-]?odds|api\.opticodds\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,128})\b`, 1, verifyOpticOdds),
		...
		NewRegex("trulioo-api-key", "Trulioo API Key", "critical", []string{"trulioo", "api.globaldatacompany.com"}, `(?i)\b(?:trulioo|api\.trulioo\.com|api\.globaldatacompany\.com|x-trulioo-api-key)\b[\s\S]{0,200}\b(?:api[_-]?key|bearer|token)\b\s*[:=]\s*['\"]?(eyJ[A-Za-z0-9_-]{10,}\.eyJ[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,})\b`, 1, verifyTrulioo),
		...
		NewRegex("teamwork-token", "Teamwork Token", "critical", []string{"teamwork", "teamworkcrm", "teamworkdesk", "tkn.v1_"}, `\b(tkn\.v1_[0-9A-Za-z]{71}=)`, 1, verifyTeamwork),
		...
		NewRegex("supabase-management-token", "Supabase Management Token", "critical", []string{"sbp_"}, `\b(sbp_[a-z0-9]{40})\b`, 1, verifySupabaseManagement),
		NewRegex("prefect-api-key", "Prefect API Key", "critical", []string{"pnu_"}, `\b(pnu_[A-Za-z0-9]{36})\b`, 1, verifyPrefect),
		...
		NewRegex("neon-api-key", "Neon API Key", "critical", []string{"neon.tech", "api.neon.tech"}, `(?i)\b(?:neon\.tech|api\.neon\.tech|neon[_-]?api)\b[\s\S]{0,160}\b(?:api[_-]?key|api[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyNeon),
		NewRegex("turso-api-token", "Turso API Token", "critical", []string{"turso", "turso.tech"}, `(?i)\b(?:turso|turso\.tech|api\.turso\.tech)\b[\s\S]{0,160}\b(?:api[_-]?token|auth[_-]?token|bearer|token|jwt)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,512})\b`, 1, verifyTurso),
		NewRegex("xata-api-key", "Xata API Key", "critical", []string{"xata", "api.xata.io"}, `(?i)\b(?:xata|api\.xata\.io)\b[\s\S]{0,160}\b(?:api[_-]?key|api[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyXata),
		...
		NewRegex("gcp-service-account-json", "GCP Service Account JSON", "critical", []string{"auth_provider_x509_cert_url", "private_key", "gserviceaccount.com"}, `(?s)(\{[^{}]*"type"\s*:\s*"service_account"[^{}]*"private_key"\s*:\s*"-----BEGIN PRIVATE KEY-----[^\"]+"[^{}]*"client_email"\s*:\s*"[^"]+@[^"]+\.iam\.gserviceaccount\.com"[^{}]*"auth_provider_x509_cert_url"[^{}]*\})`, 1, verifyGCPServiceAccount),
		NewRegex("gcp-application-default-credentials", "GCP Application Default Credentials", "critical", []string{".apps.googleusercontent.com", "refresh_token", "client_secret"}, `(?s)(\{[^{}]*"client_id"\s*:\s*"[^"]+\.apps\.googleusercontent\.com"[^{}]*"client_secret"\s*:\s*"[^"]{20,}"[^{}]*"refresh_token"\s*:\s*"[^"]{20,}"[^{}]*\})`, 1, verifyGCPAuthorizedUser),
		...
		NewRegex("helpcrunch-api-key", "HelpCrunch API Key", "high", []string{"helpcrunch"}, `(?i)\bhelpcrunch[A-Za-z0-9_-]*.{0,80}\b([A-Za-z0-9+/=-]{328})\b`, 1, verifyHelpCrunch),
		...
		NewRegex("securitytrails-api-key", "SecurityTrails API Key", "high", []string{"securitytrails"}, `(?i)\bsecuritytrails.{0,40}['\"\s:=]+([A-Za-z0-9]{32})\b`, 1, verifySecurityTrails),
		...
		NewRegex("ipinfo-token", "IPinfo Token", "high", []string{"ipinfo"}, `(?i)\bipinfo\b.{0,40}\b(?:api[_-]?key|access[_-]?token|token|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyIPInfo),
		NewRegex("coinlayer-api-key", "CoinLayer API Key", "high", []string{"coinlayer"}, `(?i)\bcoinlayer\b.{0,40}\b(?:api[_-]?key|access[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyCoinlayer),
		...
		NewRegex("worldcoinindex-api-key", "WorldCoinIndex API Key", "high", []string{"worldcoinindex"}, `(?i)\bworldcoinindex\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{35})\b`, 1, verifyWorldCoinIndex),
		...
		NewRegex("fixerio-api-key", "Fixer.io API Key", "high", []string{"fixer.io", "fixerio"}, `(?i)\b(?:fixer\.io|fixerio)\b.{0,40}\b(?:api[_-]?key|access[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyFixer),
		NewRegex("currencylayer-api-key", "Currencylayer API Key", "high", []string{"currencylayer"}, `(?i)\bcurrencylayer\b.{0,40}\b(?:api[_-]?key|access[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyCurrencyLayer),
		NewRegex("exchangerate-api-key", "ExchangeRate-API Key", "high", []string{"exchangerate-api", "exchangerateapi"}, `(?i)\b(?:exchangerate-api|exchangerateapi)\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{24})\b`, 1, verifyExchangeRateAPI),
		NewRegex("exchangeratesapi-api-key", "ExchangeRatesAPI Key", "high", []string{"exchangeratesapi"}, `(?i)\bexchangeratesapi\b.{0,40}\b(?:api[_-]?key|access[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyExchangeRatesAPI),
		NewRegex("currencyfreaks-api-key", "CurrencyFreaks API Key", "high", []string{"currencyfreaks"}, `(?i)\bcurrencyfreaks\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyCurrencyFreaks),
		...
		NewRegex("marketstack-api-key", "Marketstack API Key", "high", []string{"marketstack"}, `(?i)\bmarketstack\b.{0,40}\b(?:api[_-]?key|access[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyMarketstack),
		NewRegex("financialmodelingprep-api-key", "Financial Modeling Prep API Key", "high", []string{"financialmodelingprep"}, `(?i)\bfinancialmodelingprep\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyFinancialModelingPrep),
		NewRegex("finnhub-api-key", "Finnhub API Key", "high", []string{"finnhub"}, `(?i)\bfinnhub\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{20})\b`, 1, verifyFinnhub),
		...
		NewRegex("vatlayer-api-key", "VATLayer API Key", "high", []string{"vatlayer"}, `(?i)\bvatlayer\b.{0,40}\b(?:api[_-]?key|access[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyVATLayer),
		...
		NewRegex("positionstack-api-key", "Positionstack API Key", "high", []string{"positionstack"}, `(?i)\bpositionstack\b.{0,40}\b(?:api[_-]?key|access[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyPositionstack),
		NewRegex("geocodio-api-key", "Geocodio API Key", "high", []string{"geocodio"}, `(?i)\bgeocodio\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{39})\b`, 1, verifyGeocodio),
		NewRegex("fastly-personal-token", "Fastly Personal Token", "critical", []string{"fastly"}, `(?i)\bfastly\b.{0,40}\b(?:api[_-]?token|personal[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32})\b`, 1, verifyFastly),
		...
		NewRegex("bitly-access-token", "Bitly Access Token", "high", []string{"bitly"}, `(?i)\bbitly\b.{0,40}\b(?:access[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{40})\b`, 1, verifyBitly),
		...
		NewRegex("pinata-jwt", "Pinata JWT", "critical", []string{"pinata", "eyJ"}, `(?i)\bpinata\b[\s\S]{0,120}\b(?:jwt|api[_-]?jwt|token)\b\s*[:=]\s*['\"]?(eyJ[A-Za-z0-9_-]{20,}\.eyJ[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{20,})\b`, 1, verifyPinata),
		...
		NewRegex("stormglass-api-key", "StormGlass API Key", "high", []string{"stormglass"}, `(?i)\bstormglass\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9-]{73})\b`, 1, verifyStormGlass),
		NewRegex("todoist-api-token", "Todoist API Token", "high", []string{"todoist"}, `(?i)\btodoist\b.{0,40}\b(?:api[_-]?token|access[_-]?token|token)\b\s*[:=]\s*['\"]?([a-f0-9]{40})\b`, 1, verifyTodoist),
		...
		NewRegex("incidentio-api-key", "incident.io API Key", "critical", []string{"incident.io", "api.incident.io"}, `(?i)\b(?:incident\.io|api\.incident\.io|incidentio)\b[\s\S]{0,160}\b(?:api[_-]?key|bearer|token|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyIncidentIO),
		NewRegex("firehydrant-api-key", "FireHydrant API Key", "critical", []string{"firehydrant", "api.firehydrant.io"}, `(?i)\b(?:firehydrant|api\.firehydrant\.io)\b[\s\S]{0,160}\b(?:api[_-]?key|bearer|token|service[_-]?token|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyFireHydrant),
		...
		NewRegex("ilert-api-key", "ilert API Key", "critical", []string{"ilert", "api.ilert.com"}, `(?i)\b(?:ilert|i-lert|api\.ilert\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|api[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyIlert),
		...
		NewRegex("semgrep-app-token", "Semgrep App Token", "critical", []string{"semgrep", "semgrep.dev"}, `(?i)\b(?:semgrep|semgrep\.dev|semgrep_app_token)\b[\s\S]{0,160}\b(?:app[_-]?token|api[_-]?token|api[_-]?key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifySemgrep),
		...
		NewRegex("gocardless-access-token", "GoCardless Access Token", "critical", []string{"gocardless", "live_", "sandbox_"}, `(?i)\bgocardless\b.{0,80}\b(?:access[_-]?token|token)\b\s*[:=]\s*['\"]?((?:live|sandbox)_[A-Za-z0-9_-]{32,128})\b`, 1, verifyGoCardless),
		NewRegex("gumroad-access-token", "Gumroad Access Token", "high", []string{"gumroad"}, `(?i)\bgumroad\b.{0,80}\b(?:access[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyGumroad),
		NewRegex("hellosign-api-key", "HelloSign API Key", "high", []string{"hellosign", "dropboxsign"}, `(?i)\b(?:hellosign|dropbox[ _-]?sign)\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyHelloSign),
		NewRegex("mailboxlayer-api-key", "Mailboxlayer API Key", "high", []string{"mailboxlayer"}, `(?i)\bmailboxlayer\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyMailboxlayer),
		NewRegex("mediastack-api-key", "Mediastack API Key", "high", []string{"mediastack"}, `(?i)\bmediastack\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyMediastack),
		NewRegex("opencage-api-key", "OpenCage API Key", "high", []string{"opencage"}, `(?i)\bopencage\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyOpenCage),
		NewRegex("packagecloud-token", "Packagecloud Token", "critical", []string{"packagecloud"}, `(?i)\bpackagecloud\b.{0,80}\b(?:api[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{48})\b`, 1, verifyPackagecloud),
		...
		NewRegex("openuv-api-key", "OpenUV API Key", "high", []string{"openuv"}, `(?i)\bopenuv\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyOpenUV),
		NewRegex("pandascore-api-key", "PandaScore API Key", "high", []string{"pandascore"}, `(?i)\bpandascore\b.{0,80}\b(?:api[_-]?key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,64})\b`, 1, verifyPandaScore),
		NewRegex("paperform-api-key", "Paperform API Key", "high", []string{"paperform"}, `(?i)\bpaperform\b.{0,80}\b(?:api[_-]?key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyPaperform),
		...
		NewRegex("peopledatalabs-api-key", "People Data Labs API Key", "high", []string{"peopledatalabs", "people data labs"}, `(?i)\b(?:peopledatalabs|people[ _-]?data[ _-]?labs)\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{64})\b`, 1, verifyPeopleDataLabs),
		...
		NewRegex("serpstack-api-key", "Serpstack API Key", "high", []string{"serpstack"}, `(?i)\bserpstack\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([a-z0-9]{32})\b`, 1, verifySerpstack),
		...
		NewRegex("alchemy-api-key", "Alchemy API Key", "critical", []string{"alchemy"}, `(?i)\balchemy\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32})\b`, 1, verifyAlchemy),
		NewRegex("apify-token", "Apify Token", "critical", []string{"apify"}, `(?i)\bapify\b.{0,80}\b(?:api[_-]?token|token)\b\s*[:=]\s*['\"]?(apify_api_[A-Za-z0-9_-]{32,80})\b`, 1, verifyApify),
		...
		NewRegex("beamer-api-key", "Beamer API Key", "high", []string{"beamer"}, `(?i)\bbeamer\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_+/]{45}=)(?:['\"\s,;]|$)`, 1, verifyBeamer),
		...
		NewRegex("buttercms-api-token", "ButterCMS API Token", "high", []string{"buttercms"}, `(?i)\bbuttercms\b.{0,80}\b(?:api[_-]?token|auth[_-]?token|token)\b\s*[:=]\s*['\"]?([a-z0-9]{40})\b`, 1, verifyButterCMS),
		...
		NewRegex("chartmogul-api-key", "ChartMogul API Key", "critical", []string{"chartmogul"}, `(?i)\bchartmogul\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyChartMogul),
		NewRegex("clearbit-api-key", "Clearbit API Key", "critical", []string{"clearbit"}, `(?i)\bclearbit\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-z0-9_]{35})\b`, 1, verifyClearbit),
		...
		NewRegex("fullstory-api-key", "FullStory API Key", "critical", []string{"fullstory"}, `(?i)\bfullstory\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyFullStory),
		...
		NewRegex("kustomer-api-token", "Kustomer API Token", "critical", []string{"kustomer", "api.kustomerapp.com"}, `(?i)\b(?:kustomer|api\.kustomerapp\.com|kustomerapp\.com)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyKustomer),
		...
		NewRegex("planhat-api-token", "Planhat API Token", "critical", []string{"planhat", "api.planhat.com"}, `(?i)\b(?:planhat|api\.planhat\.com)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyPlanhat),
		NewRegex("geoapify-api-key", "Geoapify API Key", "high", []string{"geoapify"}, `(?i)\bgeoapify\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyGeoapify),
		NewRegex("graphhopper-api-key", "GraphHopper API Key", "high", []string{"graphhopper"}, `(?i)\bgraphhopper\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,64})\b`, 1, verifyGraphHopper),
		...
		NewRegex("kickbox-api-key", "Kickbox API Key", "high", []string{"kickbox"}, `(?i)\bkickbox\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyKickbox),
		...
		NewRegex("numverify-api-key", "Numverify API Key", "high", []string{"numverify"}, `(?i)\bnumverify\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyNumverify),
		NewRegex("omnisend-api-key", "Omnisend API Key", "critical", []string{"omnisend"}, `(?i)\bomnisend\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyOmnisend),
		...
		NewRegex("photoroom-api-key", "PhotoRoom API Key", "high", []string{"photoroom"}, `(?i)\bphotoroom\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyPhotoRoom),
		...
		NewRegex("rebrandly-api-key", "Rebrandly API Key", "high", []string{"rebrandly"}, `(?i)\brebrandly\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32})\b`, 1, verifyRebrandly),
		...
		NewRegex("attio-api-key", "Attio API Key", "critical", []string{"attio", "api.attio.com"}, `(?i)\b(?:attio|api\.attio\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|access[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyAttio),
		NewRegex("affinity-api-key", "Affinity API Key", "critical", []string{"affinity.co", "api.affinity.co"}, `(?i)\b(?:affinity\.co|api\.affinity\.co|affinity[_-]?api)\b[\s\S]{0,160}\b(?:api[_-]?key|bearer|token|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyAffinity),
		...
		NewRegex("instantly-api-key", "Instantly API Key", "critical", []string{"instantly.ai", "api.instantly.ai"}, `(?i)\b(?:instantly\.ai|api\.instantly\.ai|instantly[_-]?api)\b[\s\S]{0,160}\b(?:api[_-]?key|access[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyInstantly),
		...
		NewRegex("ashby-api-key", "Ashby API Key", "critical", []string{"ashbyhq.com", "api.ashbyhq.com"}, `(?i)\b(?:ashbyhq\.com|api\.ashbyhq\.com|ashby[_-]?api)\b[\s\S]{0,160}\b(?:api[_-]?key|access[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyAshby),
		...
		NewRegex("deel-api-token", "Deel API Token", "critical", []string{"deel.com", "api.deel.com"}, `(?i)\b(?:deel\.com|api\.deel\.com|deel[_-]?api)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|access[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyDeel),
		...
		NewRegex("airvisual-api-key", "AirVisual API Key", "high", []string{"airvisual", "iqair"}, `(?i)\b(?:airvisual|iqair)\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-z0-9-]{36})\b`, 1, verifyAirVisual),
		...
		NewRegex("ambee-api-key", "Ambee API Key", "high", []string{"ambee"}, `(?i)\bambee\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{64})\b`, 1, verifyAmbee),
		...
		NewRegex("apitemplate-api-key", "APITemplate API Key", "high", []string{"apitemplate"}, `(?i)\bapitemplate\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{39})\b`, 1, verifyAPITemplate),
		...
		NewRegex("appfollow-api-key", "AppFollow API Key", "high", []string{"appfollow"}, `(?i)\bappfollow\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?(eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9\.[A-Za-z0-9_-]{74}\.[A-Za-z0-9_-]{43})\b`, 1, verifyAppFollow),
		...
		NewRegex("aviationstack-api-key", "Aviationstack API Key", "high", []string{"aviationstack"}, `(?i)\baviationstack\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyAviationstack),
		...
		NewRegex("besttime-api-key", "BestTime API Key", "high", []string{"besttime"}, `(?i)\bbesttime\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?(pri_[a-f0-9]{32})\b`, 1, verifyBestTime),
		NewRegex("brandfetch-api-key", "Brandfetch API Key", "high", []string{"brandfetch"}, `(?i)\bbrandfetch\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_+/]{43}=)(?:['\"\s,;]|$)`, 1, verifyBrandfetch),
		...
		NewRegex("calendarific-api-key", "Calendarific API Key", "high", []string{"calendarific"}, `(?i)\bcalendarific\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyCalendarific),
		...
		NewRegex("currentsapi-api-key", "CurrentsAPI Key", "high", []string{"currentsapi"}, `(?i)\bcurrentsapi\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{48})\b`, 1, verifyCurrentsAPI),
		NewRegex("debounce-api-key", "DeBounce API Key", "high", []string{"debounce"}, `(?i)\bdebounce\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{13})\b`, 1, verifyDeBounce),
		NewRegex("detectlanguage-api-key", "Detect Language API Key", "high", []string{"detectlanguage", "detect language"}, `(?i)\b(?:detectlanguage|detect[ _-]?language)\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-z0-9]{32})\b`, 1, verifyDetectLanguage),
		...
		NewRegex("diffbot-api-token", "Diffbot API Token", "high", []string{"diffbot"}, `(?i)\bdiffbot\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([a-z0-9]{32})\b`, 1, verifyDiffbot),
		...
		NewRegex("ethplorer-api-key", "Ethplorer API Key", "high", []string{"ethplorer"}, `(?i)\bethplorer\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyEthplorer),
		...
		NewRegex("holidayapi-key", "Holiday API Key", "high", []string{"holidayapi", "holiday api"}, `(?i)\b(?:holidayapi|holiday[ _-]?api)\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-z0-9-]{36})\b`, 1, verifyHolidayAPI),
		...
		NewRegex("ip2location-api-key", "IP2Location API Key", "high", []string{"ip2location"}, `(?i)\bip2location\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyIP2Location),
		NewRegex("ipapi-api-key", "ipapi API Key", "high", []string{"ipapi"}, `(?i)\bipapi\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyIPAPI),
		NewRegex("ipinfodb-api-key", "IPInfoDB API Key", "high", []string{"ipinfodb"}, `(?i)\bipinfodb\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{64})\b`, 1, verifyIPInfoDB),
		...
		NewRegex("languagelayer-api-key", "Languagelayer API Key", "high", []string{"languagelayer"}, `(?i)\blanguagelayer\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyLanguageLayer),
		...
		NewRegex("linkpreview-api-key", "LinkPreview API Key", "high", []string{"linkpreview"}, `(?i)\blinkpreview\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyLinkPreview),
		...
		NewRegex("mockaroo-api-key", "Mockaroo API Key", "high", []string{"mockaroo"}, `(?i)\bmockaroo\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-z0-9]{8})\b`, 1, verifyMockaroo),
		...
		NewRegex("pixabay-api-key", "Pixabay API Key", "high", []string{"pixabay"}, `(?i)\bpixabay\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-z0-9-]{34})\b`, 1, verifyPixabay),
		...
		NewRegex("rawg-api-key", "RAWG API Key", "high", []string{"rawg"}, `(?i)\brawg\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyRAWG),
		...
		NewRegex("spoonacular-api-key", "Spoonacular API Key", "high", []string{"spoonacular"}, `(?i)\bspoonacular\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-z0-9]{32})\b`, 1, verifySpoonacular),
		NewRegex("sportsmonk-api-token", "SportsMonk API Token", "high", []string{"sportsmonk"}, `(?i)\bsportsmonk\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{60})\b`, 1, verifySportsMonk),
		...
		NewRegex("stockdata-api-key", "StockData API Key", "high", []string{"stockdata"}, `(?i)\bstockdata\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{40})\b`, 1, verifyStockData),
		...
		NewRegex("tatum-api-key", "Tatum API Key", "critical", []string{"tatum"}, `(?i)\btatum\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyTatum),
		...
		NewRegex("tiingo-api-token", "Tiingo API Token", "high", []string{"tiingo"}, `(?i)\btiingo\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyTiingo),
		NewRegex("timecamp-api-token", "TimeCamp API Token", "high", []string{"timecamp"}, `(?i)\btimecamp\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([a-z0-9]{26})\b`, 1, verifyTimeCamp),
		...
		NewRegex("tomtom-api-key", "TomTom API Key", "high", []string{"tomtom"}, `(?i)\btomtom\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyTomTom),
		...
		NewRegex("unsplash-access-key", "Unsplash Access Key", "high", []string{"unsplash"}, `(?i)\bunsplash\b.{0,80}\b(?:access[_-]?key|api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_]{43})\b`, 1, verifyUnsplash),
		NewRegex("userstack-api-key", "Userstack API Key", "high", []string{"userstack"}, `(?i)\buserstack\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([a-z0-9]{32})\b`, 1, verifyUserstack),
		NewRegex("visualcrossing-api-key", "Visual Crossing API Key", "high", []string{"visualcrossing", "visual crossing"}, `(?i:\b(?:visualcrossing|visual[ _-]?crossing)\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?)([A-Z0-9]{25})\b`, 1, verifyVisualCrossing),
		...
		NewRegex("yelp-api-key", "Yelp API Key", "high", []string{"yelp"}, `(?i)\byelp\b.{0,80}\b(?:api[_-]?key|key|bearer[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{128})\b`, 1, verifyYelp),
		...
		NewRegex("ticketmaster-api-key", "Ticketmaster API Key", "high", []string{"ticketmaster"}, `(?i)\bticketmaster\b.{0,80}\b(?:api[_-]?key|consumer[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyTicketmaster),
		NewRegex("theoddsapi-key", "The Odds API Key", "high", []string{"theoddsapi", "the odds api"}, `(?i)\b(?:theoddsapi|the[ _-]?odds[ _-]?api)\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyTheOddsAPI),
		...
		NewRegex("uclassify-api-key", "uClassify API Key", "high", []string{"uclassify"}, `(?i)\buclassify\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{12})\b`, 1, verifyUClassify),
		NewRegex("upcdatabase-api-key", "UPC Database API Key", "high", []string{"upcdatabase", "upc database"}, `(?i)\b(?:upcdatabase|upc[ _-]?database)\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyUPCDatabase),
		NewRegex("uplead-api-key", "UpLead API Key", "critical", []string{"uplead"}, `(?i)\buplead\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9-]{32})\b`, 1, verifyUpLead),
		NewRegex("vbout-api-key", "VBOUT API Key", "high", []string{"vbout"}, `(?i)\bvbout\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([0-9]{25})\b`, 1, verifyVBOUT),
		NewRegex("veriphone-api-key", "Veriphone API Key", "high", []string{"veriphone"}, `(?i)\bveriphone\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyVeriphone),
		NewRegex("walkscore-api-key", "Walk Score API Key", "high", []string{"walkscore", "walk score"}, `(?i)\b(?:walkscore|walk[ _-]?score)\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyWalkScore),
		...
		NewRegex("readme-api-key", "ReadMe API Key", "critical", []string{"rdme_"}, `\b(rdme_[a-z0-9]{70})\b`, 1, verifyReadMe),
		NewRegex("rootly-api-key", "Rootly API Key", "critical", []string{"rootly_"}, `\b(rootly_[a-f0-9]{64})\b`, 1, verifyRootly),
