...
func DefaultRegistry() []Detector {
	return []Detector{
		...
		NewRegex("cockroachcloud-api-key", "Cockroach Cloud API Key", "critical", []string{"cockroach", "cockroachlabs"}, `(?i)\b(?:cockroach|cockroachlabs|api\.cockroachlabs\.cloud)\b[\s\S]{0,160}\b(?:api[_-]?key|service[_-]?account[_-]?secret|client[_-]?secret|secret|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._~/-]{32,256})\b`, 1, nil),
		...
		NewRegex("singlestore-api-key", "SingleStore API Key", "critical", []string{"singlestore", "singlestore.com"}, `(?i)\b(?:singlestore|single[ _-]?store|api\.singlestore\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|management[_-]?token|password|token|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9._~/-]{32,256})\b`, 1, nil),
		...
		NewRegex("qovery-api-token", "Qovery API Token", "critical", []string{"qovery", "api.qovery.com"}, `(?i)\b(?:qovery|api\.qovery\.com)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, nil),
