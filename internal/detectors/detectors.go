...
func DefaultRegistry() []Detector {
	return []Detector{
		...
		NewRegex("contentful-pat", "Contentful Personal Access Token", "critical", []string{"CFPAT-"}, `\b(CFPAT-[A-Za-z0-9_-]{43})\b`, 1, verifyContentful),
		NewRegex("storyblok-personal-access-token", "Storyblok Personal Access Token", "critical", []string{"storyblok"}, `(?i)\bstoryblok.{0,40}['\"\s:=]+([0-9A-Za-z]{22}tt-[0-9]{6}-[A-Za-z0-9_-]{20})\b`, 1, verifyStoryblokPersonal),
		...
		NewRegex("cloudconvert-api-key", "CloudConvert API Key", "critical", []string{"cloudconvert"}, `(?i)\bcloudconvert\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{40,128})\b`, 1, verifyCloudConvert),
		...
		NewRegex("smartsheet-access-token", "Smartsheet Access Token", "critical", []string{"smartsheet"}, `(?i)\bsmartsheets?\b.{0,80}\b(?:access[_-]?token|api[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifySmartsheet),
		...
		NewRegex("mailerlite-api-key", "MailerLite API Key", "high", []string{"mailerlite", "mailer lite"}, `(?i)\b(?:mailerlite|mailer[ _-]?lite).{0,40}['\"\s:=]+([A-Za-z0-9]{32})\b`, 1, verifyMailerLite),
