...
func DefaultRegistry() []Detector {
	return []Detector{
		...
		NewRegex("sentry-user-token", "Sentry User Token", "critical", []string{"sntryu_"}, `\b(sntryu_[a-f0-9]{64})\b`, 1, verifySentry),
		NewRegex("sentry-org-token", "Sentry Organization Token", "critical", []string{"sntrys_eyJ"}, `\b(sntrys_eyJ[A-Za-z0-9=_+/]{197})\b`, 1, verifySentry),
		...
		NewRegex("cloudsmith-api-key", "Cloudsmith API Key", "critical", []string{"cloudsmith"}, `(?i)\bcloudsmith\b.{0,80}\b(?:api[_-]?key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{40,128})\b`, 1, verifyCloudsmith),
		...
		NewRegex("railway-token", "Railway Token", "critical", []string{"railway"}, `(?i)\brailway.{0,40}['\"\s:=]+([A-Za-z0-9_-]{24,64})\b`, 1, verifyRailway),
		...
		NewRegex("travisci-token", "Travis CI Token", "high", []string{"travis", "TRAVIS_TOKEN"}, `(?i)\btravis(?:ci)?.{0,40}['\"\s:=]+([A-Za-z0-9]{22})\b`, 1, verifyTravisCI),
