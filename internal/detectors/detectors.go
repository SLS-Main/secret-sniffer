...
func DefaultRegistry() []Detector {
	return []Detector{
		...
		NewRegex("platformsh-api-token", "Platform.sh API Token", "critical", []string{"platform.sh", "platformsh"}, `(?i)\b(?:platform\.sh|platformsh|api\.platform\.sh)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, nil),
