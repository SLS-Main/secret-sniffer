...
func DefaultRegistry() []Detector {
	return []Detector{
		...
		NewRegex("front-api-token", "Front API Token", "critical", []string{"frontapp.com", "api2.frontapp.com"}, `(?i)\b(?:frontapp\.com|api2\.frontapp\.com|front[_-]?api)\b[\s\S]{0,160}\b(?:api[_-]?token|access[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, nil),
