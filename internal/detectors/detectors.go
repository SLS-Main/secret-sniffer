...
func DefaultRegistry() []Detector {
	return []Detector{
		...
		NewRegex("northflank-api-token", "Northflank API Token", "critical", []string{"northflank", "api.northflank.com"}, `(?i)\b(?:northflank|api\.northflank\.com)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, nil),
