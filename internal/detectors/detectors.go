...
func DefaultRegistry() []Detector {
	return []Detector{
		...
		NewRegex("zilliz-api-key", "Zilliz API Key", "critical", []string{"zilliz", "api.cloud.zilliz.com"}, `(?i)\b(?:zilliz|api\.cloud\.zilliz\.com|cloud\.zilliz\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|authorization|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyZilliz),
		...
		NewRegex("storyblok-access-token", "Storyblok Access Token", "high", []string{"storyblok"}, `(?i)\bstoryblok.{0,40}['\"\s:=]+([0-9A-Za-z]{22}tt)\b`, 1, verifyStoryblokAccess),
		...
		NewRegex("dailyco-api-key", "Daily.co API Key", "critical", []string{"daily.co", "dailyco"}, `(?i)\b(?:daily\.co|dailyco)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyDaily),
		...
		NewRegex("hunter-api-key", "Hunter API Key", "high", []string{"hunter"}, `(?i)\bhunter\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{40})\b`, 1, verifyHunter),
		...
		NewRegex("koyeb-api-token", "Koyeb API Token", "critical", []string{"koyeb", "api.koyeb.com"}, `(?i)\b(?:koyeb|api\.koyeb\.com)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyKoyeb),
