...
func DefaultRegistry() []Detector {
	return []Detector{
		...
		NewRegex("vultr-api-key", "Vultr API Key", "critical", []string{"vultr"}, `(?i)\bvultr\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Fa-f0-9]{36})\b`, 1, verifyVultr),
		...
		NewRegex("eventbrite-private-token", "Eventbrite Private Token", "critical", []string{"eventbrite"}, `(?i)\beventbrite\b.{0,80}\b(?:private[_-]?token|oauth[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,80})\b`, 1, verifyEventbrite),
		...
		NewRegex("baremetrics-api-key", "Baremetrics API Key", "high", []string{"baremetrics"}, `(?i)\bbaremetrics\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyBaremetrics),
		...
		NewRegex("clockify-api-key", "Clockify API Key", "high", []string{"clockify"}, `(?i)\bclockify\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{48})\b`, 1, verifyClockify),
