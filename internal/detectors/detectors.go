...
func DefaultRegistry() []Detector {
	return []Detector{
		...
		NewRegex("resend-api-key", "Resend API Key", "critical", []string{"RESEND_API_KEY", "api.resend.com", "resend"}, `(?i)\b(?:resend[_-]?api[_-]?key|(?:api\.resend\.com|resend)\b.{0,80}\b(?:api[_-]?key|authorization|bearer))\b\s*[:=]\s*['\"]?(re_[A-Za-z0-9_-]{20,128})\b`, 1, nil),
		...
		NewRegex("neon-api-key", "Neon API Key", "critical", []string{"neon.tech", "api.neon.tech"}, `(?i)\b(?:neon\.tech|api\.neon\.tech|neon[_-]?api)\b[\s\S]{0,160}\b(?:api[_-]?key|api[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, nil),
