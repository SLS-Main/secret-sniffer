...
type VerificationSafety string

const (
	VerificationSafetyUnreviewed VerificationSafety = "unreviewed"
	VerificationSafetyReadOnly   VerificationSafety = "read_only"
	VerificationSafetyAuthOnly   VerificationSafety = "auth_only"
	VerificationSafetyUnsafe     VerificationSafety = "unsafe"
)
...
func (c Candidate) Verify(ctx context.Context) VerificationResult {
	return c.VerifyWithPolicy(ctx, VerificationPolicy{})
}

func (c Candidate) VerifyWithPolicy(ctx context.Context, policy VerificationPolicy) VerificationResult {
	if result, blocked := c.VerificationPolicyResult(policy); blocked {
		return result
	}
...
func (c Candidate) VerificationPolicyResult(policy VerificationPolicy) (VerificationResult, bool) {
	if !c.Verifiable() {
		return VerificationResult{Status: VerificationUnsupported}, true
	}
	safety := normalizedVerificationSafety(c.VerificationSafety, true)
	if safety == VerificationSafetyUnreviewed && !policy.AllowUnreviewed {
		return VerificationResult{Status: VerificationNotAttempted, ErrorCategory: "unreviewed_verification_disabled", Message: "unreviewed verification requires explicit opt-in"}, true
	}
	if safety == VerificationSafetyUnsafe && !policy.AllowUnsafe {
		return VerificationResult{Status: VerificationNotAttempted, ErrorCategory: "unsafe_verification_disabled", Message: "unsafe verification requires explicit opt-in"}, true
	}
	return VerificationResult{}, false
}

func normalizedVerificationSafety(safety VerificationSafety, verifiable bool) VerificationSafety {
	if !verifiable {
		return ""
	}
	switch safety {
	case VerificationSafetyReadOnly, VerificationSafetyAuthOnly, VerificationSafetyUnsafe:
		return safety
	default:
		return VerificationSafetyUnreviewed
	}
}
...
func (d RegexDetector) Info() Info {
	verifiable := d.Verifier != nil || d.CompositeVerifier != nil
...
func NewMultipartRegexWithSafety(id, name, severity string, keywords []string, expr string, secretGroup int, partGroups map[string]int, safety VerificationSafety, verifier CompositeVerifier) Detector {
	return RegexDetector{ID: id, Name: name, Severity: severity, Keywords: keywords, Regex: regexp.MustCompile(expr), SecretGroup: secretGroup, PartGroups: partGroups, CompositeVerifier: verifier, VerificationSafety: safety, BroadContext: strings.Contains(expr, `[\s\S]{0,`)}
}
...
func DefaultRegistry() []Detector {
	return []Detector{
		...
		NewReadOnlyRegex("sendgrid-key", "SendGrid API Key", "critical", []string{"SG."}, `\b(SG\.[A-Za-z0-9_-]{22}\.[A-Za-z0-9_-]{43})\b`, 1, verifySendGrid),
		NewRegex("twilio-key", "Twilio API Key", "high", []string{"SK", "twilio"}, `\b(SK[0-9a-fA-F]{32})\b`, 1, nil),
		NewReadOnlyRegex("mailgun-key", "Mailgun API Key", "high", []string{"key-", "mailgun"}, `\b(key-[0-9a-fA-F]{32})\b`, 1, verifyMailgun),
		NewReadOnlyRegex("gitlab-token", "GitLab Token", "critical", []string{"glpat-", "gldt-"}, `\b((?:glpat|gldt)-[A-Za-z0-9_-]{20,128})\b`, 1, verifyGitLab),
		...
		NewReadOnlyRegex("discord-token", "Discord Token", "critical", []string{"discord", "Bot "}, `\b([MN][A-Za-z\d]{23}\.[\w-]{6}\.[\w-]{27,})\b`, 1, verifyDiscordBot),
		NewReadOnlyRegex("discord-bot-token", "Discord Bot Token", "critical", []string{"discord", "Bot "}, `(?i)\bBot\s+([A-Za-z0-9._-]{50,90})\b`, 1, verifyDiscordBot),
		NewReadOnlyRegex("telegram-bot-token", "Telegram Bot Token", "critical", []string{"bot", "telegram"}, `\b(\d{8,10}:[A-Za-z0-9_-]{35})\b`, 1, verifyTelegramBot),
		NewReadOnlyRegex("npm-token", "npm Token", "critical", []string{"npm_", "//registry.npmjs.org"}, `\b(npm_[A-Za-z0-9]{36})\b`, 1, verifyNPM),
		...
		NewReadOnlyRegex("datadog-api-key", "Datadog API Key", "critical", []string{"datadog", "DD_API_KEY"}, `(?i)\b(datadog|dd_api_key).{0,20}['\"\s:=]+([a-f0-9]{32})\b`, 2, verifyDatadog),
		CorrelatedDetector{
			...
			PrimaryPart: "app_key", MaxDistance: 256, StopAtBlankLine: true, CompositeVerifier: verifyDatadogCredentials, VerificationSafety: VerificationSafetyReadOnly,
		},
		NewReadOnlyRegex("new-relic-key", "New Relic Key", "high", []string{"NRAK-", "newrelic"}, `\b(NR(?:AK|II)-[A-Za-z0-9]{20,80})\b`, 1, verifyNewRelic),
		NewRegex("pagerduty-token", "PagerDuty Token", "high", []string{"pagerduty"}, `(?i)\bpagerduty.{0,20}['\"\s:=]+([A-Za-z0-9_+=-]{20,128})\b`, 1, verifyPagerDuty),
		NewReadOnlyRegex("heroku-api-key", "Heroku API Key", "critical", []string{"heroku"}, `(?i)\bheroku.{0,20}['\"\s:=]+([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})\b`, 1, verifyHeroku),
		NewReadOnlyRegex("cloudflare-api-token", "Cloudflare API Token", "critical", []string{"cloudflare"}, `(?i)\bcloudflare.{0,20}['\"\s:=]+([A-Za-z0-9_-]{35,80})\b`, 1, verifyCloudflare),
		NewReadOnlyRegex("digitalocean-token", "DigitalOcean Token", "critical", []string{"dop_v1_", "digitalocean"}, `\b(dop_v1_[A-Fa-f0-9]{64})\b`, 1, verifyDigitalOcean),
		...
		NewReadOnlyRegex("rubygems-api-key", "RubyGems API Key", "high", []string{"rubygems_"}, `\b(rubygems_[a-f0-9]{48})\b`, 1, verifyRubyGems),
		NewReadOnlyRegex("linear-api-key", "Linear API Key", "high", []string{"lin_api_"}, `\b(lin_api_[A-Za-z0-9]{40})\b`, 1, verifyLinear),
		NewReadOnlyRegex("notion-token", "Notion Token", "high", []string{"secret_", "notion"}, `\b(secret_[A-Za-z0-9]{43})\b`, 1, verifyNotion),
		NewReadOnlyRegex("postman-api-key", "Postman API Key", "high", []string{"PMAK-"}, `\b(PMAK-[A-Za-z0-9-]{59})\b`, 1, verifyPostman),
		...
		NewReadOnlyRegex("discord-webhook", "Discord Webhook URL", "critical", []string{"discord.com/api/webhooks"}, `\b(https://discord\.com/api/webhooks/[0-9]{18,19}/[0-9A-Za-z-]{68})\b`, 1, verifyDiscordWebhook),
		...
		NewRegex("grafana-token", "Grafana Token", "critical", []string{"glc_eyJ"}, `\b(glc_eyJ[A-Za-z0-9+/=]{60,160})\b`, 1, verifyGrafanaCloud),
		NewRegex("grafana-service-account-token", "Grafana Service Account Token", "critical", []string{"glsa_"}, `\b(glsa_[0-9A-Za-z_]{41})\b`, 1, nil),
		NewAuthOnlyRegex("sentry-user-token", "Sentry User Token", "critical", []string{"sntryu_"}, `\b(sntryu_[a-f0-9]{64})\b`, 1, verifySentry),
		NewAuthOnlyRegex("sentry-org-token", "Sentry Organization Token", "critical", []string{"sntrys_eyJ"}, `\b(sntrys_eyJ[A-Za-z0-9=_+/]{197})\b`, 1, verifySentry),
		NewRegex("honeycomb-api-key", "Honeycomb API Key", "high", []string{"honeycomb"}, `(?i)\bhoneycomb.{0,40}['\"\s:=]+([0-9a-f]{32}|[0-9A-Za-z]{22})\b`, 1, verifyHoneycomb),
		...
		NewReadOnlyRegex("nightfall-api-key", "Nightfall API Key", "critical", []string{"NF-", "nightfall"}, `\b(NF-[A-Za-z0-9]{32})\b`, 1, verifyNightfall),
		...
		NewReadOnlyRegex("pinecone-api-key", "Pinecone API Key", "critical", []string{"pcsk_"}, `\b(pcsk_[A-Za-z0-9]{5,6}_[A-Za-z0-9]{63})\b`, 1, verifyPinecone),
		...
		NewReadOnlyRegex("voyageai-api-key", "Voyage AI API Key", "critical", []string{"voyageai", "api.voyageai.com"}, `(?i)\b(?:voyageai|voyage[_-]?ai|api\.voyageai\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|authorization|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyVoyageAI),
		...
		NewReadOnlyRegex("ai21-api-key", "AI21 API Key", "critical", []string{"ai21", "api.ai21.com"}, `(?i)\b(?:ai21|api\.ai21\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|authorization|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyAI21),
		...
		NewReadOnlyRegex("runpod-api-key", "Runpod API Key", "critical", []string{"runpod", "api.runpod.ai"}, `(?i)\b(?:runpod|api\.runpod\.ai)\b[\s\S]{0,160}\b(?:api[_-]?key|authorization|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyRunpod),
		...
		NewReadOnlyRegex("novita-api-key", "Novita AI API Key", "critical", []string{"novita", "api.novita.ai"}, `(?i)\b(?:novita|api\.novita\.ai)\b[\s\S]{0,160}\b(?:api[_-]?key|authorization|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyNovita),
		...
		NewReadOnlyRegex("zilliz-api-key", "Zilliz API Key", "critical", []string{"zilliz", "api.cloud.zilliz.com"}, `(?i)\b(?:zilliz|api\.cloud\.zilliz\.com|cloud\.zilliz\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|authorization|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyZilliz),
		NewReadOnlyRegex("chroma-cloud-api-key", "Chroma Cloud API Key", "critical", []string{"trychroma", "chroma"}, `(?i)\b(?:trychroma|api\.trychroma\.com|chroma[_-]?cloud)\b[\s\S]{0,160}\b(?:api[_-]?key|authorization|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyChromaCloud),
		...
		NewReadOnlyRegex("intercom-access-token", "Intercom Access Token", "critical", []string{"intercom", "dG9rO"}, `(?i)\bintercom.{0,40}['\"\s:=]+(dG9rO[A-Za-z0-9+/]{54}=)`, 1, verifyIntercom),
		...
		NewReadOnlyRegex("segment-api-key", "Segment API Key", "high", []string{"segment"}, `(?i)\bsegment.{0,40}['\"\s:=]+([A-Za-z0-9_-]{43}\.[A-Za-z0-9_-]{43})\b`, 1, verifySegment),
		NewReadOnlyRegex("posthog-personal-api-key", "PostHog Personal API Key", "critical", []string{"phx_", "posthog"}, `\b(phx_[A-Za-z0-9_]{43})\b`, 1, verifyPostHog),
		NewReadOnlyRegex("launchdarkly-key", "LaunchDarkly Key", "critical", []string{"api-", "sdk-", "launchdarkly"}, `\b((?:api|sdk)-[a-z0-9]{8}-[a-z0-9]{4}-4[a-z0-9]{3}-[a-z0-9]{4}-[a-z0-9]{12})\b`, 1, verifyLaunchDarkly),
		...
		NewAuthOnlyRegexWithTrailingBoundary("calendly-api-key", "Calendly API Key", "high", []string{"calendly"}, `(?i)\bcalendly.{0,40}['\"\s:=]+(eyJ[A-Za-z0-9_-]{10,200}\.eyJ[A-Za-z0-9_-]{20,600}\.[A-Za-z0-9_-]{20,300})`, 1, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_-", verifyCalendly),
		NewReadOnlyRegex("monday-api-token", "Monday.com API Token", "high", []string{"monday"}, `(?i)\bmonday.{0,40}['\"\s:=]+(eyJ[A-Za-z0-9_-]{15,100}\.eyJ[A-Za-z0-9_-]{100,300}\.[A-Za-z0-9_-]{25,100})\b`, 1, verifyMonday),
		...
		NewReadOnlyRegex("azure-app-config-connection-string", "Azure App Configuration Connection String", "critical", []string{".azconfig.io", "Endpoint=", "Secret="}, `(Endpoint=https://[A-Za-z0-9-]+\.azconfig\.io;Id=[A-Za-z0-9+/=]+;Secret=[A-Za-z0-9+/=]+)`, 1, verifyAzureAppConfiguration),
		NewReadOnlyRegex("azure-storage-connection-string", "Azure Storage Connection String", "critical", []string{"AccountName", "AccountKey", "core.windows.net"}, `(DefaultEndpointsProtocol=https?;AccountName=[a-z0-9]{3,24};AccountKey=[A-Za-z0-9+/-]{86,88}={0,2};EndpointSuffix=core\.windows\.net)`, 1, verifyAzureStorageConnectionString),
		...
		NewReadOnlyRegex("spectralops-token", "SpectralOps Token", "critical", []string{"spu-"}, `\b(spu-[a-z0-9]{32})\b`, 1, verifySpectralOps),
		NewReadOnlyRegex("atlassian-api-token", "Atlassian API Token", "critical", []string{"ATCTT3xFfG", "atlassian"}, `\b(ATCTT3xFfG[A-Za-z0-9+/=_-]+=[A-Za-z0-9]{8})\b`, 1, verifyAtlassian),
		...
		NewMultipartRegexWithSafety("twilio-auth-token", "Twilio Auth Token", "critical", []string{"twilio", "auth_token", "AC"}, `(?i)\b(AC[0-9a-f]{32})\b[\s\S]{0,160}\b(?:auth[_-]?token|token|secret)\b\s*[:=]\s*['\"]?([0-9a-f]{32})\b`, 2, map[string]int{"account_sid": 1, "auth_token": 2}, VerificationSafetyReadOnly, verifyTwilio),
		...
		NewReadOnlyRegex("dialpad-api-key", "Dialpad API Key", "critical", []string{"dialpad", "dialpad.com"}, `(?i)\b(?:dialpad|dialpad\.com|dialpad[_-]?api)\b[\s\S]{0,160}\b(?:api[_-]?key|access[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyDialpad),
		...
		NewReadOnlyRegex("ringover-api-key", "Ringover API Key", "critical", []string{"ringover", "api.ringover.com"}, `(?i)\b(?:ringover|api\.ringover\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|api[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyRingover),
		...
		NewReadOnlyRegex("callrail-api-key", "CallRail API Key", "critical", []string{"callrail", "api.callrail.com"}, `(?i)\b(?:callrail|api\.callrail\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|api[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyCallRail),
		...
		NewReadOnlyRegex("mailjet-basic-auth", "Mailjet Basic Auth Credential", "high", []string{"mailjet"}, `(?i)\bmailjet\b[\s\S]{0,80}\b([A-Za-z0-9]{87}=)`, 1, verifyMailjetBasicAuth),
		...
		NewReadOnlyRegex("urlscan-api-key", "urlscan.io API Key", "high", []string{"urlscan"}, `(?i)\burlscan\b.{0,40}\b([a-z0-9]{8}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{12})\b`, 1, verifyURLScan),
		...
		NewReadOnlyRegex("deepseek-api-key", "DeepSeek API Key", "critical", []string{"deepseek", "DEEPSEEK_API_KEY"}, `(?i)\bdeepseek.{0,40}['\"\s:=]+(sk-[a-z0-9]{32})\b`, 1, verifyDeepSeek),
		NewReadOnlyRegex("weightsandbiases-api-key", "Weights & Biases API Key", "critical", []string{"wandb", "WANDB_API_KEY", "weightsandbiases", "weights & biases"}, `(?i)\b(?:wandb|weights.?and.?biases).{0,40}['\"\s:=]+([0-9a-f]{40})\b`, 1, verifyWeightsAndBiases),
		NewReadOnlyRegex("assemblyai-api-key", "AssemblyAI API Key", "critical", []string{"assemblyai", "ASSEMBLYAI_API_KEY"}, `(?i)\bassemblyai.{0,40}['\"\s:=]+([0-9a-z]{32})\b`, 1, verifyAssemblyAI),
		NewReadOnlyRegex("deepgram-api-key", "Deepgram API Key", "critical", []string{"deepgram", "DEEPGRAM_API_KEY"}, `(?i)\bdeepgram.{0,40}['\"\s:=]+([0-9a-z]{40})\b`, 1, verifyDeepgram),
		...
		NewReadOnlyRegex("contentful-pat", "Contentful Personal Access Token", "critical", []string{"CFPAT-"}, `\b(CFPAT-[A-Za-z0-9_-]{43})\b`, 1, verifyContentful),
		NewReadOnlyRegex("storyblok-personal-access-token", "Storyblok Personal Access Token", "critical", []string{"storyblok"}, `(?i)\bstoryblok.{0,40}['\"\s:=]+([0-9A-Za-z]{22}tt-[0-9]{6}-[A-Za-z0-9_-]{20})\b`, 1, verifyStoryblokPersonal),
		NewReadOnlyRegex("storyblok-access-token", "Storyblok Access Token", "high", []string{"storyblok"}, `(?i)\bstoryblok.{0,40}['\"\s:=]+([0-9A-Za-z]{22}tt)\b`, 1, verifyStoryblokAccess),
		NewRegex("sanity-auth-token", "Sanity Auth Token", "critical", []string{"sanity"}, `(?i)\bsanity.{0,40}['\"\s:=]+(sk[A-Za-z0-9]{79})\b`, 1, verifySanity),
		...
		NewUnsafeRegex("datocms-api-token", "DatoCMS API Token", "critical", []string{"datocms", "dato.cms"}, `(?i)\b(?:datocms|dato[ _-]?cms|site-api\.datocms\.com)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|read[_-]?only[_-]?token|full[_-]?access[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyDatoCMS),
		...
		NewAuthOnlyRegex("webflow-api-key", "Webflow API Key", "high", []string{"webflow"}, `(?i)\bwebflow.{0,40}['\"\s:=]+([A-Za-z0-9]{64})\b`, 1, verifyWebflow),
		...
		NewReadOnlyRegex("locationiq-api-key", "LocationIQ API Key", "high", []string{"locationiq", "pk."}, `\b(pk\.[A-Za-z0-9-]{32})\b`, 1, verifyLocationIQ),
		...
		NewReadOnlyRegex("unit-api-token", "Unit API Token", "critical", []string{"api.unit.co", "api.s.unit.sh", "UNIT_API_TOKEN"}, `(?i)\b(?:api\.unit\.co|api\.s\.unit\.sh|unit[_-]?api[_-]?token|unit)\b[\s\S]{0,160}\b(?:api[_-]?token|org[_-]?api[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,512})\b`, 1, verifyUnit),
		NewRegex("increase-api-key", "Increase API Key", "critical", []string{"increase", "api.increase.com"}, `(?i)\b(?:increase|api\.increase\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|webhook[_-]?secret|oauth[_-]?client[_-]?secret|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyIncrease),
		NewReadOnlyRegex("lithic-api-key", "Lithic API Key", "critical", []string{"lithic", "api.lithic.com"}, `(?i)\b(?:lithic|api\.lithic\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyLithic),
		...
		NewRegex("persona-api-key", "Persona API Key", "critical", []string{"withpersona", "persona"}, `(?i)\b(?:withpersona|api\.withpersona\.com|persona)\b[\s\S]{0,160}\b(?:persona[_-]?api[_-]?key|api[_-]?key|api[_-]?token|webhook[_-]?secret|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyPersona),
		...
		NewRegex("apisports-api-key", "API-Sports API Key", "critical", []string{"api-sports", "x-apisports-key", "football.api-sports.io"}, `(?i)\b(?:api-sports|x-apisports-key|[a-z0-9.-]*\.api-sports\.io)\b[\s\S]{0,160}\b(?:x-apisports-key|api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,128})\b`, 1, verifyAPISports),
		...
		NewReadOnlyRegex("opticodds-api-key", "OpticOdds API Key", "critical", []string{"opticodds", "api.opticodds.com"}, `(?i)\b(?:opticodds|optic[ _-]?odds|api\.opticodds\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,128})\b`, 1, verifyOpticOdds),
		...
		NewRegex("bitgo-access-token", "BitGo Access Token", "critical", []string{"bitgo", "app.bitgo.com"}, `(?i)\b(?:bitgo|app\.bitgo\.com|test\.bitgo\.com)\b[\s\S]{0,160}\b(?:access[_-]?token|api[_-]?token|token|bearer)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyBitGo),
		NewRegex("circle-api-key", "Circle API Key", "critical", []string{"api.circle.com", "api-sandbox.circle.com", "CIRCLE_API_KEY"}, `(?i)\b(?:api\.circle\.com|api-sandbox\.circle\.com|circle[_-]?api[_-]?key|circle)\b[\s\S]{0,160}\b(?:api[_-]?key|bearer|token|webhook[_-]?secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyCircle),
		...
		NewAuthOnlyRegex("trulioo-api-key", "Trulioo API Key", "critical", []string{"trulioo", "api.globaldatacompany.com"}, `(?i)\b(?:trulioo|api\.trulioo\.com|api\.globaldatacompany\.com|x-trulioo-api-key)\b[\s\S]{0,200}\b(?:api[_-]?key|bearer|token)\b\s*[:=]\s*['\"]?(eyJ[A-Za-z0-9_-]{10,}\.eyJ[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,})\b`, 1, verifyTrulioo),
		...
		NewRegex("etherscan-api-key", "Etherscan API Key", "high", []string{"etherscan", "apikey"}, `(?i)\betherscan.{0,40}\b([0-9A-Z]{34})\b`, 1, verifyEtherscan),
		NewRegex("bscscan-api-key", "BscScan API Key", "high", []string{"bscscan", "apikey"}, `(?i)\bbscscan.{0,40}\b([0-9A-Z]{34})\b`, 1, nil),
		NewRegex("guardian-api-key", "Guardian API Key", "high", []string{"guardianapi", "guardian", "content.guardianapis.com"}, `(?i)\b(?:guardianapi|guardian|content\.guardianapis\.com).{0,40}\b([0-9A-Za-z]{8}-[0-9a-z]{4}-[0-9a-z]{4}-[0-9a-z]{4}-[0-9a-z]{12})\b`, 1, verifyGuardian),
		...
		NewRegex("mailmodo-api-key", "Mailmodo API Key", "high", []string{"mailmodo"}, `(?i)\bmailmodo.{0,40}['\"\s:=]+([A-Z0-9]{7}-[A-Z0-9]{7}-[A-Z0-9]{7}-[A-Z0-9]{7})\b`, 1, verifyMailmodo),
		...
		NewRegex("triggerdev-api-key", "Trigger.dev API Key", "critical", []string{"trigger.dev", "tr_dev_", "tr_prod_"}, `(?i)\b(?:trigger\.dev|api\.trigger\.dev|triggerdev|tr_dev_|tr_prod_)\b[\s\S]{0,160}\b(?:api[_-]?key|access[_-]?token|secret[_-]?key|token|key)\b\s*[:=]\s*['\"]?(tr_(?:dev|prod|stg|preview)_(?:sk_)?[A-Za-z0-9_-]{24,256})\b`, 1, verifyTriggerDev),
		NewReadOnlyRegexWithTrailingBoundary("resend-api-key", "Resend API Key", "critical", []string{"RESEND_API_KEY", "api.resend.com", "resend"}, `(?i)\b(?:resend[_-]?api[_-]?key|(?:api\.resend\.com|resend)\b.{0,80}\b(?:api[_-]?key|authorization|bearer))\b\s*[:=]\s*['\"]?(re_[A-Za-z0-9_-]{8}_[A-Za-z0-9_-]{24})`, 1, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_-", verifyResend),
		NewRegex("clerk-secret-key", "Clerk Secret Key", "critical", []string{"CLERK_SECRET_KEY", "clerk.com", "clerk"}, `(?i)\b(?:clerk[_-]?secret[_-]?key|(?:clerk\.com|clerk)\b.{0,80}\b(?:secret[_-]?key|api[_-]?key))\b\s*[:=]\s*['\"]?(sk_(?:test|live)_[A-Za-z0-9]{20,128})\b`, 1, verifyClerk),
		NewReadOnlyRegex("workos-api-key", "WorkOS API Key", "critical", []string{"WORKOS_API_KEY", "api.workos.com", "workos"}, `(?i)\b(?:workos[_-]?api[_-]?key|(?:api\.workos\.com|workos)\b.{0,80}\b(?:api[_-]?key|secret[_-]?key))\b\s*[:=]\s*['\"]?(sk_(?:test|live)_[A-Za-z0-9]{20,128})\b`, 1, verifyWorkOS),
		NewReadOnlyRegexWithTrailingBoundary("liveblocks-secret-key", "Liveblocks Secret Key", "critical", []string{"LIVEBLOCKS_SECRET_KEY", "liveblocks"}, `(?i)\b(?:liveblocks[_-]?secret[_-]?key|liveblocks\b.{0,80}\b(?:secret[_-]?key|api[_-]?key))\b\s*[:=]\s*['\"]?(sk_(?:test|prod)_[A-Za-z0-9_-]{20,128})`, 1, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_-", verifyLiveblocks),
		NewRegex("polar-access-token", "Polar Access Token", "critical", []string{"polar_oat_", "polar_pat_"}, `\b(polar_(?:oat|pat)_[A-Za-z0-9_-]{20,128})\b`, 1, verifyPolar),
		NewRegex("supabase-secret-key", "Supabase Secret Key", "critical", []string{"sb_secret_"}, `\b(sb_secret_[A-Za-z0-9_-]{20,128})\b`, 1, nil),
		NewRegex("webhook-signing-secret", "Webhook Signing Secret", "critical", []string{"whsec_"}, `\b(whsec_[A-Za-z0-9_-]{24,128})\b`, 1, nil),
		NewRegex("temporal-cloud-api-key", "Temporal Cloud API Key", "critical", []string{"temporal", "temporal.io"}, `(?i)\b(?:temporal|temporal\.io|cloud\.temporal\.io)\b[\s\S]{0,160}\b(?:api[_-]?key|client[_-]?secret|account[_-]?key|bearer|token|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9._~/-]{32,256})\b`, 1, verifyTemporalCloud),
		...
		NewRegex("trayio-api-token", "Tray.io API Token", "critical", []string{"tray.io", "trayio"}, `(?i)\b(?:tray\.io|trayio|api\.tray\.io)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|bearer|token)\b\s*[:=]\s*['\"]?([a-f0-9]{40})\b`, 1, verifyTrayIO),
		NewRegex("airbyte-api-token", "Airbyte API Token", "critical", []string{"airbyte", "api.airbyte.com"}, `(?i)\b(?:airbyte|api\.airbyte\.com|cloud\.airbyte\.com)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|client[_-]?secret|bearer|token|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9._~/-]{32,256})\b`, 1, verifyAirbyte),
		...
		NewRegex("hightouch-api-key", "Hightouch API Key", "critical", []string{"hightouch", "api.hightouch.com"}, `(?i)\b(?:hightouch|api\.hightouch\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|api[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyHightouch),
		NewRegex("deno-deploy-token", "Deno Deploy Token", "critical", []string{"ddp_", "ddw_"}, `\b(dd[pw]_[A-Za-z0-9]{36})\b`, 1, verifyDenoDeploy),
		NewReadOnlyRegex("supabase-management-token", "Supabase Management Token", "critical", []string{"sbp_"}, `\b(sbp_[a-z0-9]{40})\b`, 1, verifySupabaseManagement),
		NewReadOnlyRegex("prefect-api-key", "Prefect API Key", "critical", []string{"pnu_"}, `\b(pnu_[A-Za-z0-9]{36})\b`, 1, verifyPrefect),
		NewRegex("figma-pat", "Figma Personal Access Token", "critical", []string{"figma", "figd_", "figu_", "figo_"}, `\b(fig(?:d|u|o|ur|uh|or|oh)_[A-Za-z0-9_-]{40})\b`, 1, verifyFigma),
		NewRegex("saladcloud-api-key", "SaladCloud API Key", "critical", []string{"salad_cloud_"}, `\b(salad_cloud_[0-9A-Za-z]{1,7}_[0-9A-Za-z]{7,235})\b`, 1, verifySaladCloud),
		...
		NewAuthOnlyRegex("neon-api-key", "Neon API Key", "critical", []string{"neon.tech", "api.neon.tech"}, `(?i)\b(?:neon\.tech|api\.neon\.tech|neon[_-]?api)\b[\s\S]{0,160}\b(?:api[_-]?key|api[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyNeon),
		NewReadOnlyRegex("turso-api-token", "Turso API Token", "critical", []string{"turso", "turso.tech"}, `(?i)\b(?:turso|turso\.tech|api\.turso\.tech)\b[\s\S]{0,160}\b(?:api[_-]?token|auth[_-]?token|bearer|token|jwt)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,512})\b`, 1, verifyTurso),
		NewReadOnlyRegex("xata-api-key", "Xata API Key", "critical", []string{"xata", "api.xata.io"}, `(?i)\b(?:xata|api\.xata\.io)\b[\s\S]{0,160}\b(?:api[_-]?key|api[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyXata),
		NewRegex("cockroachcloud-api-key", "Cockroach Cloud API Key", "critical", []string{"cockroach", "cockroachlabs"}, `(?i)\b(?:cockroach|cockroachlabs|api\.cockroachlabs\.cloud)\b[\s\S]{0,160}\b(?:api[_-]?key|service[_-]?account[_-]?secret|client[_-]?secret|secret|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._~/-]{32,256})\b`, 1, verifyCockroachCloud),
		NewRegex("upstash-token", "Upstash Token", "critical", []string{"upstash", "upstash.io"}, `(?i)\b(?:upstash|upstash\.io|api\.upstash\.com)\b[\s\S]{0,160}\b(?:rest[_-]?token|api[_-]?key|api[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._=-]{32,512})\b`, 1, nil),
		NewReadOnlyRegex("motherduck-token", "MotherDuck Token", "critical", []string{"motherduck", "motherduck.com"}, `(?i)\b(?:motherduck|motherduck\.com)\b[\s\S]{0,160}\b(?:token|access[_-]?token|api[_-]?key|md[_-]?token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,512})\b`, 1, verifyMotherDuck),
		NewRegex("grafbase-api-key", "Grafbase API Key", "critical", []string{"grafbase", "grafbase.com"}, `(?i)\b(?:grafbase|grafbase\.com|api\.grafbase\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|access[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, nil),
		NewReadOnlyRegex("singlestore-api-key", "SingleStore API Key", "critical", []string{"singlestore", "singlestore.com"}, `(?i)\b(?:singlestore|single[ _-]?store|api\.singlestore\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|management[_-]?token|password|token|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9._~/-]{32,256})\b`, 1, verifySingleStore),
		NewRegex("timescale-api-key", "Timescale API Key", "critical", []string{"timescale", "timescale.com"}, `(?i)\b(?:timescale|timescale\.com|api\.timescale\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|api[_-]?token|password|token|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9._~/-]{32,256})\b`, 1, nil),
		NewRegex("convex-deploy-key", "Convex Deploy Key", "critical", []string{"convex", "convex.cloud"}, `(?i)\b(?:convex|convex\.cloud|api\.convex\.dev)\b[\s\S]{0,160}\b(?:deploy[_-]?key|deployment[_-]?key|admin[_-]?key|api[_-]?key|token|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, nil),
		...
		NewAuthOnlyRegex("gcp-service-account-json", "GCP Service Account JSON", "critical", []string{"auth_provider_x509_cert_url", "private_key", "gserviceaccount.com"}, `(?s)(\{[^{}]*"type"\s*:\s*"service_account"[^{}]*"private_key"\s*:\s*"-----BEGIN PRIVATE KEY-----[^\"]+"[^{}]*"client_email"\s*:\s*"[^"]+@[^"]+\.iam\.gserviceaccount\.com"[^{}]*"auth_provider_x509_cert_url"[^{}]*\})`, 1, verifyGCPServiceAccount),
		NewAuthOnlyRegex("gcp-application-default-credentials", "GCP Application Default Credentials", "critical", []string{".apps.googleusercontent.com", "refresh_token", "client_secret"}, `(?s)(\{[^{}]*"client_id"\s*:\s*"[^"]+\.apps\.googleusercontent\.com"[^{}]*"client_secret"\s*:\s*"[^"]{20,}"[^{}]*"refresh_token"\s*:\s*"[^"]{20,}"[^{}]*\})`, 1, verifyGCPAuthorizedUser),
		...
		NewReadOnlyRegex("paystack-secret-key", "Paystack Secret Key", "critical", []string{"sk_live_", "sk_test_", "paystack"}, `\b(sk_(?:live|test)_[A-Za-z0-9]{40})\b`, 1, verifyPaystack),
		...
		NewReadOnlyRegex("helpcrunch-api-key", "HelpCrunch API Key", "high", []string{"helpcrunch"}, `(?i)\bhelpcrunch[A-Za-z0-9_-]*.{0,80}\b([A-Za-z0-9+/=-]{328})\b`, 1, verifyHelpCrunch),
		NewRegex("line-messaging-token", "LINE Messaging Token", "critical", []string{"line_messaging", "LINE_MESSAGING"}, `(?i)\bline[_ -]?messaging[A-Za-z0-9_-]*.{0,100}\b([A-Za-z0-9+/]{171,172})\b`, 1, verifyLINEMessaging),
		NewReadOnlyRegexWithTrailingBoundary("courier-api-key", "Courier API Key", "critical", []string{"courier", "pk_"}, `(?i)\bcourier[A-Za-z0-9_-]*.{0,80}\b(pk_[A-Za-z0-9_-]{20,128})`, 1, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_-", verifyCourier),
		...
		NewRegex("virustotal-api-key", "VirusTotal API Key", "high", []string{"virustotal"}, `(?i)\bvirustotal.{0,40}['\"\s:=]+([a-f0-9]{64})\b`, 1, verifyVirusTotal),
		NewReadOnlyRegex("shodan-api-key", "Shodan API Key", "high", []string{"shodan"}, `(?i)\bshodan.{0,40}['\"\s:=]+([A-Za-z0-9]{32})\b`, 1, verifyShodan),
		NewAuthOnlyRegex("securitytrails-api-key", "SecurityTrails API Key", "high", []string{"securitytrails"}, `(?i)\bsecuritytrails.{0,40}['\"\s:=]+([A-Za-z0-9]{32})\b`, 1, verifySecurityTrails),
		...
		NewRegex("newsapi-key", "NewsAPI Key", "high", []string{"newsapi", "newsapi.org"}, `(?i)\bnewsapi.{0,40}['\"\s:=]+([0-9a-f]{32})\b`, 1, verifyNewsAPI),
		NewRegex("openweather-api-key", "OpenWeather API Key", "high", []string{"openweather", "api.openweathermap.org", "APPID"}, `(?i)\b(?:openweather|api\.openweathermap\.org|appid).{0,80}\b([0-9a-f]{32})\b`, 1, verifyOpenWeather),
		...
		NewAuthOnlyRegex("braintree-access-token", "Braintree Access Token", "critical", []string{"access_token$production$", "access_token$sandbox$"}, `\b(access_token\$(?:production|sandbox)\$[A-Za-z0-9_]+\$[A-Za-z0-9]{32})\b`, 1, verifyBraintree),
		...
		NewReadOnlyRegex("webex-access-token", "Webex Access Token", "critical", []string{"webex", "WEBEX_ACCESS_TOKEN"}, `(?i)\bwebex\b.{0,60}\b(?:access[_-]?token|bearer[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{64}_[A-Za-z0-9]{4}_[A-Za-z0-9_-]{36})\b`, 1, verifyWebex),
		...
		NewRegex("detectify-api-key", "Detectify API Key", "high", []string{"detectify"}, `(?i)\bdetectify\b.{0,40}\b(?:api[_-]?key|token|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyDetectify),
		...
		NewUnsafeRegex("coinlayer-api-key", "CoinLayer API Key", "high", []string{"coinlayer"}, `(?i)\bcoinlayer\b.{0,40}\b(?:api[_-]?key|access[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyCoinlayer),
		...
		NewRegex("cryptocompare-api-key", "CryptoCompare API Key", "high", []string{"cryptocompare"}, `(?i)\bcryptocompare\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{64})\b`, 1, verifyCryptoCompare),
		...
		NewUnsafeRegex("worldcoinindex-api-key", "WorldCoinIndex API Key", "high", []string{"worldcoinindex"}, `(?i)\bworldcoinindex\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{35})\b`, 1, verifyWorldCoinIndex),
		...
		NewUnsafeRegex("fixerio-api-key", "Fixer.io API Key", "high", []string{"fixer.io", "fixerio"}, `(?i)\b(?:fixer\.io|fixerio)\b.{0,40}\b(?:api[_-]?key|access[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyFixer),
		NewUnsafeRegex("currencylayer-api-key", "Currencylayer API Key", "high", []string{"currencylayer"}, `(?i)\bcurrencylayer\b.{0,40}\b(?:api[_-]?key|access[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyCurrencyLayer),
		NewUnsafeRegex("exchangerate-api-key", "ExchangeRate-API Key", "high", []string{"exchangerate-api", "exchangerateapi"}, `(?i)\b(?:exchangerate-api|exchangerateapi)\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{24})\b`, 1, verifyExchangeRateAPI),
		NewUnsafeRegex("exchangeratesapi-api-key", "ExchangeRatesAPI Key", "high", []string{"exchangeratesapi"}, `(?i)\bexchangeratesapi\b.{0,40}\b(?:api[_-]?key|access[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyExchangeRatesAPI),
		NewUnsafeRegex("currencyfreaks-api-key", "CurrencyFreaks API Key", "high", []string{"currencyfreaks"}, `(?i)\bcurrencyfreaks\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyCurrencyFreaks),
		NewRegex("currencyscoop-api-key", "CurrencyScoop API Key", "high", []string{"currencyscoop"}, `(?i)\bcurrencyscoop\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyCurrencyScoop),
		NewRegex("fastforex-api-key", "FastForex API Key", "high", []string{"fastforex"}, `(?i)\bfastforex\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9-]{28,32})\b`, 1, verifyFastForex),
		NewUnsafeRegex("marketstack-api-key", "Marketstack API Key", "high", []string{"marketstack"}, `(?i)\bmarketstack\b.{0,40}\b(?:api[_-]?key|access[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyMarketstack),
		NewUnsafeRegex("financialmodelingprep-api-key", "Financial Modeling Prep API Key", "high", []string{"financialmodelingprep"}, `(?i)\bfinancialmodelingprep\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyFinancialModelingPrep),
		NewUnsafeRegex("finnhub-api-key", "Finnhub API Key", "high", []string{"finnhub"}, `(?i)\bfinnhub\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{20})\b`, 1, verifyFinnhub),
		NewRegex("tradier-token", "Tradier Token", "high", []string{"tradier"}, `(?i)\btradier\b.{0,40}\b(?:access[_-]?token|bearer|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyTradier),
		NewRegex("twelvedata-api-key", "Twelve Data API Key", "high", []string{"twelvedata", "twelve data"}, `(?i)\b(?:twelvedata|twelve[ _-]?data)\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyTwelveData),
		NewUnsafeRegex("vatlayer-api-key", "VATLayer API Key", "high", []string{"vatlayer"}, `(?i)\bvatlayer\b.{0,40}\b(?:api[_-]?key|access[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyVATLayer),
		NewRegex("worldweather-api-key", "World Weather Online API Key", "high", []string{"worldweatheronline", "world weather"}, `(?i)\b(?:worldweatheronline|world[ _-]?weather)\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyWorldWeather),
		NewUnsafeRegex("positionstack-api-key", "Positionstack API Key", "high", []string{"positionstack"}, `(?i)\bpositionstack\b.{0,40}\b(?:api[_-]?key|access[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyPositionstack),
		NewUnsafeRegex("geocodio-api-key", "Geocodio API Key", "high", []string{"geocodio"}, `(?i)\bgeocodio\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{39})\b`, 1, verifyGeocodio),
		NewReadOnlyRegex("fastly-personal-token", "Fastly Personal Token", "critical", []string{"fastly"}, `(?i)\bfastly\b.{0,40}\b(?:api[_-]?token|personal[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32})\b`, 1, verifyFastly),
		NewReadOnlyRegex("telnyx-api-key", "Telnyx API Key", "critical", []string{"telnyx"}, `(?i)\btelnyx\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?(KEY[A-Za-z0-9_-]{32,80})\b`, 1, verifyTelnyx),
		NewRegex("vagrant-cloud-token", "Vagrant Cloud Token", "high", []string{"vagrant"}, `(?i)\bvagrant(?:cloud)?\b.{0,40}\b(?:token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{64})\b`, 1, verifyVagrantCloud),
		NewRegex("zeplin-token", "Zeplin Token", "high", []string{"zeplin"}, `(?i)\bzeplin\b.{0,40}\b(?:token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{40})\b`, 1, verifyZeplin),
		NewReadOnlyRegex("vultr-api-key", "Vultr API Key", "critical", []string{"vultr"}, `(?i)\bvultr\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Fa-f0-9]{36})\b`, 1, verifyVultr),
		NewReadOnlyRegex("bitly-access-token", "Bitly Access Token", "high", []string{"bitly"}, `(?i)\bbitly\b.{0,40}\b(?:access[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{40})\b`, 1, verifyBitly),
		...
		NewRegex("airbrake-user-key", "Airbrake User Key", "high", []string{"airbrake"}, `(?i)\bairbrake\b.{0,80}\b(?:user[_-]?key|api[_-]?key)\b\s*[:=]\s*['\"]?([a-f0-9]{40})\b`, 1, verifyAirbrakeUser),
		NewRegex("bugsnag-api-key", "Bugsnag API Key", "high", []string{"bugsnag"}, `(?i)\bbugsnag\b.{0,40}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, nil),
		NewRegex("infura-project-id", "Infura Project ID", "high", []string{"infura"}, `(?i)\binfura\b.{0,80}\b(?:project[_-]?id|api[_-]?key)\b\s*[:=]\s*['\"]?([a-z0-9]{32})\b`, 1, verifyInfura),
		NewRegex("messagebird-api-key", "MessageBird API Key", "critical", []string{"messagebird", "live_", "test_"}, `(?i)\bmessagebird\b.{0,40}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?((?:live|test)_[A-Za-z0-9]{25})\b`, 1, verifyMessageBird),
		NewAuthOnlyRegex("pinata-jwt", "Pinata JWT", "critical", []string{"pinata", "eyJ"}, `(?i)\bpinata\b[\s\S]{0,120}\b(?:jwt|api[_-]?jwt|token)\b\s*[:=]\s*['\"]?(eyJ[A-Za-z0-9_-]{20,}\.eyJ[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{20,})\b`, 1, verifyPinata),
		NewAuthOnlyRegex("pushbullet-token", "Pushbullet Access Token", "high", []string{"pushbullet", "o."}, `(?i)\bpushbullet\b.{0,40}\b(?:access[_-]?token|token)\b\s*[:=]\s*['\"]?(o\.[A-Za-z0-9]{32})\b`, 1, verifyPushbullet),
		...
		NewUnsafeRegex("stormglass-api-key", "StormGlass API Key", "high", []string{"stormglass"}, `(?i)\bstormglass\b.{0,40}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9-]{73})\b`, 1, verifyStormGlass),
		NewReadOnlyRegex("todoist-api-token", "Todoist API Token", "high", []string{"todoist"}, `(?i)\btodoist\b.{0,40}\b(?:api[_-]?token|access[_-]?token|token)\b\s*[:=]\s*['\"]?([a-f0-9]{40})\b`, 1, verifyTodoist),
		...
		NewRegex("bunny-api-key", "Bunny.net API Key", "critical", []string{"bunny.net", "bunnycdn"}, `(?i)\b(?:bunny\.net|bunnycdn|api\.bunny\.net|storage\.bunnycdn\.com)\b[\s\S]{0,160}\b(?:access[_-]?key|api[_-]?key|storage[_-]?password|password|token|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyBunny),
		NewRegex("imgix-api-token", "imgix API Token", "critical", []string{"imgix", "api.imgix.com"}, `(?i)\b(?:imgix|api\.imgix\.com)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|secure[_-]?url[_-]?token|token|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyImgix),
		...
		NewRegex("keycdn-api-key", "KeyCDN API Key", "critical", []string{"keycdn", "api.keycdn.com"}, `(?i)\b(?:keycdn|api\.keycdn\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|api[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyKeyCDN),
		...
		NewUnsafeRegex("tinypng-api-key", "TinyPNG API Key", "critical", []string{"tinypng", "tinify"}, `(?i)\b(?:tinypng|tinify|api\.tinify\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyTinyPNG),
		...
		NewReadOnlyRegex("cloudsmith-api-key", "Cloudsmith API Key", "critical", []string{"cloudsmith"}, `(?i)\bcloudsmith\b.{0,80}\bapi[_-]?key\b\s*[:=]\s*['\"]?([a-f0-9]{40})\b`, 1, verifyCloudsmith),
		NewReadOnlyRegex("eventbrite-private-token", "Eventbrite Private Token", "critical", []string{"eventbrite"}, `(?i)\beventbrite\b.{0,80}\b(?:private[_-]?token|oauth[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,80})\b`, 1, verifyEventbrite),
		NewAuthOnlyRegex("harvest-access-token", "Harvest Access Token", "critical", []string{"harvest"}, `(?i)\bharvest\b.{0,80}\b(?:access[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,128})\b`, 1, verifyHarvest),
		NewReadOnlyRegex("lokalise-token", "Lokalise Token", "critical", []string{"lokalise"}, `(?i)\blokalise\b.{0,80}\b(?:api[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{40})\b`, 1, verifyLokalise),
		...
		NewRegex("nylas-api-key", "Nylas API Key", "critical", []string{"nylas"}, `(?i)\bnylas\b.{0,80}\b(?:api[_-]?key|client[_-]?secret|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyNylas),
		NewRegex("pipedream-api-key", "Pipedream API Key", "critical", []string{"pipedream"}, `(?i)\bpipedream\b.{0,80}\b(?:api[_-]?key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyPipedream),
		NewRegex("percy-token", "Percy Token", "critical", []string{"PERCY_TOKEN", "percy"}, `(?i)\b(?:percy|percy_token)\b.{0,80}\b(?:token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{20,80})\b`, 1, verifyPercy),
		...
		NewReadOnlyRegex("ilert-api-key", "ilert API Key", "critical", []string{"ilert", "api.ilert.com"}, `(?i)\b(?:ilert|i-lert|api\.ilert\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|api[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyIlert),
		...
		NewReadOnlyRegex("semgrep-app-token", "Semgrep App Token", "critical", []string{"semgrep", "semgrep.dev"}, `(?i)\b(?:semgrep|semgrep\.dev|semgrep_app_token)\b[\s\S]{0,160}\b(?:app[_-]?token|api[_-]?token|api[_-]?key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifySemgrep),
		NewRegex("socketdev-api-key", "Socket.dev API Key", "critical", []string{"socket.dev", "api.socket.dev"}, `(?i)\b(?:socket\.dev|api\.socket\.dev|socketdev)\b[\s\S]{0,160}\b(?:api[_-]?key|access[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifySocketDev),
		...
		NewRegex("cronitor-api-key", "Cronitor API Key", "critical", []string{"cronitor", "cronitor.io"}, `(?i)\b(?:cronitor|cronitor\.io|cronitor_api_key)\b[\s\S]{0,160}\b(?:api[_-]?key|telemetry[_-]?key|ping[_-]?key|token|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyCronitor),
		...
		NewReadOnlyRegex("gocardless-access-token", "GoCardless Access Token", "critical", []string{"gocardless", "live_", "sandbox_"}, `(?i)\bgocardless\b.{0,80}\b(?:access[_-]?token|token)\b\s*[:=]\s*['\"]?((?:live|sandbox)_[A-Za-z0-9_-]{32,128})\b`, 1, verifyGoCardless),
		NewReadOnlyRegex("gumroad-access-token", "Gumroad Access Token", "high", []string{"gumroad"}, `(?i)\bgumroad\b.{0,80}\b(?:access[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyGumroad),
		NewReadOnlyRegex("hellosign-api-key", "HelloSign API Key", "high", []string{"hellosign", "dropboxsign"}, `(?i)\b(?:hellosign|dropbox[ _-]?sign)\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9+/_-]{64})(?:['\"\s,;]|$)`, 1, verifyHelloSign),
		NewUnsafeRegex("mailboxlayer-api-key", "Mailboxlayer API Key", "high", []string{"mailboxlayer"}, `(?i)\bmailboxlayer\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyMailboxlayer),
		NewUnsafeRegex("mediastack-api-key", "Mediastack API Key", "high", []string{"mediastack"}, `(?i)\bmediastack\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyMediastack),
		NewUnsafeRegex("opencage-api-key", "OpenCage API Key", "high", []string{"opencage"}, `(?i)\bopencage\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyOpenCage),
		NewReadOnlyRegex("packagecloud-token", "Packagecloud Token", "critical", []string{"packagecloud"}, `(?i)\bpackagecloud\b.{0,80}\b(?:api[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{48})\b`, 1, verifyPackagecloud),
		NewRegex("phrase-access-token", "Phrase Access Token", "critical", []string{"phrase"}, `(?i)\bphrase\b.{0,80}\b(?:access[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{40})\b`, 1, verifyPhrase),
		NewRegex("semaphore-api-token", "Semaphore API Token", "critical", []string{"semaphore"}, `(?i)\bsemaphore\b.{0,80}\b(?:api[_-]?token|auth[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifySemaphore),
		NewRegex("scrutinizer-token", "Scrutinizer CI Token", "high", []string{"scrutinizer"}, `(?i)\bscrutinizer\b.{0,80}\b(?:api[_-]?token|token)\b\s*[:=]\s*['\"]?([a-z0-9]{64})\b`, 1, verifyScrutinizer),
		...
		NewUnsafeRegex("openuv-api-key", "OpenUV API Key", "high", []string{"openuv"}, `(?i)\bopenuv\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyOpenUV),
		NewUnsafeRegex("pandascore-api-key", "PandaScore API Key", "high", []string{"pandascore"}, `(?i)\bpandascore\b.{0,80}\b(?:api[_-]?key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,64})\b`, 1, verifyPandaScore),
		NewReadOnlyRegex("paperform-api-key", "Paperform API Key", "high", []string{"paperform"}, `(?i)\bpaperform\b.{0,80}\b(?:api[_-]?key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._~+/=-]{800,1000})(?:['\"\s,;]|$)`, 1, verifyPaperform),
		NewReadOnlyRegex("parsehub-api-key", "ParseHub API Key", "high", []string{"parsehub"}, `(?i)\bparsehub\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyParseHub),
		NewRegex("pdfshift-api-key", "PDFShift API Key", "high", []string{"pdfshift"}, `(?i)\bpdfshift\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyPDFShift),
		NewAuthOnlyRegex("peopledatalabs-api-key", "People Data Labs API Key", "high", []string{"peopledatalabs", "people data labs"}, `(?i)\b(?:peopledatalabs|people[ _-]?data[ _-]?labs)\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{64})\b`, 1, verifyPeopleDataLabs),
		...
		NewUnsafeRegex("scraperapi-key", "ScraperAPI Key", "high", []string{"scraperapi"}, `(?i)\bscraperapi\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyScraperAPI),
		NewUnsafeRegex("scrapestack-api-key", "Scrapestack API Key", "high", []string{"scrapestack"}, `(?i)\bscrapestack\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([a-z0-9]{32})\b`, 1, verifyScrapestack),
		NewRegex("scrapingbee-api-key", "ScrapingBee API Key", "high", []string{"scrapingbee"}, `(?i)\bscrapingbee\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{80})\b`, 1, verifyScrapingBee),
		NewUnsafeRegex("serpstack-api-key", "Serpstack API Key", "high", []string{"serpstack"}, `(?i)\bserpstack\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([a-z0-9]{32})\b`, 1, verifySerpstack),
		NewRegex("shotstack-api-key", "Shotstack API Key", "high", []string{"shotstack"}, `(?i)\bshotstack\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{40})\b`, 1, verifyShotstack),
		...
		NewRegex("abstract-api-key", "Abstract API Key", "high", []string{"abstractapi", "abstract"}, `(?i)\b(?:abstractapi|abstract)\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-z0-9]{32})\b`, 1, verifyAbstractAPI),
		NewUnsafeRegex("alchemy-api-key", "Alchemy API Key", "critical", []string{"alchemy"}, `(?i)\balchemy\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32})\b`, 1, verifyAlchemy),
		NewReadOnlyRegex("apify-token", "Apify Token", "critical", []string{"apify_api_"}, `\b(apify_api_[A-Za-z0-9_-]{36})\b`, 1, verifyApify),
		NewRegex("apilayer-key", "APILayer Key", "high", []string{"apilayer"}, `(?i)\bapilayer\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyAPILayer),
		NewRegex("bannerbear-api-key", "Bannerbear API Key", "high", []string{"bannerbear"}, `(?i)\bbannerbear\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyBannerbear),
		NewReadOnlyRegex("baremetrics-api-key", "Baremetrics API Key", "high", []string{"baremetrics"}, `(?i)\bbaremetrics\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyBaremetrics),
		NewReadOnlyRegex("beamer-api-key", "Beamer API Key", "high", []string{"beamer"}, `(?i)\bbeamer\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_+/]{45}=)(?:['\"\s,;]|$)`, 1, verifyBeamer),
		NewRegex("bitbar-api-key", "Bitbar API Key", "high", []string{"bitbar"}, `(?i)\bbitbar\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyBitBar),
		NewRegex("blazemeter-api-key", "BlazeMeter API Key", "critical", []string{"blazemeter"}, `(?i)\bblazemeter\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{8}(?:-[a-f0-9]{4}){3}-[a-f0-9]{12})\b`, 1, verifyBlazeMeter),
		NewUnsafeRegex("buttercms-api-token", "ButterCMS API Token", "high", []string{"buttercms"}, `(?i)\bbuttercms\b.{0,80}\b(?:api[_-]?token|auth[_-]?token|token)\b\s*[:=]\s*['\"]?([a-z0-9]{40})\b`, 1, verifyButterCMS),
		NewRegex("canny-api-key", "Canny API Key", "high", []string{"canny"}, `(?i)\bcanny\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyCanny),
		NewAuthOnlyRegex("chartmogul-api-key", "ChartMogul API Key", "critical", []string{"chartmogul"}, `(?i)\bchartmogul\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyChartMogul),
		NewUnsafeRegex("clearbit-api-key", "Clearbit API Key", "critical", []string{"clearbit"}, `(?i)\bclearbit\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-z0-9_]{35})\b`, 1, verifyClearbit),
		NewReadOnlyRegex("clockify-api-key", "Clockify API Key", "high", []string{"clockify"}, `(?i)\bclockify\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{48})\b`, 1, verifyClockify),
		NewReadOnlyRegex("cloudconvert-api-key", "CloudConvert API Key", "critical", []string{"cloudconvert"}, `(?i)\bcloudconvert\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{40,128})\b`, 1, verifyCloudConvert),
		...
		NewRegex("convertapi-secret", "ConvertAPI Secret", "high", []string{"convertapi"}, `(?i)\bconvertapi\b.{0,80}\b(?:secret|api[_-]?secret|key)\b\s*[:=]\s*['\"]?(secret_[A-Za-z0-9]{16})\b`, 1, verifyConvertAPI),
		NewRegex("convertkit-api-secret", "ConvertKit API Secret", "critical", []string{"convertkit", "kit_api_secret"}, `(?i)\b(?:convertkit|kit[_-]?api[_-]?secret)\b.{0,80}\b(?:api[_-]?secret|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyConvertKit),
		NewReadOnlyRegex("dailyco-api-key", "Daily.co API Key", "critical", []string{"daily.co", "dailyco"}, `(?i)\b(?:daily\.co|dailyco)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyDaily),
		NewUnsafeRegex("deepai-api-key", "DeepAI API Key", "high", []string{"deepai"}, `(?i)\bdeepai\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-z0-9-]{36})\b`, 1, verifyDeepAI),
		...
		NewAuthOnlyRegex("fullstory-api-key", "FullStory API Key", "critical", []string{"fullstory"}, `(?i)\bfullstory\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyFullStory),
		NewRegex("flagsmith-server-key", "Flagsmith Server Key", "critical", []string{"flagsmith", "flagsmith.com"}, `(?i)\b(?:flagsmith|api\.flagsmith\.com)\b[\s\S]{0,160}\b(?:server[_-]?key|environment[_-]?key|api[_-]?key|token|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyFlagsmith),
		NewRegex("growthbook-api-key", "GrowthBook API Key", "critical", []string{"growthbook", "growthbook.io"}, `(?i)\b(?:growthbook|growthbook\.io|api\.growthbook\.io)\b[\s\S]{0,160}\b(?:api[_-]?key|secret[_-]?key|client[_-]?key|sdk[_-]?key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyGrowthBook),
		...
		NewRegex("configcat-sdk-key", "ConfigCat SDK Key", "critical", []string{"configcat", "configcat.com"}, `(?i)\b(?:configcat|cdn-global\.configcat\.com|api\.configcat\.com)\b[\s\S]{0,160}\b(?:sdk[_-]?key|api[_-]?key|configcat[_-]?key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._/-]{32,256})\b`, 1, verifyConfigCat),
		...
		NewRegex("pendo-integration-key", "Pendo Integration Key", "critical", []string{"pendo", "pendo.io"}, `(?i)\b(?:pendo|app\.pendo\.io|api\.pendo\.io)\b[\s\S]{0,160}\b(?:integration[_-]?key|api[_-]?key|metadata[_-]?key|token|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyPendo),
		...
		NewAuthOnlyRegex("kustomer-api-token", "Kustomer API Token", "critical", []string{"kustomer", "api.kustomerapp.com"}, `(?i)\b(?:kustomer|api\.kustomerapp\.com|kustomerapp\.com)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyKustomer),
		...
		NewRegex("productboard-api-token", "Productboard API Token", "critical", []string{"productboard", "api.productboard.com"}, `(?i)\b(?:productboard|api\.productboard\.com)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyProductboard),
		...
		NewReadOnlyRegex("planhat-api-token", "Planhat API Token", "critical", []string{"planhat", "api.planhat.com"}, `(?i)\b(?:planhat|api\.planhat\.com)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyPlanhat),
		NewUnsafeRegex("geoapify-api-key", "Geoapify API Key", "high", []string{"geoapify"}, `(?i)\bgeoapify\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyGeoapify),
		NewUnsafeRegex("graphhopper-api-key", "GraphHopper API Key", "high", []string{"graphhopper"}, `(?i)\bgraphhopper\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,64})\b`, 1, verifyGraphHopper),
		NewReadOnlyRegex("hunter-api-key", "Hunter API Key", "high", []string{"hunter"}, `(?i)\bhunter\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{40})\b`, 1, verifyHunter),
		NewReadOnlyRegex("imagekit-private-key", "ImageKit Private Key", "critical", []string{"imagekit", "private_"}, `(?i)\bimagekit\b.{0,80}\b(?:private[_-]?key|api[_-]?private[_-]?key)\b\s*[:=]\s*['\"]?(private_[A-Za-z0-9]{24,80})\b`, 1, verifyImageKit),
		NewUnsafeRegex("kickbox-api-key", "Kickbox API Key", "high", []string{"kickbox"}, `(?i)\bkickbox\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyKickbox),
		NewRegex("klipfolio-api-key", "Klipfolio API Key", "high", []string{"klipfolio"}, `(?i)\bklipfolio\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{40})\b`, 1, verifyKlipfolio),
		NewReadOnlyRegex("lob-api-key", "Lob API Key", "critical", []string{"lob", "live_", "test_"}, `(?i)\blob\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?((?:live|test)_[A-Za-z0-9]{35})\b`, 1, verifyLob),
		NewRegex("moosend-api-key", "Moosend API Key", "high", []string{"moosend"}, `(?i)\bmoosend\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9-]{36})\b`, 1, verifyMoosend),
		...
		NewUnsafeRegex("numverify-api-key", "Numverify API Key", "high", []string{"numverify"}, `(?i)\bnumverify\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyNumverify),
		NewReadOnlyRegex("omnisend-api-key", "Omnisend API Key", "critical", []string{"omnisend"}, `(?i)\bomnisend\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyOmnisend),
		...
		NewRegex("pandadoc-api-key", "PandaDoc API Key", "critical", []string{"pandadoc"}, `(?i)\bpandadoc\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyPandaDoc),
		NewRegex("partnerstack-api-key", "PartnerStack API Key", "critical", []string{"partnerstack"}, `(?i)\bpartnerstack\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{64})\b`, 1, verifyPartnerStack),
		...
		NewRegex("paymongo-secret-key", "PayMongo Secret Key", "critical", []string{"paymongo"}, `(?i)\bpaymongo\b.{0,80}\b(?:secret[_-]?key|key)\b\s*[:=]\s*['\"]?(sk_(?:live|test)_[A-Za-z0-9]{32,128})\b`, 1, verifyPayMongo),
		NewReadOnlyRegex("photoroom-api-key", "PhotoRoom API Key", "high", []string{"photoroom"}, `(?i)\bphotoroom\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyPhotoRoom),
		NewUnsafeRegex("proxycrawl-api-token", "ProxyCrawl API Token", "high", []string{"proxycrawl"}, `(?i)\bproxycrawl\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyProxyCrawl),
		NewReadOnlyRegex("qase-api-token", "Qase API Token", "critical", []string{"qase"}, `(?i)\bqase\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyQase),
		NewReadOnlyRegex("rebrandly-api-key", "Rebrandly API Key", "high", []string{"rebrandly"}, `(?i)\brebrandly\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32})\b`, 1, verifyRebrandly),
		...
		NewAuthOnlyRegexWithTrailingBoundary("replyio-api-key", "Reply.io API Key", "high", []string{"reply.io", "replyio"}, `(?i)\b(?:reply\.io|replyio)\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{20,64})`, 1, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_-", verifyReplyIO),
		NewRegex("restpack-htmltopdf-api-key", "Restpack HTML to PDF API Key", "high", []string{"restpack", "htmltopdf"}, `(?i)\brestpack\b.{0,120}\bhtml[_-]?to[_-]?pdf\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{48})\b`, 1, verifyRestpack),
		NewRegex("restpack-screenshot-api-key", "Restpack Screenshot API Key", "high", []string{"restpack", "screenshot"}, `(?i)\brestpack\b.{0,120}\bscreenshot\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{48})\b`, 1, verifyRestpackScreenshot),
		NewReadOnlyRegex("rocketreach-api-key", "RocketReach API Key", "critical", []string{"rocketreach"}, `(?i)\brocketreach\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyRocketReach),
		NewRegex("route4me-api-key", "Route4Me API Key", "high", []string{"route4me"}, `(?i:\broute4me\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?)([A-Z0-9]{32})\b`, 1, verifyRoute4Me),
		NewAuthOnlyRegex("salesflare-api-key", "Salesflare API Key", "critical", []string{"salesflare"}, `(?i)\bsalesflare\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_]{45})\b`, 1, verifySalesflare),
		NewAuthOnlyRegex("attio-api-key", "Attio API Key", "critical", []string{"attio", "api.attio.com"}, `(?i)\b(?:attio|api\.attio\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|access[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyAttio),
		NewAuthOnlyRegex("affinity-api-key", "Affinity API Key", "critical", []string{"affinity.co", "api.affinity.co"}, `(?i)\b(?:affinity\.co|api\.affinity\.co|affinity[_-]?api)\b[\s\S]{0,160}\b(?:api[_-]?key|bearer|token|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyAffinity),
		...
		NewAuthOnlyRegex("salesloft-api-key", "Salesloft API Key", "critical", []string{"ak_", "salesloft"}, `\b(ak_[0-9a-fA-F]{64})\b`, 1, verifySalesloft),
		...
		NewReadOnlyRegex("instantly-api-key", "Instantly API Key", "critical", []string{"instantly.ai", "api.instantly.ai"}, `(?i)\b(?:instantly\.ai|api\.instantly\.ai|instantly[_-]?api)\b[\s\S]{0,160}\b(?:api[_-]?key|access[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyInstantly),
		NewRegex("smartlead-api-key", "Smartlead API Key", "critical", []string{"smartlead.ai", "api.smartlead.ai"}, `(?i)\b(?:smartlead\.ai|api\.smartlead\.ai|server\.smartlead\.ai|smartlead[_-]?api)\b[\s\S]{0,160}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifySmartlead),
		...
		NewRegex("greenhouse-harvest-api-key", "Greenhouse Harvest API Key", "critical", []string{"greenhouse.io", "harvest.greenhouse.io"}, `(?i)\b(?:greenhouse\.io|harvest\.greenhouse\.io|greenhouse[_-]?harvest)\b[\s\S]{0,160}\b(?:harvest[_-]?api[_-]?key|api[_-]?key|access[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyGreenhouse),
		...
		NewAuthOnlyRegex("ashby-api-key", "Ashby API Key", "critical", []string{"ashbyhq.com", "api.ashbyhq.com"}, `(?i)\b(?:ashbyhq\.com|api\.ashbyhq\.com|ashby[_-]?api)\b[\s\S]{0,160}\b(?:api[_-]?key|access[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyAshby),
		NewReadOnlyRegex("workable-api-token", "Workable API Token", "critical", []string{"workable.com", "workable"}, `(?i)\b(?:workable\.com|api\.workable\.com|workable)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|access[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyWorkable),
		NewRegex("smartrecruiters-api-key", "SmartRecruiters API Key", "critical", []string{"smartrecruiters.com", "api.smartrecruiters.com"}, `(?i)\b(?:smartrecruiters\.com|api\.smartrecruiters\.com|smartrecruiters)\b[\s\S]{0,160}\b(?:api[_-]?key|x-smarttoken|access[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifySmartRecruiters),
		...
		NewUnsafeRegex("deel-api-token", "Deel API Token", "critical", []string{"deel.com", "api.deel.com"}, `(?i)\b(?:deel\.com|api\.deel\.com|deel[_-]?api)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|access[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyDeel),
		NewRegex("gusto-api-token", "Gusto API Token", "critical", []string{"gusto.com", "api.gusto.com"}, `(?i)\b(?:gusto\.com|api\.gusto\.com|gusto[_-]?api)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|access[_-]?token|bearer|token|client[_-]?secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9._~/-]{32,256})\b`, 1, verifyGusto),
		...
		NewUnsafeRegex("airvisual-api-key", "AirVisual API Key", "high", []string{"airvisual", "iqair"}, `(?i)\b(?:airvisual|iqair)\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-z0-9-]{36})\b`, 1, verifyAirVisual),
		...
		NewUnsafeRegex("ambee-api-key", "Ambee API Key", "high", []string{"ambee"}, `(?i)\bambee\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{64})\b`, 1, verifyAmbee),
		...
		NewReadOnlyRegex("apiflash-access-key", "APIFLASH Access Key", "high", []string{"apiflash"}, `(?i)\bapiflash\b.{0,80}\b(?:access[_-]?key|api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyAPIFlash),
		NewReadOnlyRegex("apitemplate-api-key", "APITemplate API Key", "high", []string{"apitemplate"}, `(?i)\bapitemplate\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{39})\b`, 1, verifyAPITemplate),
		...
		NewUnsafeRegex("appfollow-api-key", "AppFollow API Key", "high", []string{"appfollow"}, `(?i)\bappfollow\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?(eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9\.[A-Za-z0-9_-]{74}\.[A-Za-z0-9_-]{43})\b`, 1, verifyAppFollow),
		NewRegex("autoklose-api-key", "Autoklose API Key", "critical", []string{"autoklose"}, `(?i)\bautoklose\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9-]{32})\b`, 1, verifyAutoklose),
		NewUnsafeRegex("aviationstack-api-key", "Aviationstack API Key", "high", []string{"aviationstack"}, `(?i)\baviationstack\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyAviationstack),
		NewRegex("ayrshare-api-key", "Ayrshare API Key", "critical", []string{"ayrshare"}, `(?i)\bayrshare\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyAyrshare),
		NewAuthOnlyRegex("besttime-api-key", "BestTime API Key", "high", []string{"besttime"}, `(?i)\bbesttime\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?(pri_[a-f0-9]{32})\b`, 1, verifyBestTime),
		NewUnsafeRegex("brandfetch-api-key", "Brandfetch API Key", "high", []string{"brandfetch"}, `(?i)\bbrandfetch\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_+/]{43}=)(?:['\"\s,;]|$)`, 1, verifyBrandfetch),
		NewReadOnlyRegex("browshot-api-key", "Browshot API Key", "high", []string{"browshot"}, `(?i)\bbrowshot\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9-]{28})\b`, 1, verifyBrowshot),
		NewUnsafeRegex("calendarific-api-key", "Calendarific API Key", "high", []string{"calendarific"}, `(?i)\bcalendarific\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyCalendarific),
		NewUnsafeRegex("carboninterface-api-key", "Carbon Interface API Key", "high", []string{"carboninterface", "carbon interface"}, `(?i)\b(?:carboninterface|carbon[ _-]?interface)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{21})\b`, 1, verifyCarbonInterface),
		NewRegex("craftmypdf-api-key", "CraftMyPDF API Key", "high", []string{"craftmypdf"}, `(?i)\bcraftmypdf\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyCraftMyPDF),
		NewUnsafeRegex("currentsapi-api-key", "CurrentsAPI Key", "high", []string{"currentsapi"}, `(?i)\bcurrentsapi\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{48})\b`, 1, verifyCurrentsAPI),
		NewUnsafeRegex("debounce-api-key", "DeBounce API Key", "high", []string{"debounce"}, `(?i)\bdebounce\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{13})\b`, 1, verifyDeBounce),
		NewAuthOnlyRegex("detectlanguage-api-key", "Detect Language API Key", "high", []string{"detectlanguage", "detect language"}, `(?i)\b(?:detectlanguage|detect[ _-]?language)\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-z0-9]{32})\b`, 1, verifyDetectLanguage),
		...
		NewRegex("codemagic-api-token", "Codemagic API Token", "critical", []string{"codemagic"}, `(?i)\bcodemagic\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyCodemagic),
		...
		NewReadOnlyRegex("diffbot-api-token", "Diffbot API Token", "high", []string{"diffbot"}, `(?i)\bdiffbot\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([a-z0-9]{32})\b`, 1, verifyDiffbot),
		...
		NewUnsafeRegex("ethplorer-api-key", "Ethplorer API Key", "high", []string{"ethplorer"}, `(?i)\bethplorer\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyEthplorer),
		...
		NewRegex("geckoboard-api-key", "Geckoboard API Key", "high", []string{"geckoboard"}, `(?i)\bgeckoboard\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{44})\b`, 1, verifyGeckoboard),
		...
		NewUnsafeRegex("holidayapi-key", "Holiday API Key", "high", []string{"holidayapi", "holiday api"}, `(?i)\b(?:holidayapi|holiday[ _-]?api)\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-z0-9-]{36})\b`, 1, verifyHolidayAPI),
		NewUnsafeRegex("html2pdf-api-key", "HTML2PDF API Key", "high", []string{"html2pdf"}, `(?i)\bhtml2pdf\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{64})\b`, 1, verifyHTML2PDF),
		NewUnsafeRegex("ip2location-api-key", "IP2Location API Key", "high", []string{"ip2location"}, `(?i)\bip2location\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyIP2Location),
		NewUnsafeRegex("ipapi-api-key", "ipapi API Key", "high", []string{"ipapi"}, `(?i)\bipapi\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyIPAPI),
		NewUnsafeRegex("ipinfodb-api-key", "IPInfoDB API Key", "high", []string{"ipinfodb"}, `(?i)\bipinfodb\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{64})\b`, 1, verifyIPInfoDB),
		NewRegex("jotform-api-key", "Jotform API Key", "high", []string{"jotform"}, `(?i)\bjotform\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyJotform),
		...
		NewUnsafeRegex("languagelayer-api-key", "Languagelayer API Key", "high", []string{"languagelayer"}, `(?i)\blanguagelayer\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyLanguageLayer),
		...
		NewUnsafeRegex("linkpreview-api-key", "LinkPreview API Key", "high", []string{"linkpreview"}, `(?i)\blinkpreview\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyLinkPreview),
		...
		NewUnsafeRegex("mockaroo-api-key", "Mockaroo API Key", "high", []string{"mockaroo"}, `(?i)\bmockaroo\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-z0-9]{8})\b`, 1, verifyMockaroo),
		...
		NewReadOnlyRegex("oanda-api-token", "OANDA API Token", "critical", []string{"oanda"}, `(?i)\boanda\b.{0,80}\b(?:api[_-]?token|personal[_-]?access[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyOANDA),
		NewAuthOnlyRegex("onfleet-api-key", "Onfleet API Key", "critical", []string{"onfleet"}, `(?i)\bonfleet\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyOnfleet),
		NewUnsafeRegex("pdflayer-api-key", "PDFLayer API Key", "high", []string{"pdflayer"}, `(?i)\bpdflayer\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyPDFLayer),
		NewRegex("pepipost-api-key", "Pepipost API Key", "high", []string{"pepipost", "netcore"}, `(?i)\b(?:pepipost|netcore)\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9-]{32})\b`, 1, verifyPepipost),
		NewRegex("pivotaltracker-api-token", "Pivotal Tracker API Token", "high", []string{"pivotaltracker", "pivotal tracker"}, `(?i)\b(?:pivotaltracker|pivotal[ _-]?tracker)\b.{0,80}\b(?:api[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyPivotalTracker),
		NewUnsafeRegex("pixabay-api-key", "Pixabay API Key", "high", []string{"pixabay"}, `(?i)\bpixabay\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-z0-9-]{34})\b`, 1, verifyPixabay),
		NewAuthOnlyRegex("podio-api-token", "Podio API Token", "critical", []string{"podio"}, `(?i)\bpodio\b.{0,80}\b(?:api[_-]?token|access[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyPodio),
		...
		NewUnsafeRegex("rawg-api-key", "RAWG API Key", "high", []string{"rawg"}, `(?i)\brawg\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyRAWG),
		...
		NewUnsafeRegex("scrapfly-api-key", "Scrapfly API Key", "high", []string{"scrapfly"}, `(?i)\bscrapfly\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?((?:scp-(?:live|test)-)?[a-z0-9]{32})\b`, 1, verifyScrapfly),
		...
		NewUnsafeRegex("screenshotlayer-api-key", "Screenshotlayer API Key", "high", []string{"screenshotlayer"}, `(?i)\bscreenshotlayer\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_]{32})\b`, 1, verifyScreenshotLayer),
		NewReadOnlyRegex("selectpdf-api-key", "SelectPdf API Key", "high", []string{"selectpdf"}, `(?i)\bselectpdf\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-z0-9-]{36})\b`, 1, verifySelectPDF),
		...
		NewReadOnlyRegex("shipday-api-key", "Shipday API Key", "critical", []string{"shipday"}, `(?i)\bshipday\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9.]{11}[A-Za-z0-9]{20})\b`, 1, verifyShipday),
		...
		NewRegex("signaturit-api-key", "Signaturit API Key", "critical", []string{"signaturit"}, `(?i)\bsignaturit\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{86})\b`, 1, verifySignaturit),
		NewRegex("simplesat-api-key", "Simplesat API Key", "high", []string{"simplesat"}, `(?i)\bsimplesat\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9]{40})\b`, 1, verifySimpleSat),
		...
		NewRegex("snipcart-api-key", "Snipcart API Key", "critical", []string{"snipcart"}, `(?i)\bsnipcart\b.{0,80}\b(?:secret[_-]?api[_-]?key|api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_]{75})\b`, 1, verifySnipcart),
		NewUnsafeRegex("spoonacular-api-key", "Spoonacular API Key", "high", []string{"spoonacular"}, `(?i)\bspoonacular\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([a-z0-9]{32})\b`, 1, verifySpoonacular),
		NewReadOnlyRegex("sportsmonk-api-token", "SportsMonk API Token", "high", []string{"sportsmonk"}, `(?i)\bsportsmonk\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{60})\b`, 1, verifySportsMonk),
		...
		NewReadOnlyRegex("statuscake-api-key", "StatusCake API Key", "high", []string{"statuscake"}, `(?i)\bstatuscake\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyStatusCake),
		NewUnsafeRegex("stockdata-api-key", "StockData API Key", "high", []string{"stockdata"}, `(?i)\bstockdata\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{40})\b`, 1, verifyStockData),
		...
		NewRegex("shippo-api-token", "Shippo API Token", "critical", []string{"shippo", "api.goshippo.com"}, `(?i)\b(?:shippo|api\.goshippo\.com|goshippo)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|bearer|token)\b\s*[:=]\s*['\"]?((?:shippo_(?:live|test)_)?[A-Za-z0-9._-]{32,256})\b`, 1, verifyShippo),
		NewReadOnlyRegexWithTrailingBoundary("easypost-api-key", "EasyPost API Key", "critical", []string{"EZAK", "EZTK", "easypost"}, `\b((?:EZAK|EZTK)[A-Za-z0-9._-]{24,256})`, 1, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789._-", verifyEasyPost),
		...
		NewRegex("shipengine-api-key", "ShipEngine API Key", "critical", []string{"shipengine", "api.shipengine.com"}, `(?i)\b(?:shipengine|api\.shipengine\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|api[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyShipEngine),
		NewReadOnlyRegex("aftership-api-key", "AfterShip API Key", "critical", []string{"asat_", "aftership"}, `\b(asat_[0-9a-f]{32})\b`, 1, verifyAfterShip),
		NewRegex("easyship-api-token", "Easyship API Token", "critical", []string{"easyship", "api.easyship.com"}, `(?i)\b(?:easyship|api\.easyship\.com)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|access[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyEasyship),
		...
		NewUnsafeRegex("tatum-api-key", "Tatum API Key", "critical", []string{"tatum"}, `(?i)\btatum\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyTatum),
		NewRegex("taxjar-api-token", "TaxJar API Token", "critical", []string{"taxjar"}, `(?i)\btaxjar\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyTaxJar),
		...
		NewAuthOnlyRegex("tiingo-api-token", "Tiingo API Token", "high", []string{"tiingo"}, `(?i)\btiingo\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyTiingo),
		NewReadOnlyRegex("timecamp-api-token", "TimeCamp API Token", "high", []string{"timecamp"}, `(?i)\btimecamp\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([a-z0-9]{26})\b`, 1, verifyTimeCamp),
		...
		NewAuthOnlyRegex("toggltrack-api-token", "Toggl Track API Token", "high", []string{"toggl", "toggltrack"}, `(?i)\b(?:toggltrack|toggl)\b.{0,80}\b(?:api[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyTogglTrack),
		NewUnsafeRegex("tomtom-api-key", "TomTom API Key", "high", []string{"tomtom"}, `(?i)\btomtom\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyTomTom),
		NewReadOnlyRegex("transferwise-api-token", "Wise API Token", "critical", []string{"transferwise", "wise"}, `(?i)\b(?:transferwise|wise)\b.{0,80}\b(?:api[_-]?token|token|access[_-]?token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyWise),
		NewUnsafeRegex("unsplash-access-key", "Unsplash Access Key", "high", []string{"unsplash"}, `(?i)\bunsplash\b.{0,80}\b(?:access[_-]?key|api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_]{43})\b`, 1, verifyUnsplash),
		NewUnsafeRegex("userstack-api-key", "Userstack API Key", "high", []string{"userstack"}, `(?i)\buserstack\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([a-z0-9]{32})\b`, 1, verifyUserstack),
		NewUnsafeRegex("visualcrossing-api-key", "Visual Crossing API Key", "high", []string{"visualcrossing", "visual crossing"}, `(?i:\b(?:visualcrossing|visual[ _-]?crossing)\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?)([A-Z0-9]{25})\b`, 1, verifyVisualCrossing),
		NewRegex("voicegain-api-key", "Voicegain API Key", "critical", []string{"voicegain"}, `(?i)\bvoicegain\b.{0,80}\b(?:api[_-]?key|jwt|token|key)\b\s*[:=]\s*['\"]?(ey[A-Za-z0-9_-]{34}\.ey[A-Za-z0-9_-]{108}\.[A-Za-z0-9_-]{43})\b`, 1, verifyVoicegain),
		...
		NewUnsafeRegex("yelp-api-key", "Yelp API Key", "high", []string{"yelp"}, `(?i)\byelp\b.{0,80}\b(?:api[_-]?key|key|bearer[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{128})\b`, 1, verifyYelp),
		NewAuthOnlyRegex("ynab-api-token", "YNAB API Token", "critical", []string{"youneedabudget", "ynab"}, `(?i)\b(?:youneedabudget|ynab)\b.{0,80}\b(?:api[_-]?token|personal[_-]?access[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyYNAB),
		...
		NewRegex("zerobounce-api-key", "ZeroBounce API Key", "high", []string{"zerobounce"}, `(?i)\bzerobounce\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyZeroBounce),
		...
		NewReadOnlyRegex("smartsheet-access-token", "Smartsheet Access Token", "critical", []string{"smartsheet"}, `(?i)\bsmartsheets?\b.{0,80}\b(?:access[_-]?token|api[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifySmartsheet),
		...
		NewRegex("thousandeyes-token", "ThousandEyes Token", "critical", []string{"thousandeyes", "thousand eyes"}, `(?i)\b(?:thousandeyes|thousand[ _-]?eyes)\b.{0,80}\b(?:bearer[_-]?token|oauth[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyThousandEyes),
		NewUnsafeRegex("ticketmaster-api-key", "Ticketmaster API Key", "high", []string{"ticketmaster"}, `(?i)\bticketmaster\b.{0,80}\b(?:api[_-]?key|consumer[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyTicketmaster),
		NewReadOnlyRegex("theoddsapi-key", "The Odds API Key", "high", []string{"theoddsapi", "the odds api"}, `(?i)\b(?:theoddsapi|the[ _-]?odds[ _-]?api)\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyTheOddsAPI),
		...
		NewRegex("teachable-api-key", "Teachable API Key", "critical", []string{"teachable", "teachable.com"}, `(?i)\b(?:teachable|teachable\.com|developers\.teachable\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|apikey)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyTeachable),
		...
		NewRegex("ubidots-token", "Ubidots Token", "high", []string{"ubidots"}, `(?i)\bubidots\b.{0,80}\b(?:token|api[_-]?token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyUbidots),
		NewUnsafeRegex("uclassify-api-key", "uClassify API Key", "high", []string{"uclassify"}, `(?i)\buclassify\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{12})\b`, 1, verifyUClassify),
		NewReadOnlyRegex("upcdatabase-api-key", "UPC Database API Key", "high", []string{"upcdatabase", "upc database"}, `(?i)\b(?:upcdatabase|upc[ _-]?database)\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyUPCDatabase),
		NewReadOnlyRegex("uplead-api-key", "UpLead API Key", "critical", []string{"uplead"}, `(?i)\buplead\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9-]{32})\b`, 1, verifyUpLead),
		NewReadOnlyRegex("vbout-api-key", "VBOUT API Key", "high", []string{"vbout"}, `(?i)\bvbout\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([0-9]{25})\b`, 1, verifyVBOUT),
		NewReadOnlyRegex("veriphone-api-key", "Veriphone API Key", "high", []string{"veriphone"}, `(?i)\bveriphone\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyVeriphone),
		NewUnsafeRegex("walkscore-api-key", "Walk Score API Key", "high", []string{"walkscore", "walk score"}, `(?i)\b(?:walkscore|walk[ _-]?score)\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyWalkScore),
		...
		NewReadOnlyRegex("whoxy-api-key", "Whoxy API Key", "high", []string{"whoxy"}, `(?i)\bwhoxy\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{33})\b`, 1, verifyWhoxy),
		NewRegex("wistia-api-token", "Wistia API Token", "critical", []string{"wistia"}, `(?i)\bwistia\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyWistia),
		NewRegex("wit-ai-token", "Wit.ai Token", "critical", []string{"wit.ai", "wit"}, `(?i)\b(?:wit\.ai|wit)\b.{0,80}\b(?:server[_-]?access[_-]?token|access[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyWitAI),
		NewRegex("ticket-tailor-api-key", "Ticket Tailor API Key", "high", []string{"tickettailor", "ticket tailor"}, `(?i)\b(?:tickettailor|ticket[ _-]?tailor)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?(sk_[0-9]{4}_[0-9]{6}_[a-f0-9]{32})\b`, 1, verifyTicketTailor),
		NewAuthOnlyRegex("tmetric-api-token", "TMetric API Token", "high", []string{"tmetric"}, `(?i)\btmetric\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyTMetric),
		...
		NewUnsafeRegex("signupgenius-api-key", "SignUpGenius API Key", "high", []string{"signupgenius", "signup genius"}, `(?i)\b(?:signupgenius|signup[ _-]?genius)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifySignUpGenius),
		...
		NewReadOnlyRegex("skrapp-api-key", "Skrapp API Key", "high", []string{"skrapp"}, `(?i)\bskrapp\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{42})\b`, 1, verifySkrapp),
		...
		NewRegex("sslmate-api-key", "SSLMate API Key", "critical", []string{"sslmate"}, `(?i)\bsslmate\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifySSLMate),
		...
		NewRegex("storecove-api-key", "Storecove API Key", "critical", []string{"storecove"}, `(?i)\bstorecove\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{43})\b`, 1, verifyStorecove),
		NewRegex("stormboard-api-key", "Stormboard API Key", "high", []string{"stormboard"}, `(?i)\bstormboard\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyStormboard),
		NewAuthOnlyRegex("streak-api-key", "Streak API Key", "high", []string{"streak"}, `(?i)\bstreak\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyStreak),
		...
		NewAuthOnlyRegex("abyssale-api-key", "Abyssale API Key", "high", []string{"abyssale"}, `(?i)\babyssale\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{40})\b`, 1, verifyAbyssale),
		NewRegex("adafruit-io-key", "Adafruit IO Key", "high", []string{"adafruit", "adafruitio"}, `(?i)\b(?:adafruit[ _-]?io|adafruit)\b.{0,80}\b(?:aio[_-]?key|io[_-]?key|api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyAdafruitIO),
		...
		NewRegex("alconost-api-key", "Alconost API Key", "high", []string{"alconost"}, `(?i)\balconost\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyAlconost),
		...
		NewUnsafeRegex("aletheia-api-key", "Aletheia API Key", "high", []string{"aletheia"}, `(?i)\baletheia\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Z0-9]{32})\b`, 1, verifyAletheia),
		NewUnsafeRegex("allsports-api-key", "AllSports API Key", "high", []string{"allsports", "all sports"}, `(?i)\b(?:allsports|all[ _-]?sports)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9]{64})\b`, 1, verifyAllSports),
		...
		NewRegex("apacta-api-key", "Apacta API Key", "high", []string{"apacta"}, `(?i)\bapacta\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{8}(?:-[a-f0-9]{4}){3}-[a-f0-9]{12})\b`, 1, verifyApacta),
		NewUnsafeRegex("api2cart-api-key", "API2Cart API Key", "high", []string{"api2cart"}, `(?i)\bapi2cart\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyAPI2Cart),
		...
		NewRegex("apimatic-api-key", "APIMatic API Key", "high", []string{"apimatic"}, `(?i)\bapimatic\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{64})\b`, 1, verifyAPIMatic),
		NewReadOnlyRegex("apimetrics-api-key", "APImetrics API Key", "high", []string{"apimetrics"}, `(?i)\bapimetrics\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyAPIMetrics),
		NewRegex("appointedd-api-key", "Appointedd API Key", "high", []string{"appointedd"}, `(?i)\bappointedd\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9+=]{88})(?:['\"\s,;]|$)`, 1, verifyAppointedd),
		...
		NewRegex("autopilot-api-key", "Autopilot API Key", "high", []string{"autopilot"}, `(?i)\bautopilot\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyAutopilot),
		NewRegex("axonaut-api-key", "Axonaut API Key", "high", []string{"axonaut"}, `(?i)\baxonaut\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9]{32})\b`, 1, verifyAxonaut),
		...
		NewRegex("beebole-api-token", "Beebole API Token", "high", []string{"beebole"}, `(?i)\bbeebole\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([a-z0-9]{40})\b`, 1, verifyBeebole),
		...
		NewUnsafeRegex("bugherd-api-key", "BugHerd API Key", "high", []string{"bugherd"}, `(?i)\bbugherd\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9]{22})\b`, 1, verifyBugHerd),
		...
		NewRegex("caflou-api-key", "Caflou API Key", "high", []string{"caflou"}, `(?i)\bcaflou\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?(eyJhbGciOiJIUzI1NiJ9\.[A-Za-z0-9_-]{40,200}\.[A-Za-z0-9_-]{43})\b`, 1, verifyCaflou),
		NewUnsafeRegex("calorieninja-api-key", "CalorieNinjas API Key", "high", []string{"calorieninja", "calorieninjas", "calorie ninjas"}, `(?i)\b(?:calorie[ _-]?ninjas?|calorieninjas?)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{40})\b`, 1, verifyCalorieNinjas),
		...
		NewReadOnlyRegex("captaindata-api-key", "Captain Data API Key", "high", []string{"captaindata", "captain data"}, `(?i)\b(?:captaindata|captain[ _-]?data)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyCaptainData),
		...
		NewReadOnlyRegex("chatbot-api-key", "ChatBot API Key", "critical", []string{"chatbot"}, `(?i)\bchatbot\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_]{32})\b`, 1, verifyChatBot),
		...
		NewRegex("cliengo-api-key", "Cliengo API Key", "high", []string{"cliengo"}, `(?i)\bcliengo\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{8}(?:-[a-f0-9]{4}){3}-[a-f0-9]{12})\b`, 1, verifyCliengo),
		...
		NewReadOnlyRegex("column-api-key", "Column API Key", "critical", []string{"column"}, `(?i)\bcolumn\b.{0,80}\b(?:api[_-]?key|key|token|secret)\b\s*[:=]\s*['\"]?((?:test|live)_[A-Za-z0-9]{27})\b`, 1, verifyColumn),
		...
		NewUnsafeRegex("commodities-api-key", "Commodities API Key", "high", []string{"commodities"}, `(?i)\bcommodities\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyCommoditiesAPI),
		...
		NewUnsafeRegex("countrylayer-api-key", "Countrylayer API Key", "high", []string{"countrylayer"}, `(?i)\bcountrylayer\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyCountryLayer),
		...
		NewUnsafeRegex("dandelion-api-key", "Dandelion API Key", "high", []string{"dandelion"}, `(?i)\bdandelion\b.{0,80}\b(?:api[_-]?key|token|key)\b\s*[:=]\s*['\"]?([a-z0-9]{32})\b`, 1, verifyDandelion),
		...
		NewRegex("datagov-api-key", "Data.gov API Key", "high", []string{"data.gov", "datagov"}, `(?i)\b(?:data\.gov|datagov)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{40})\b`, 1, verifyDataGov),
		...
		NewReadOnlyRegex("diggernaut-api-key", "Diggernaut API Key", "high", []string{"diggernaut"}, `(?i)\bdiggernaut\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9]{40})\b`, 1, verifyDiggernaut),
		...
		NewAuthOnlyRegex("docparser-api-key", "Docparser API Key", "high", []string{"docparser"}, `(?i)\bdocparser\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{40})\b`, 1, verifyDocparser),
		...
		NewRegex("dynalist-api-token", "Dynalist API Token", "high", []string{"dynalist"}, `(?i)\bdynalist\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyDynalist),
		NewReadOnlyRegex("dyspatch-api-key", "Dyspatch API Key", "critical", []string{"dyspatch"}, `(?i)\bdyspatch\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{52})\b`, 1, verifyDyspatch),
		...
		NewRegex("envoy-api-key", "Envoy API Key", "critical", []string{"envoy"}, `(?i)\benvoy\b.{0,80}\b(?:api[_-]?key|key|token|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{220})\b`, 1, verifyEnvoy),
		...
		NewAuthOnlyRegex("everhour-api-key", "Everhour API Key", "high", []string{"everhour"}, `(?i)\beverhour\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyEverhour),
		...
		NewRegex("feedier-api-key", "Feedier API Key", "high", []string{"feedier"}, `(?i)\bfeedier\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyFeedier),
		...
		NewUnsafeRegex("finage-api-key", "Finage API Key", "high", []string{"finage"}, `(?i)\bfinage\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?(API_KEY[A-Z0-9]{32})\b`, 1, verifyFinage),
		...
		NewAuthOnlyRegex("flatio-api-key", "Flatio API Key", "high", []string{"flatio"}, `(?i)\bflatio\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyFlat),
		...
		NewRegex("flexport-api-key", "Flexport API Key", "critical", []string{"flexport"}, `(?i)\bflexport\b.{0,80}\b(?:api[_-]?key|key|token|secret)\b\s*[:=]\s*['\"]?(shltm_[A-Za-z0-9_-]{40})\b`, 1, verifyFlexport),
		NewRegex("flickr-api-key", "Flickr API Key", "high", []string{"flickr"}, `(?i)\bflickr\b.{0,80}\b(?:api[_-]?key|key|secret|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyFlickr),
		NewUnsafeRegex("flightapi-key", "FlightAPI Key", "high", []string{"flightapi", "flight api"}, `(?i)\b(?:flightapi|flight[ _-]?api)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9]{24})\b`, 1, verifyFlightAPI),
		...
		NewRegex("float-api-key", "Float API Key", "high", []string{"float"}, `(?i)\bfloat\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{16}[A-Za-z0-9+/]{42,43}=)(?:['\"\s,;]|$)`, 1, verifyFloat),
		...
		NewRegex("frameio-api-token", "Frame.io API Token", "critical", []string{"frame.io", "frameio"}, `(?i)\b(?:frame\.io|frameio)\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyFrameIO),
		...
		NewRegex("fulcrum-api-token", "Fulcrum API Token", "high", []string{"fulcrum"}, `(?i)\bfulcrum\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyFulcrum),
		...
		NewUnsafeRegex("geocodify-api-key", "Geocodify API Key", "high", []string{"geocodify"}, `(?i)\bgeocodify\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9]{40})\b`, 1, verifyGeocodify),
		...
		NewUnsafeRegex("getgeoapi-key", "GetGeoAPI Key", "high", []string{"getgeoapi", "get geo api"}, `(?i)\b(?:getgeoapi|get[ _-]?geo[ _-]?api)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9]{40})\b`, 1, verifyGetGeoAPI),
		...
		NewReadOnlyRegex("glassnode-api-key", "Glassnode API Key", "high", []string{"glassnode"}, `(?i)\bglassnode\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{27})\b`, 1, verifyGlassnode),
		...
		NewRegex("goodday-api-key", "GoodDay API Key", "high", []string{"goodday", "good day"}, `(?i)\b(?:goodday|good[ _-]?day)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9]{32})\b`, 1, verifyGoodDay),
		...
		NewReadOnlyRegex("groovehq-api-key", "GrooveHQ API Key", "high", []string{"groovehq", "groove hq"}, `(?i)\b(?:groovehq|groove[ _-]?hq)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{64})\b`, 1, verifyGrooveHQ),
		NewReadOnlyRegex("gtmetrix-api-key", "GTmetrix API Key", "high", []string{"gtmetrix"}, `(?i)\bgtmetrix\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyGTmetrix),
		NewRegex("guru-api-key", "Guru API Key", "high", []string{"guru"}, `(?i)\bguru\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, nil),
		NewRegex("gyazo-api-token", "Gyazo API Token", "high", []string{"gyazo"}, `(?i)\bgyazo\b.{0,80}\b(?:api[_-]?token|access[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyGyazo),
		NewRegex("happyscribe-api-key", "Happy Scribe API Key", "high", []string{"happyscribe", "happy scribe"}, `(?i)\b(?:happyscribe|happy[ _-]?scribe)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{24})\b`, 1, verifyHappyScribe),
		...
		NewAuthOnlyRegex("humanity-api-key", "Humanity API Key", "high", []string{"humanity"}, `(?i)\bhumanity\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9]{40})\b`, 1, verifyHumanity),
		NewReadOnlyRegexWithTrailingBoundary("hybiscus-api-key", "Hybiscus API Key", "high", []string{"hybiscus"}, `(?i)\bhybiscus\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{43})`, 1, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_-", verifyHybiscus),
		...
		NewAuthOnlyRegex("ibmcloud-user-key", "IBM Cloud User Key", "critical", []string{"ibmcloud", "ibm cloud"}, `(?i)\b(?:ibmcloud|ibm[ _-]?cloud)\b.{0,120}\b(?:user[_-]?key|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{44})\b`, 1, verifyIBMCloud),
		NewReadOnlyRegex("iconfinder-api-key", "Iconfinder API Key", "high", []string{"iconfinder"}, `(?i)\biconfinder\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{64})\b`, 1, verifyIconfinder),
		...
		NewRegex("insightly-api-key", "Insightly API Key", "high", []string{"insightly"}, `(?i)\binsightly\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9-]{36})\b`, 1, verifyInsightly),
		...
		NewRegex("interseller-api-key", "Interseller API Key", "high", []string{"interseller"}, `(?i)\binterseller\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{8}(?:-[a-f0-9]{4}){3}-[a-f0-9]{12})\b`, 1, verifyInterseller),
		...
		NewRegex("intrinio-api-key", "Intrinio API Key", "high", []string{"intrinio"}, `(?i)\bintrinio\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyIntrinio),
		...
		NewReadOnlyRegex("juro-api-key", "Juro API Key", "critical", []string{"juro"}, `(?i)\bjuro\b.{0,80}\b(?:api[_-]?key|key|token|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{40})\b`, 1, verifyJuro),
		...
		NewReadOnlyRegex("knapsackpro-api-token", "Knapsack Pro API Token", "high", []string{"knapsackpro", "knapsack pro"}, `(?i)\b(?:knapsackpro|knapsack[ _-]?pro)\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([a-z0-9]{32})\b`, 1, verifyKnapsackPro),
		...
		NewReadOnlyRegex("kylas-api-key", "Kylas API Key", "high", []string{"kylas"}, `(?i)\bkylas\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9-]{36})\b`, 1, verifyKylas),
		NewReadOnlyRegex("leadfeeder-api-key", "Leadfeeder API Key", "high", []string{"leadfeeder"}, `(?i)\bleadfeeder\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9-]{43})\b`, 1, verifyLeadfeeder),
		...
		NewReadOnlyRegex("loadmill-api-key", "Loadmill API Key", "high", []string{"loadmill"}, `(?i)\bloadmill\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{40})\b`, 1, verifyLoadmill),
		NewAuthOnlyRegex("loyverse-api-token", "Loyverse API Token", "high", []string{"loyverse"}, `(?i)\bloyverse\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyLoyverse),
		NewAuthOnlyRegex("lunchmoney-api-token", "Lunch Money API Token", "high", []string{"lunchmoney", "lunch money"}, `(?i)\b(?:lunchmoney|lunch[ _-]?money)\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyLunchMoney),
		...
		NewAuthOnlyRegex("madkudu-api-key", "MadKudu API Key", "high", []string{"madkudu"}, `(?i)\bmadkudu\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyMadKudu),
		...
		NewRegex("mailjetsms-api-token", "Mailjet SMS API Token", "critical", []string{"mailjetsms", "mailjet sms"}, `(?i)\b(?:mailjetsms|mailjet[ _-]?sms)\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyMailjetSMS),
		NewUnsafeRegex("mailsac-api-key", "Mailsac API Key", "high", []string{"mailsac", "k_"}, `(?i)\bmailsac\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?(k_[a-z0-9]{24}|[A-Za-z0-9_-]{32,128})\b`, 1, verifyMailsac),
		...
		NewRegex("mavenlink-api-token", "Mavenlink API Token", "high", []string{"mavenlink"}, `(?i)\bmavenlink\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyMavenlink),
		NewAuthOnlyRegex("meistertask-api-token", "MeisterTask API Token", "high", []string{"meistertask", "meister task"}, `(?i)\b(?:meistertask|meister[ _-]?task)\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyMeisterTask),
		NewRegex("meraki-api-key", "Meraki API Key", "critical", []string{"meraki"}, `(?i)\bmeraki\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyMeraki),
		...
		NewAuthOnlyRegex("mindmeister-api-token", "MindMeister API Token", "high", []string{"mindmeister", "mind meister"}, `(?i)\b(?:mindmeister|mind[ _-]?meister)\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{43})\b`, 1, verifyMindMeister),
		NewRegex("miro-api-token", "Miro API Token", "critical", []string{"miro"}, `(?i)\bmiro\b.{0,80}\b(?:api[_-]?token|access[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyMiro),
		...
		NewRegex("nimble-api-key", "Nimble API Key", "high", []string{"nimble"}, `(?i)\bnimble\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{30})\b`, 1, verifyNimble),
		...
		NewRegex("optimizely-api-key", "Optimizely API Key", "critical", []string{"optimizely"}, `(?i)\boptimizely\b.{0,80}\b(?:api[_-]?key|personal[_-]?access[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9:-]{54})\b`, 1, verifyOptimizely),
		NewRegex("overloop-api-key", "Overloop API Key", "high", []string{"overloop"}, `(?i)\boverloop\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{50})\b`, 1, verifyOverloop),
		...
		NewAuthOnlyRegexWithTrailingBoundary("parseur-api-key", "Parseur API Key", "high", []string{"parseur"}, `(?i)\bparseur\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{40}|sk_[A-Za-z0-9_-]{8,128})`, 1, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_-", verifyParseur),
		...
		NewRegex("qubole-api-token", "Qubole API Token", "critical", []string{"qubole"}, `(?i)\bqubole\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([a-z0-9]{64})\b`, 1, verifyQubole),
		...
		NewAuthOnlyRegexWithTrailingBoundary("reachmail-api-key", "ReachMail API Key", "high", []string{"reachmail", "reach mail"}, `(?i)\b(?:reachmail|reach[ _-]?mail)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{64})`, 1, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_-", verifyReachMail),
		...
		NewReadOnlyRegex("rev-ai-api-key", "Rev.ai API Key", "high", []string{"rev.ai", "rev ai"}, `(?i)\b(?:rev\.ai|rev[ _-]?ai)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyRevAI),
		...
		NewRegex("survicate-api-key", "Survicate API Key", "high", []string{"survicate"}, `(?i)\bsurvicate\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9]{32})\b`, 1, verifySurvicate),
		...
		NewRegex("upwave-api-key", "Upwave API Key", "high", []string{"upwave"}, `(?i)\bupwave\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9]{32})\b`, 1, verifyUpwave),
		...
		NewRegex("webscraper-api-key", "WebScraper API Key", "high", []string{"webscraper", "web scraper"}, `(?i)\b(?:webscraper|web[ _-]?scraper)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{60})\b`, 1, verifyWebScraper),
		...
		NewReadOnlyRegex("vercel-token", "Vercel Token", "critical", []string{"vercel", "VERCEL_TOKEN"}, `(?i)\bvercel.{0,40}['\"\s:=]+([A-Za-z0-9_-]{24,64})\b`, 1, verifyVercel),
		NewReadOnlyRegex("railway-token", "Railway Token", "critical", []string{"railway"}, `(?i)\brailway.{0,40}['\"\s:=]+([A-Za-z0-9_-]{24,64})\b`, 1, verifyRailway),
		NewAuthOnlyRegex("render-api-key", "Render API Key", "critical", []string{"render.com", "render"}, `(?i)\b(?:render\.com|api\.render\.com|render[_-]?api)\b[\s\S]{0,160}\b(?:api[_-]?key|api[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyRender),
		NewReadOnlyRegex("koyeb-api-token", "Koyeb API Token", "critical", []string{"koyeb", "api.koyeb.com"}, `(?i)\b(?:koyeb|api\.koyeb\.com)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyKoyeb),
		NewRegex("northflank-api-token", "Northflank API Token", "critical", []string{"northflank", "api.northflank.com"}, `(?i)\b(?:northflank|api\.northflank\.com)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyNorthflank),
		NewAuthOnlyRegexWithTrailingBoundary("qovery-api-token", "Qovery API Token", "critical", []string{"qov_", "qovery"}, `\b(qov_[A-Za-z0-9._-]{20,256})`, 1, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789._-", verifyQovery),
		NewRegex("porter-api-token", "Porter API Token", "critical", []string{"porter.run", "porter"}, `(?i)\b(?:porter\.run|dashboard\.porter\.run|porter[_-]?api)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, nil),
		NewRegex("envkey-api-key", "EnvKey API Key", "critical", []string{"envkey", "envkey.com"}, `(?i)\b(?:envkey|envkey\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|org[_-]?token|server[_-]?key|envkey|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._~/-]{32,256})\b`, 1, nil),
		NewRegex("akeyless-access-secret", "Akeyless Access Secret", "critical", []string{"akeyless", "akeyless.io"}, `(?i)\b(?:akeyless|akeyless\.io|api\.akeyless\.io)\b[\s\S]{0,160}\b(?:access[_-]?secret|access[_-]?id|api[_-]?key|token|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9._~/-]{32,256})\b`, 1, nil),
		NewRegex("platformsh-api-token", "Platform.sh API Token", "critical", []string{"platform.sh", "platformsh"}, `(?i)\b(?:platform\.sh|platformsh|api\.platform\.sh)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyPlatformSH),
		...
		NewReadOnlyRegex("travisci-token", "Travis CI Token", "high", []string{"travis", "TRAVIS_TOKEN"}, `(?i)\btravis(?:ci)?.{0,40}['\"\s:=]+([A-Za-z0-9]{22})\b`, 1, verifyTravisCI),
		...
		NewRegex("helpscout-api-key", "Help Scout API Key", "high", []string{"helpscout", "help scout"}, `(?i)\b(?:helpscout|help[ _-]?scout).{0,40}['\"\s:=]+([A-Za-z0-9]{40})\b`, 1, verifyHelpScout),
		NewReadOnlyRegex("mailerlite-api-key", "MailerLite API Key", "high", []string{"mailerlite", "mailer lite"}, `(?i)\b(?:mailerlite|mailer[ _-]?lite).{0,40}['\"\s:=]+([A-Za-z0-9]{32})\b`, 1, verifyMailerLite),
		NewReadOnlyRegexWithTrailingBoundary("mandrill-api-key", "Mandrill API Key", "high", []string{"mandrill"}, `(?i)\bmandrill.{0,40}['\"\s:=]+([A-Za-z0-9_-]{22})`, 1, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_-", verifyMandrill),
		...
		NewReadOnlyRegex("capsulecrm-api-key", "Capsule CRM API Key", "high", []string{"capsule"}, `(?i)\bcapsule(?:crm)?.{0,40}['\"\s:=]+([A-Za-z0-9]{32})\b`, 1, verifyCapsuleCRM),
		...
		CorrelatedDetector{
			...
			PrimaryPart: "app_secret", MaxDistance: 256, StopAtBlankLine: true, CompositeVerifier: verifyLarkSuiteCredentials, VerificationSafety: VerificationSafetyAuthOnly,
		},
...
func verifyHTTPRequestWithClassifier(ctx context.Context, req *http.Request, classifier verificationResponseClassifier) VerificationResult {
	...
	case resp.StatusCode >= 200 && resp.StatusCode < 300:
		return VerificationResult{Status: VerificationVerified, Response: response}
	case resp.StatusCode >= 300 && resp.StatusCode < 400:
		return VerificationResult{Status: VerificationUnknown, ErrorCategory: "redirect", Message: "provider redirected verification", Response: response}
	case resp.StatusCode == http.StatusTooManyRequests:
		return VerificationResult{Status: VerificationUnknown, ErrorCategory: "rate_limited", Message: "provider rate limited verification", Response: response}
	case resp.StatusCode >= 500:
		return VerificationResult{Status: VerificationUnknown, ErrorCategory: "provider", Message: "provider unavailable", Response: response}
	case resp.StatusCode == http.StatusForbidden:
		return VerificationResult{Status: VerificationUnknown, ErrorCategory: "authorization", Message: "provider could not authorize verification", Response: response}
	default:
		return VerificationResult{Status: VerificationUnverified, ErrorCategory: "invalid_credentials", Message: "provider rejected credential", Response: response}
	}
}

type verificationHTTPClientKey struct{}

func WithVerificationHTTPClient(ctx context.Context, client *http.Client) context.Context {
	return context.WithValue(ctx, verificationHTTPClientKey{}, client)
}

func verificationHTTPClient(ctx context.Context) *http.Client {
	client, _ := ctx.Value(verificationHTTPClientKey{}).(*http.Client)
	return client
}

func truncateVerificationResponse(response string) string {
