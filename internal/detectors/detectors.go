...
func DefaultRegistry() []Detector {
	return []Detector{
		...
		NewRegex("datocms-api-token", "DatoCMS API Token", "critical", []string{"datocms", "dato.cms"}, `(?i)\b(?:datocms|dato[ _-]?cms|site-api\.datocms\.com)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|read[_-]?only[_-]?token|full[_-]?access[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyDatoCMS),
		...
		NewRegex("courier-api-key", "Courier API Key", "critical", []string{"courier", "pk_"}, `(?i)\bcourier[A-Za-z0-9_-]*.{0,80}\b(pk_[A-Za-z0-9]+_[A-Za-z0-9]{28})\b`, 1, verifyCourier),
		...
		NewRegex("harvest-access-token", "Harvest Access Token", "critical", []string{"harvest"}, `(?i)\bharvest\b.{0,80}\b(?:access[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,128})\b`, 1, verifyHarvest),
		...
		NewRegex("workable-api-token", "Workable API Token", "critical", []string{"workable.com", "workable"}, `(?i)\b(?:workable\.com|api\.workable\.com|workable)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|access[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyWorkable),
		...
		NewRegex("easypost-api-key", "EasyPost API Key", "critical", []string{"easypost", "api.easypost.com"}, `(?i)\b(?:easypost|api\.easypost\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|api[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?((?:EZAK|EZTK)[A-Za-z0-9._-]{24,256}|[A-Za-z0-9._-]{32,256})\b`, 1, verifyEasyPost),
		...
		NewRegex("everhour-api-key", "Everhour API Key", "high", []string{"everhour"}, `(?i)\beverhour\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyEverhour),
		...
		NewRegex("flatio-api-key", "Flatio API Key", "high", []string{"flatio"}, `(?i)\bflatio\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyFlat),
		...
		NewRegex("humanity-api-key", "Humanity API Key", "high", []string{"humanity"}, `(?i)\bhumanity\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9]{40})\b`, 1, verifyHumanity),
		...
		NewRegex("render-api-key", "Render API Key", "critical", []string{"render.com", "render"}, `(?i)\b(?:render\.com|api\.render\.com|render[_-]?api)\b[\s\S]{0,160}\b(?:api[_-]?key|api[_-]?token|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyRender),
