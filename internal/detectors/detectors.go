...
func DefaultRegistry() []Detector {
	return []Detector{
		...
		NewRegex("chroma-cloud-api-key", "Chroma Cloud API Key", "critical", []string{"trychroma", "chroma"}, `(?i)\b(?:trychroma|api\.trychroma\.com|chroma[_-]?cloud)\b[\s\S]{0,160}\b(?:api[_-]?key|authorization|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyChromaCloud),
		...
		NewRegex("singlestore-api-key", "SingleStore API Key", "critical", []string{"singlestore", "singlestore.com"}, `(?i)\b(?:singlestore|single[ _-]?store|api\.singlestore\.com)\b[\s\S]{0,160}\b(?:api[_-]?key|management[_-]?token|password|token|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9._~/-]{32,256})\b`, 1, verifySingleStore),
		...
		NewRegex("imagekit-private-key", "ImageKit Private Key", "critical", []string{"imagekit", "private_"}, `(?i)\bimagekit\b.{0,80}\b(?:private[_-]?key|api[_-]?private[_-]?key)\b\s*[:=]\s*['\"]?(private_[A-Za-z0-9]{24,80})\b`, 1, verifyImageKit),
		...
		NewRegex("lob-api-key", "Lob API Key", "critical", []string{"lob", "live_", "test_"}, `(?i)\blob\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?((?:live|test)_[A-Za-z0-9]{35})\b`, 1, verifyLob),
		...
		NewRegex("qase-api-token", "Qase API Token", "critical", []string{"qase"}, `(?i)\bqase\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyQase),
		...
		NewRegex("oanda-api-token", "OANDA API Token", "critical", []string{"oanda"}, `(?i)\boanda\b.{0,80}\b(?:api[_-]?token|personal[_-]?access[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyOANDA),
		...
		NewRegex("transferwise-api-token", "Wise API Token", "critical", []string{"transferwise", "wise"}, `(?i)\b(?:transferwise|wise)\b.{0,80}\b(?:api[_-]?token|token|access[_-]?token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyWise),
