...
func DefaultRegistry() []Detector {
	return []Detector{
		...
		NewRegex("clerk-secret-key", "Clerk Secret Key", "critical", []string{"CLERK_SECRET_KEY", "clerk.com", "clerk"}, `(?i)\b(?:clerk[_-]?secret[_-]?key|(?:clerk\.com|clerk)\b.{0,80}\b(?:secret[_-]?key|api[_-]?key))\b\s*[:=]\s*['\"]?(sk_(?:test|live)_[A-Za-z0-9]{20,128})\b`, 1, nil),
		...
		NewRegex("liveblocks-secret-key", "Liveblocks Secret Key", "critical", []string{"LIVEBLOCKS_SECRET_KEY", "liveblocks"}, `(?i)\b(?:liveblocks[_-]?secret[_-]?key|liveblocks\b.{0,80}\b(?:secret[_-]?key|api[_-]?key))\b\s*[:=]\s*['\"]?(sk_(?:test|prod)_[A-Za-z0-9_-]{20,128})\b`, 1, nil),
