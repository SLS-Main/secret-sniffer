...
func DefaultRegistry() []Detector {
	return []Detector{
		...
		NewRegex("deepgram-api-key", "Deepgram API Key", "critical", []string{"deepgram", "DEEPGRAM_API_KEY"}, `(?i)\bdeepgram.{0,40}['\"\s:=]+([0-9a-z]{40})\b`, 1, verifyDeepgram),
		...
		NewRegex("resend-api-key", "Resend API Key", "critical", []string{"RESEND_API_KEY", "api.resend.com", "resend"}, `(?i)\b(?:resend[_-]?api[_-]?key|(?:api\.resend\.com|resend)\b.{0,80}\b(?:api[_-]?key|authorization|bearer))\b\s*[:=]\s*['\"]?(re_[A-Za-z0-9_-]{20,128})\b`, 1, verifyResend),
		...
		NewRegex("workos-api-key", "WorkOS API Key", "critical", []string{"WORKOS_API_KEY", "api.workos.com", "workos"}, `(?i)\b(?:workos[_-]?api[_-]?key|(?:api\.workos\.com|workos)\b.{0,80}\b(?:api[_-]?key|secret[_-]?key))\b\s*[:=]\s*['\"]?(sk_(?:test|live)_[A-Za-z0-9]{20,128})\b`, 1, verifyWorkOS),
		NewRegex("liveblocks-secret-key", "Liveblocks Secret Key", "critical", []string{"LIVEBLOCKS_SECRET_KEY", "liveblocks"}, `(?i)\b(?:liveblocks[_-]?secret[_-]?key|liveblocks\b.{0,80}\b(?:secret[_-]?key|api[_-]?key))\b\s*[:=]\s*['\"]?(sk_(?:test|prod)_[A-Za-z0-9_-]{20,128})\b`, 1, verifyLiveblocks),
		...
		NewRegex("motherduck-token", "MotherDuck Token", "critical", []string{"motherduck", "motherduck.com"}, `(?i)\b(?:motherduck|motherduck\.com)\b[\s\S]{0,160}\b(?:token|access[_-]?token|api[_-]?key|md[_-]?token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,512})\b`, 1, verifyMotherDuck),
		...
		NewRegex("braintree-access-token", "Braintree Access Token", "critical", []string{"access_token$production$", "access_token$sandbox$"}, `\b(access_token\$(?:production|sandbox)\$[A-Za-z0-9_]+\$[A-Za-z0-9]{32})\b`, 1, verifyBraintree),
		...
		NewRegex("pushbullet-token", "Pushbullet Access Token", "high", []string{"pushbullet", "o."}, `(?i)\bpushbullet\b.{0,40}\b(?:access[_-]?token|token)\b\s*[:=]\s*['\"]?(o\.[A-Za-z0-9]{32})\b`, 1, verifyPushbullet),
		...
		NewRegex("replyio-api-key", "Reply.io API Key", "high", []string{"reply.io", "replyio"}, `(?i)\b(?:reply\.io|replyio)\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{24})\b`, 1, verifyReplyIO),
		...
		NewRegex("salesflare-api-key", "Salesflare API Key", "critical", []string{"salesflare"}, `(?i)\bsalesflare\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_]{45})\b`, 1, verifySalesflare),
		...
		NewRegex("onfleet-api-key", "Onfleet API Key", "critical", []string{"onfleet"}, `(?i)\bonfleet\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32,64})\b`, 1, verifyOnfleet),
		...
		NewRegex("toggltrack-api-token", "Toggl Track API Token", "high", []string{"toggl", "toggltrack"}, `(?i)\b(?:toggltrack|toggl)\b.{0,80}\b(?:api[_-]?token|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyTogglTrack),
		...
		NewRegex("whoxy-api-key", "Whoxy API Key", "high", []string{"whoxy"}, `(?i)\bwhoxy\b.{0,80}\b(?:api[_-]?key|key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{33})\b`, 1, verifyWhoxy),
		...
		NewRegex("signupgenius-api-key", "SignUpGenius API Key", "high", []string{"signupgenius", "signup genius"}, `(?i)\b(?:signupgenius|signup[ _-]?genius)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifySignUpGenius),
		...
		NewRegex("skrapp-api-key", "Skrapp API Key", "high", []string{"skrapp"}, `(?i)\bskrapp\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{42})\b`, 1, verifySkrapp),
		...
		NewRegex("abyssale-api-key", "Abyssale API Key", "high", []string{"abyssale"}, `(?i)\babyssale\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{40})\b`, 1, verifyAbyssale),
		...
		NewRegex("aletheia-api-key", "Aletheia API Key", "high", []string{"aletheia"}, `(?i)\baletheia\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Z0-9]{32})\b`, 1, verifyAletheia),
		NewRegex("allsports-api-key", "AllSports API Key", "high", []string{"allsports", "all sports"}, `(?i)\b(?:allsports|all[ _-]?sports)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9]{64})\b`, 1, verifyAllSports),
		...
		NewRegex("api2cart-api-key", "API2Cart API Key", "high", []string{"api2cart"}, `(?i)\bapi2cart\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyAPI2Cart),
		...
		NewRegex("apimetrics-api-key", "APImetrics API Key", "high", []string{"apimetrics"}, `(?i)\bapimetrics\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{32})\b`, 1, verifyAPIMetrics),
		...
		NewRegex("borgbase-api-key", "BorgBase API Key", "critical", []string{"borgbase"}, `(?i)\bborgbase\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{32,128})\b`, 1, verifyBorgBase),
		NewRegex("buddyns-api-key", "BuddyNS API Key", "high", []string{"buddyns", "buddy ns"}, `(?i)\b(?:buddyns|buddy[ _-]?ns)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9]{40})\b`, 1, verifyBuddyNS),
		...
		NewRegex("bugherd-api-key", "BugHerd API Key", "high", []string{"bugherd"}, `(?i)\bbugherd\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9]{22})\b`, 1, verifyBugHerd),
		...
		NewRegex("calorieninja-api-key", "CalorieNinjas API Key", "high", []string{"calorieninja", "calorieninjas", "calorie ninjas"}, `(?i)\b(?:calorie[ _-]?ninjas?|calorieninjas?)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{40})\b`, 1, verifyCalorieNinjas),
		...
		NewRegex("chatbot-api-key", "ChatBot API Key", "critical", []string{"chatbot"}, `(?i)\bchatbot\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_]{32})\b`, 1, verifyChatBot),
		...
		NewRegex("avaza-api-token", "Avaza API Token", "high", []string{"avaza"}, `(?i)\bavaza\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([0-9]+-[a-f0-9]{40})\b`, 1, verifyAvaza),
		...
		NewRegex("codequiry-api-key", "Codequiry API Key", "high", []string{"codequiry"}, `(?i)\bcodequiry\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9-]{64})\b`, 1, verifyCodequiry),
		...
		NewRegex("column-api-key", "Column API Key", "critical", []string{"column"}, `(?i)\bcolumn\b.{0,80}\b(?:api[_-]?key|key|token|secret)\b\s*[:=]\s*['\"]?((?:test|live)_[A-Za-z0-9]{27})\b`, 1, verifyColumn),
		...
		NewRegex("commodities-api-key", "Commodities API Key", "high", []string{"commodities"}, `(?i)\bcommodities\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyCommoditiesAPI),
		...
		NewRegex("countrylayer-api-key", "Countrylayer API Key", "high", []string{"countrylayer"}, `(?i)\bcountrylayer\b.{0,80}\b(?:api[_-]?key|access[_-]?key|key)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyCountryLayer),
		...
		NewRegex("dandelion-api-key", "Dandelion API Key", "high", []string{"dandelion"}, `(?i)\bdandelion\b.{0,80}\b(?:api[_-]?key|token|key)\b\s*[:=]\s*['\"]?([a-z0-9]{32})\b`, 1, verifyDandelion),
		...
		NewRegex("diggernaut-api-key", "Diggernaut API Key", "high", []string{"diggernaut"}, `(?i)\bdiggernaut\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9]{40})\b`, 1, verifyDiggernaut),
		...
		NewRegex("docparser-api-key", "Docparser API Key", "high", []string{"docparser"}, `(?i)\bdocparser\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{40})\b`, 1, verifyDocparser),
		...
		NewRegex("finage-api-key", "Finage API Key", "high", []string{"finage"}, `(?i)\bfinage\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?(API_KEY[A-Z0-9]{32})\b`, 1, verifyFinage),
		...
		NewRegex("flightapi-key", "FlightAPI Key", "high", []string{"flightapi", "flight api"}, `(?i)\b(?:flightapi|flight[ _-]?api)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9]{24})\b`, 1, verifyFlightAPI),
		...
		NewRegex("geocodify-api-key", "Geocodify API Key", "high", []string{"geocodify"}, `(?i)\bgeocodify\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9]{40})\b`, 1, verifyGeocodify),
		...
		NewRegex("getgeoapi-key", "GetGeoAPI Key", "high", []string{"getgeoapi", "get geo api"}, `(?i)\b(?:getgeoapi|get[ _-]?geo[ _-]?api)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9]{40})\b`, 1, verifyGetGeoAPI),
		...
		NewRegex("glassnode-api-key", "Glassnode API Key", "high", []string{"glassnode"}, `(?i)\bglassnode\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{27})\b`, 1, verifyGlassnode),
		...
		NewRegex("groovehq-api-key", "GrooveHQ API Key", "high", []string{"groovehq", "groove hq"}, `(?i)\b(?:groovehq|groove[ _-]?hq)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{64})\b`, 1, verifyGrooveHQ),
		NewRegex("gtmetrix-api-key", "GTmetrix API Key", "high", []string{"gtmetrix"}, `(?i)\bgtmetrix\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyGTmetrix),
		...
		NewRegex("ibmcloud-user-key", "IBM Cloud User Key", "critical", []string{"ibmcloud", "ibm cloud"}, `(?i)\b(?:ibmcloud|ibm[ _-]?cloud)\b.{0,120}\b(?:user[_-]?key|api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyIBMCloud),
		NewRegex("iconfinder-api-key", "Iconfinder API Key", "high", []string{"iconfinder"}, `(?i)\biconfinder\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{64})\b`, 1, verifyIconfinder),
		...
		NewRegex("juro-api-key", "Juro API Key", "critical", []string{"juro"}, `(?i)\bjuro\b.{0,80}\b(?:api[_-]?key|key|token|secret)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{40})\b`, 1, verifyJuro),
		...
		NewRegex("knapsackpro-api-token", "Knapsack Pro API Token", "high", []string{"knapsackpro", "knapsack pro"}, `(?i)\b(?:knapsackpro|knapsack[ _-]?pro)\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([a-z0-9]{32})\b`, 1, verifyKnapsackPro),
		...
		NewRegex("kylas-api-key", "Kylas API Key", "high", []string{"kylas"}, `(?i)\bkylas\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-z0-9-]{36})\b`, 1, verifyKylas),
		NewRegex("leadfeeder-api-key", "Leadfeeder API Key", "high", []string{"leadfeeder"}, `(?i)\bleadfeeder\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9-]{43})\b`, 1, verifyLeadfeeder),
		...
		NewRegex("madkudu-api-key", "MadKudu API Key", "high", []string{"madkudu"}, `(?i)\bmadkudu\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([a-f0-9]{32})\b`, 1, verifyMadKudu),
		...
		NewRegex("mindmeister-api-token", "MindMeister API Token", "high", []string{"mindmeister", "mind meister"}, `(?i)\b(?:mindmeister|mind[ _-]?meister)\b.{0,80}\b(?:api[_-]?token|token|api[_-]?key)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{43})\b`, 1, verifyMindMeister),
		...
		NewRegex("moralis-api-key", "Moralis API Key", "critical", []string{"moralis"}, `(?i)\bmoralis\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9]{64})\b`, 1, verifyMoralis),
		...
		NewRegex("newscatcher-api-key", "NewsCatcher API Key", "high", []string{"newscatcher", "news catcher"}, `(?i)\b(?:newscatcher|news[ _-]?catcher)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_]{43})\b`, 1, verifyNewsCatcher),
		...
		NewRegex("nvapi-key", "NVAPI Key", "high", []string{"nvapi"}, `(?i)\bnvapi\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?(nvapi-[A-Za-z0-9_-]{64})\b`, 1, verifyNVAPI),
		...
		NewRegex("reachmail-api-key", "ReachMail API Key", "high", []string{"reachmail", "reach mail"}, `(?i)\b(?:reachmail|reach[ _-]?mail)\b.{0,80}\b(?:api[_-]?key|key|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9_-]{64})\b`, 1, verifyReachMail),
		...
		NewRegex("qovery-api-token", "Qovery API Token", "critical", []string{"qovery", "api.qovery.com"}, `(?i)\b(?:qovery|api\.qovery\.com)\b[\s\S]{0,160}\b(?:api[_-]?token|api[_-]?key|bearer|token)\b\s*[:=]\s*['\"]?([A-Za-z0-9._-]{32,256})\b`, 1, verifyQovery),
		...
		NewRegex("mandrill-api-key", "Mandrill API Key", "high", []string{"mandrill"}, `(?i)\bmandrill.{0,40}['\"\s:=]+([A-Za-z0-9_-]{20,40})\b`, 1, verifyMandrill),
